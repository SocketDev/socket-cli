    ...
    visitor: {
      CallExpression(nodePath, state) {
        ...
        const hasInlineDirective = leadingComments.some((comment) => {
          return comment.type === "CommentBlock" && comment.value.trim() === "@__INLINE__";
        });
        ...
        try {
          ...
          if (!isSerializable(value)) {
            throw new Error(
              "Cannot inline require: value is not serializable (got " + typeof value + ")",
            );
          }
          ...
          t.addComment(replacement, "trailing", ` was: require('${requirePath}') `, false);
