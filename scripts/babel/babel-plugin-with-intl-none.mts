    ...
    visitor: {
      ...
      CallExpression(path) {
        ...
        if (
          ...
          const isNumber =
            ...
            (t.isIdentifier(objectType.node) &&
              ["count", "size", "length", "total", "num", "number", "amount", "bytes"].some((n) =>
                objectType.node.name.toLowerCase().includes(n),
              ));

          if (isNumber) {
            ensureHelper(path, "__formatNumber");
            path.replaceWith(
              t.callExpression(t.identifier("__formatNumber"), [node.callee.object]),
            );
          ...
          if (
            ...
            path.replaceWith(
              t.callExpression(t.identifier("__formatDateTime"), [node.callee.object]),
            );
            stats.toLocaleString++;
        ...
        if (
          ...
          path.replaceWith(t.callExpression(t.identifier("__formatDate"), [node.callee.object]));
        ...
        if (
          ...
          path.replaceWith(
            t.callExpression(
              t.memberExpression(
                t.callExpression(
                  t.memberExpression(node.callee.object, t.identifier("toISOString")),
                  [],
                ),
                t.identifier("split"),
              ),
          ...
          path.replaceWith(t.memberExpression(path.node, t.numericLiteral(1), true));
              ...
              t.objectExpression([
                t.objectMethod(
                  ...
                  t.blockStatement([
                    t.returnStatement(
                      t.callExpression(
                        t.memberExpression(t.identifier("date"), t.identifier("toISOString")),
                        [],
                      ),
                    ),
                  ]),
            ...
            path.replaceWith(
              t.objectExpression([
                t.objectMethod(
                  ...
                  t.blockStatement([
                    t.returnStatement(
                      t.callExpression(t.identifier("__formatNumber"), [t.identifier("num")]),
                    ),
                  ]),
                ),
              ]),
      ...
      RegExpLiteral(path) {
        ...
        if (node.flags.includes("v")) {
          ...
          if (pattern.includes("\\p{")) {
            ...
            stats.unicodeRegex++;
            path.addComment("leading", " --with-intl=none: Downgraded /v flag → /u flag");
            return;
          }
        }
        ...
        if (
          ...
          pattern = pattern.replace(/\[([^\]]*\\p\{[^}]+\}[^\]]*)\]/g, (_match, content) => {
          ...
          for (const [unicode, basic] of Object.entries(standaloneTransforms)) {
            if (pattern.includes(unicode)) {
              pattern = pattern.replace(new RegExp(unicode.replace(/[\\{}]/g, "\\$&"), "g"), basic);
              transformed = true;
            }
          }
          ...
          if (transformed) {
            ...
            path.addComment("leading", " WARNING: Complex unicode regex may not work without ICU!");
            stats.unicodeRegex++;
          }
        }
      },
