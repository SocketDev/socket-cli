/*
 * @file Mint a short-lived GitHub App installation token for the Socket release
 * App. Dep-0 (node: builtins only) so it runs before any install, and plain
 * .mjs (no type stripping) so it never depends on the runner's Node version.
 * RS256 JWT (iss = the app Client ID) -> the org installation -> an
...
function die(message) {
  ...
  // A mint failure must stop the run immediately, from wherever it is detected.
  // Setting process.exitCode instead would let the caller keep going and write a
  // blank token to $GITHUB_OUTPUT, which fails much later and far less clearly.
...
function gh(method, path, jwt, body) {
  const headers = {
    ...
    'user-agent': 'socket-cli-release-app-token',
...
export function parsePermissions(rawInput) {
  ...
  try {
    ...
    throw new Error(
      `PERMISSIONS is not valid JSON. Where: the mint step's env. ` +
  ...
  if (
    ...
    throw new Error(
      `PERMISSIONS must be a non-empty JSON object. Where: the mint step's env. ` +
