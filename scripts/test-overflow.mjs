...
function showState(label) {
  ...
  console.log(
    `Invisible at bottom: ${Math.max(0, state.outputLines.length - visibleEnd)} lines`,
  )
}
...
if (state.outputLines.length <= layout.outputBoxContentHeight) {
  ...
  console.log(
    `Setting scrollOffset = ${state.scrollOffset} (showing bottom ${layout.outputBoxContentHeight} lines)`,
  )
}
...
console.log(
  `🔴 User sees lines ${state.scrollOffset + 1}-${state.outputLines.length} at the BOTTOM of the box.`,
)
console.log(
  `🔴 There's EMPTY SPACE at the top of the box where lines 1-${state.scrollOffset} should be!`,
)
console.log(
