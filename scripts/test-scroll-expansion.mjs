...
const state = {
  inputLines: ['> '],
  outputLines: Array(30).fill(null).map((_, i) => `Line ${i + 1}`),
  scrollOffset: 0,
}
...
function showState(label) {
  ...
  console.log(`First visible: "${state.outputLines[state.scrollOffset] || 'NONE'}"`)
  console.log(`Last visible: "${state.outputLines[Math.min(state.scrollOffset + layout.outputBoxContentHeight - 1, state.outputLines.length - 1)] || 'NONE'}"`)
}
...
console.log('⚠️  But scrollOffset is still 0, so we\'re still trying to show lines 1-25.')
...
if (state.outputLines.length <= newLayout.outputBoxContentHeight) {
  ...
  state.scrollOffset = state.outputLines.length - newLayout.outputBoxContentHeight
}
...
console.log(`⚠️  Showing lines ${state.scrollOffset + 1}-${Math.min(state.scrollOffset + newLayout.outputBoxContentHeight, state.outputLines.length)}`)
console.log(`⚠️  THE GAP: Lines 1-${state.scrollOffset} are NOT visible (scrolled off top)`)
