 ...
 */
async function runPackageTypeCheck(pkg: PackageInfo, quiet: boolean = false): Promise<number> {
...
async function main(): Promise<void> {
  try {
    ...
    if (!quiet) {
      logger.step(`Type checking ${packages.length} package${packages.length > 1 ? "s" : ""}`);
      // Blank line.
      logger.error("");
    }
