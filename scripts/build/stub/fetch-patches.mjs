...
const SOCKET_PATCHES_DIR = join(PATCHES_DIR, 'socket')
 ...
 */
async function fetchPatches(options = {}) {
  ...
  try {
    ...
    for (const version of targetVersions) {
      if (patchList[version]) {
        for (const patchFile of patchList[version]) {
          ...
          const patchContent = await fetchFile(patchUrl)

          await writeFile(localPath, patchContent)
    ...
    const dirs = globSync(nodeModulesPatches.replace('*', '*'))
