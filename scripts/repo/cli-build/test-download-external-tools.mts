...
const PLATFORM_MAP: Record<string, Record<string, string> | null> = {
  __proto__: null,
  'darwin-arm64': {
...
const TOOL_REPOS: Record<
  string,
  { owner: string; repo: string; version: string } | null
> = {
  __proto__: null,
 ...
 */
async function downloadTool(toolName: string, platform: string) {
...
async function extractFromTarGz(
  ...
  const sourcePath = path.join(binaryFile.parentPath, binaryFile.name)
...
async function main() {
  ...
  for (let i = 0, { length } = tools; i < length; i += 1) {
    ...
    try {
      ...
      logger.error(`Failed to download ${tool}: ${errorMessage(e)}`)
    }
  }
