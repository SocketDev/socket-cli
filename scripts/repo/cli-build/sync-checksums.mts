...
const bundleToolsSchema = Type.Object({
  tools: Type.Record(
    ...
    Type.Object({
      ...
      checksums: Type.Optional(Type.Record(Type.String(), Type.String())),
    }),
  ),
})
const releaseAssetsSchema = Type.Object({
...
async function fetchGitHubReleaseChecksums(
  ...
  if (!Value.Check(releaseAssetsSchema, release)) {
    throw new Error(`Invalid GitHub release metadata from ${apiUrl}`)
  }
  ...
  if (checksumsAsset) {
    ...
    try {
      ...
      logger.log(`  Failed to download checksums.txt: ${errorMessage(e)}`)
 ...
 */
export function parseChecksums(content: string) {
  const checksums: Record<string, string> = Object.create(null)
 ...
 * Main sync function.
 */
async function writeUpdatedChecksums(
  config: unknown,
  updated: number,
...
async function main() {
  ...
  const config = readBundleToolsMetadata()
  ...
  for (let i = 0, { length } = githubTools; i < length; i += 1) {
    ...
    if (!tool.repository) {
      throw new Error(
        `GitHub asset tool ${tool.key} has no repository in ${EXTERNAL_TOOLS_FILE}`,
      )
    }
...
function readBundleToolsMetadata() {
  ...
  if (!Value.Check(bundleToolsSchema, config)) {
    throw new Error(`Invalid tool metadata in ${EXTERNAL_TOOLS_FILE}`)
  }
  return config
}
