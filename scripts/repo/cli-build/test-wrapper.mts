...
async function main() {
  try {
    ...
    const externalToolVersions = Object.fromEntries(
    ...
    const configArgs = expandedArgs.some(
      arg => arg === '--config' || arg === '-c' || arg.startsWith('--config='),
    )
      ? []
      : ['--config', '.config/repo/cli/vitest.config.mts']
    const result = await spawn(
      vitestPath,
      ['run', ...configArgs, '--passWithNoTests', ...expandedArgs],
      spawnOptions,
    )
