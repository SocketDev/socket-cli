 ...
 */
async function buildPackage(pkg: BuildPackageConfig, force: boolean): Promise<BuildResult> {
 ...
 */
export async function buildTarget(target: string, buildArgs: string[]): Promise<BuildTargetResult> {
  const startTime = Date.now();
 ...
 */
async function runParallelBuilds(targetsToBuild: string[], buildArgs: string[]): Promise<void> {
  logger.log("");
 ...
 */
async function runSequentialBuilds(targetsToBuild: string[], buildArgs: string[]): Promise<void> {
  logger.log("");
  logger.log("=".repeat(60));
 ...
 */
async function runTargetedBuild(target: string, buildArgs: string[]): Promise<void> {
  // Check if this is a platform target (e.g., darwin-arm64).
  const platformInfo = parsePlatformTarget(target);
  if (platformInfo) {
