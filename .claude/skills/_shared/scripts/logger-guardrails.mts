...
export type GuardrailReason = "status-emoji" | "console-call" | "inline-logger" | "dynamic-import";
...
const CONSOLE_CALL_RE = /\bconsole\s*\.\s*(?:debug|error|info|log|trace|warn)\s*\(/g;

const INLINE_LOGGER_RE = /\bgetDefaultLogger\s*\(\s*\)\s*\.\s*[a-zA-Z_$]/g;

const DYNAMIC_IMPORT_RE = /(?<![a-zA-Z_$])import\s*\(/g;

function isInBundledRoot(relativePath: string, bundledRoots: readonly string[]): boolean {
...
export async function checkLoggerGuardrails(
  ...
  const matched = files.filter((file) => extensions.some((ext) => file.endsWith(ext)));
