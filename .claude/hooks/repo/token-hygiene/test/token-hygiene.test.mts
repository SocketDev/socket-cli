...
describe("token-hygiene hook", () => {
  describe("allows safe commands", () => {
    ...
    it("curl with auth piped to jq", () => {
      assert.equal(
        runHook('curl -sS -H "Authorization: Bearer $TOKEN" https://api.example.com | jq .name')
          .code,
        0,
      );
    });
    it("curl with auth redirected to file", () => {
      assert.equal(
        runHook('curl -sS -H "Authorization: Bearer $TOKEN" https://api.example.com > out.json')
          .code,
        0,
      );
    });
  ...
  describe("blocks curl with auth to unfiltered stdout", () => {
    it("plain curl -H Authorization", () => {
      const r = runHook('curl -sS -H "Authorization: Bearer $TOKEN" https://api.example.com');
      assert.equal(r.code, 2);
