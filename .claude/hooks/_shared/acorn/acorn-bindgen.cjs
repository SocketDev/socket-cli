...
/**
 * Parse `source`, compile `selector`, run the matcher, return a JSON-encoded
 * result string. Meant to be called from JavaScript as:
 *
...
exports.findNodeAround = function (code, pos, node_type, options_js) {
  try {
    ...
    var ptr1 = isLikeNone(node_type)
      ? 0
      : passStringToWasm0(node_type, wasm.__wbindgen_export_1, wasm.__wbindgen_export_2);
...
class WasmParser {
   ...
   * (native).
   *
   * The WASM path goes: options_js (JS object) → options_from_jsvalue
   * (Reflect-based reads, no serde_json) → parser → JSON string → JSON::parse
   * (one cheap JS-side parse) → JsValue handed back to JS as the AST root.
   *
   * @param {string} code
   * @param {any} options_js
   *
