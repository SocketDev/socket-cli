...
import { bypassPhrasePresent, readLastAssistantText, readUserText } from "../transcript.mts";

function writeTranscript(content: string): string {
  const dir = mkdtempSync(path.join(os.tmpdir(), "transcript-test-"));
...
test("bypassPhrasePresent: paraphrase does not count", () => {
  const f = writeTranscript(JSON.stringify({ role: "user", content: "please revert that" }));
  try {
    assert.equal(bypassPhrasePresent(f, "Allow revert bypass"), false);
  } finally {
    cleanup(f);
...
test("bypassPhrasePresent: array of equivalent spellings — any matches", () => {
  const variants = ["Allow soaktime bypass", "Allow soak time bypass", "Allow soak-time bypass"];
  for (let i = 0, { length } = variants; i < length; i += 1) {
    const present = variants[i]!;
    const f = writeTranscript(JSON.stringify({ role: "user", content: `please ${present} now` }));
    try {
...
test("bypassPhrasePresent: empty array returns false", () => {
  const f = writeTranscript(JSON.stringify({ role: "user", content: "Allow anything bypass" }));
  try {
...
test("readLastAssistantText: returns empty when no assistant turn", () => {
  const f = writeTranscript(JSON.stringify({ role: "user", content: "user only" }));
  try {
