...
test("findInvocation: matches push reached via && chain", () => {
  assert.ok(findInvocation("cd /x/depot && git push", { binary: "git", subcommand: "push" }));
});

test("findInvocation: matches push in a pipe chain", () => {
  assert.ok(findInvocation("ls | grep x && git push", { binary: "git", subcommand: "push" }));
});
...
test("commandsFor: binary-in-a-path is NOT the binary", () => {
  ...
  assert.deepStrictEqual(commandsFor('grep -n "codex --write" file.mts', "codex"), []);
});
