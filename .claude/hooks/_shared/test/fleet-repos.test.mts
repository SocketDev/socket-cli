...
import { FLEET_REPO_NAMES, isFleetRepo, slugFromRemoteUrl } from "../fleet-repos.mts";
...
test("slugFromRemoteUrl: SSH scp-like form", () => {
  assert.strictEqual(slugFromRemoteUrl("git@github.com:SocketDev/socket-cli.git"), "socket-cli");
});
...
test("slugFromRemoteUrl: HTTPS form without .git", () => {
  assert.strictEqual(slugFromRemoteUrl("https://github.com/SocketDev/depot"), "depot");
});
...
test("slugFromRemoteUrl: a fork under a different owner still yields the slug", () => {
  ...
  assert.strictEqual(slugFromRemoteUrl("git@github.com:someuser/socket-cli.git"), "socket-cli");
});
