/**
 * @file Hook-runtime helpers: disable-env check + prefixed stderr writer. Two
 *   responsibilities every hook needs:
 *
 *   1. `isHookDisabled(slug)` — check the canonical `SOCKET_<UPPER_SLUG>_DISABLED`
 *      env var so every hook gets a uniform kill switch. The hook's name is the
 *      only input; the env-var name is derived (kebab → upper-snake +
 *      `_DISABLED` suffix). Today 15 of 47 hooks have a manually-named disable
 *      env; this helper makes it free for every hook.
 *   2. `hookLog(slug, ...lines)` — write `[<slug>] <line>` to stderr. Hooks have
 *      long duplicated this prefix shape with
 *      `process.stderr.write(\`[hook-name] ...`)`; centralizing it keeps the
 *      format consistent and lets us evolve it later (color, level prefixes,
 *      etc.) in one file. No dependency on `@socketsecurity/lib-stable` — hooks
 *      load fast at PreTool- Use time and the lib's logger ships a chunk of
 *      code (spinners, color detection, header/footer) that's wasted on a
 ...
 * string transform — exposed for tests + for hooks that want to mention the
 ...
 *
 * HookLog('foo', 'first line', '', 'after blank') → [foo] first line\n \n [foo]
 * after blank\n.
 */
