 ...
 */
export function extractSnippet(text: string, index: number, length: number): string {
...
export async function runStopReminder(config: ReminderConfig): Promise<void> {
  ...
  const text = config.stripQuotedSpans ? stripQuotedSpans(fencesStripped) : fencesStripped;
