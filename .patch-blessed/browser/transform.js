...
const widgets = fs.readdirSync(__dirname + '/../lib/widgets')

const requireWidgets = widgets.reduce(function (out, name) {
...
const infoPath = path.resolve(__dirname, '..', 'usr', 'xterm-256color'),
  capPath = path.resolve(__dirname, '..', 'usr', 'xterm.termcap')

const infoPathFake = path.resolve(
  path.sep,
  'usr',
  'share',
  'terminfo',
