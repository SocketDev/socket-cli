/**
 * @file Build script for Socket CLI with Sentry. Delegates to rolldown config
 *   for actual build. Copies data/ and images from packages/cli.
 */
...
async function main() {
  try {
    ...
    let result = await spawn('node', ['.config/rolldown.cli-sentry.build.mts'], {
      shell: WIN32,
      stdio: 'inherit',
      cwd: rootPath,
    })
