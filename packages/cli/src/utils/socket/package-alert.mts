...
import { getOwn, hasOwn } from '@socketsecurity/lib-internal/objects'
import { resolvePackageName } from '@socketsecurity/lib-internal/packages'
...
export async function addArtifactToAlertsMap<T extends AlertsByPurl>(
  ...
  const name = resolvePackageName({
    name: artifact.name!,
    ...(artifact.namespace && { namespace: artifact.namespace }),
  })
