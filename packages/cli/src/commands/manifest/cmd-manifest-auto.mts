...
export async function run(
  ...
  if (!detected.count) {
    logger.fail("Was unable to discover any targets for which we can generate manifest files…");
    logger.log("");
    logger.log(
      "- Make sure this script would work with your target build (see `socket manifest --help` for your target).",
    );
    logger.log("- Make sure to run it from the correct dir (use --cwd to target another dir)");
