...
import { PACKAGE_LOCK_JSON, PNPM_LOCK_YAML, YARN_LOCK } from "../../constants/paths.mts";
...
export async function detectNodejsCdxgenSources(
  ...
  const [pnpmLockPath, npmLockPath, yarnLockPath, nodeModulesPath] = await Promise.all([
...
export function isNodejsCdxgenType(argvType: unknown): boolean {
  ...
  if (Array.isArray(argvType)) {
    return argvType.some((t) => typeof t === "string" && nodejsPlatformTypes.has(t));
  }
  return false;
}
...
export async function runCdxgen(argvObj: ArgvObject): Promise<DlxSpawnResult> {
  ...
  const yarnLockPath =
    pnpmLockPath || npmLockPath ? undefined : await findUp(YARN_LOCK, { onlyFiles: true });
  ...
  if (
    ...
    try {
      const synpResult = await spawnSynpDlx(["--source-file", `./${YARN_LOCK}`], {
        ...dlxOpts,
        agent,
      });
  ...
  const chainedSpawnPromise = cdxgenResult.spawnPromise.finally(async () => {
    ...
    if (outputPath) {
      ...
      if (
        ...
        logger.error(`Output path "${outputPath}" resolves outside the current working directory`);
        return;
      }
