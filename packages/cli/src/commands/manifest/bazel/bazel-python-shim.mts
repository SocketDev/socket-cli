...
let cached: PythonShimResult | undefined

export async function provisionPythonShim(): Promise<PythonShimResult> {
  if (cached) {
    return cached
  }
  const pythonOnPath = firstBinPath(
    await whichReal('python', { nothrow: true }),
  )
  if (pythonOnPath) {
    cached = { augmentedEnv: undefined, shimDir: undefined }
    return cached
  }
  const python3OnPath = firstBinPath(
    await whichReal('python3', { nothrow: true }),
  )
  if (!python3OnPath) {
  ...
  // delete and symlinkSync. Tolerate EEXIST when the link is back: the
...
// Test-only: clear the per-process cache so tests can re-mock whichReal.
export function resetPythonShimCacheForTests(): void {
  cached = undefined
}
