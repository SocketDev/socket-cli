...
import { getConfigValueOrUndef, isConfigFromFlag, updateConfigValue } from "../../util/config.mts";
...
export async function attemptLogin(apiBaseUrl: string | undefined, apiProxy: string | undefined) {
  ...
  if (!orgSlugs.length) {
    ...
    return {
      ok: false,
      message: "No organizations found. Please contact Socket support to set up your account.",
    };
  }
  ...
  if (wantToComplete) {
    ...
    if (setupCResult.ok) {
      logger.success("Tab completion will be enabled after restarting your terminal");
    } else {
      logger.fail(
