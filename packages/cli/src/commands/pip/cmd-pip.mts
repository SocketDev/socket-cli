 ...
 *
 * Defined via `defineHandoffCommand`. The pip-specific binary picker
 ...
 * 2. If invoked as `socket pip`, use 'pip' if it exists, else fall back to 'pip3'.
 * 3. If pip3 was requested but is missing, fall back to 'pip'.
 *
 * If neither binary is available, return the originally-requested name and
 * let the spawn fail naturally — the user gets a recognizable PATH error
 * instead of an opaque fallback.
 *
