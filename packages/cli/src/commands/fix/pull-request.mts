...
export async function cleanupSocketFixPrs(
      ...
      if (match.state === GQL_PR_STATE_MERGED) {
        ...
        try {
          ...
          if (success) {
            ...
            debug(`pr: failed to delete branch ${match.headRefName} for ${prRef}`);
          }
          /* c8 ignore stop */
        } catch (e) {
  ...
  if (cachesToSave.size) {
    await Promise.allSettled(
      Array.from(cachesToSave).map(({ 0: key, 1: data }) => writeCache(key, data)),
    );
  }
...
export async function getSocketFixPrsWithContext(
  ...
  try {
    ...
    const errorResult = handleGraphqlError(e, `listing PRs for ${owner}/${repo}`);
...
export async function openSocketFixPr(
  ...
  try {
    ...
    if (e instanceof RequestError) {
      const errors = (e.response?.data as { errors?: unknown | undefined } | undefined)?.errors;
