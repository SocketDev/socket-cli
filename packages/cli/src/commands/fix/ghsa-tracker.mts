 ...
 */
export async function isGhsaFixed(cwd: string, ghsaId: string): Promise<boolean> {
  try {
      ...
      if (err.code === "EEXIST" && attempt < 4) {
        ...
        try {
          ...
          if (!Number.isNaN(lockPid) && !isPidAlive(lockPid)) {
            ...
            debug(`ghsa-tracker: removing stale lock from dead process ${lockPid}`);
 ...
 * doesn't exist.
 */
export async function saveGhsaTracker(cwd: string, tracker: GhsaTracker): Promise<void> {
  const trackerPath = path.join(cwd, TRACKER_FILE);
