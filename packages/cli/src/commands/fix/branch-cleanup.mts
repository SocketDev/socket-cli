...
import { gitDeleteBranch, gitDeleteRemoteBranch } from "../../util/git/operations.mjs";
 ...
 */
export async function cleanupFailedPrBranches(branch: string, cwd: string): Promise<void> {
...
export async function cleanupStaleBranch(
  ...
  if (!deleted) {
    logger.error(`Failed to delete stale remote branch ${branch}, skipping ${ghsaId}.`);
 ...
 */
export async function cleanupSuccessfulPrLocalBranch(branch: string, cwd: string): Promise<void> {
