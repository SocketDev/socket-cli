...
export async function configureGithub(
  config: NonNullable<NonNullable<NonNullable<SocketJson["defaults"]>["scan"]>["github"]>,
...
export async function configureScan(
  config: NonNullable<NonNullable<NonNullable<SocketJson["defaults"]>["scan"]>["create"]>,
  cwd = process.cwd(),
): Promise<CResult<{ canceled: boolean }>> {
  ...
  const defaultBranchName = await input({
    ...
    default: config.branch || (await gitBranch(cwd)) || (await detectDefaultBranch(cwd)),
    required: false,
    // validate: async string => bool
  });
