...
import { detectDefaultBranch, getRepoName, gitBranch } from "../../util/git/operations.mts";
...
export { findDefaultBranchValueMisuse, hasLegacyDefaultBranchFlag, isBareIdentifier };
...
export async function run(
  ...
  if (typeof autoManifest !== "boolean") {
    if (sockJson.defaults?.scan?.create?.autoManifest !== undefined) {
      ...
      logger.info(`Using default --auto-manifest from ${SOCKET_JSON}:`, autoManifest);
    } else {
      autoManifest = false;
    }
  }
  ...
  if (updatedInput && orgSlug && targets.length) {
    logger.info("Note: You can invoke this command next time to skip the interactive questions:");
    logger.error("```");
    logger.error(`    socket scan create [other flags...] ${orgSlug} ${targets.join(" ")}`);
    logger.error("```");
    logger.error("");
    logger.info(
  ...
  const isUsingNonDefaultMemoryLimit =
    reachAnalysisMemoryLimit !== reachabilityFlags["reachAnalysisMemoryLimit"]?.default;
  ...
  if (reachAnalysisMemoryLimit !== undefined && Number.isNaN(validatedReachAnalysisMemoryLimit)) {
    throw new InputError(
  ...
  if (reachAnalysisTimeout !== undefined && Number.isNaN(validatedReachAnalysisTimeout)) {
    throw new InputError(
  ...
  await handleCreateNewScan({
    ...
    reach: {
      ...
      reachUseUnreachableFromPrecomputation: Boolean(reachUseUnreachableFromPrecomputation),
