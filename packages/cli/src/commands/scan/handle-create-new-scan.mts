...
export async function handleCreateNewScan({
  ...
  debug("notice", `Fetched ${supportedFilesCResult.data["size"]} supported file types`);
  ...
  if (basics) {
    ...
    if (!basicsResult.ok) {
      logger.warn("Socket-basics scan failed, continuing without SAST/secrets findings");
      ...
      if (basicsReport && existsSync(basicsReport)) {
        ...
        if (findings.containers) {
          logger.info(`  Found ${findings.containers} container vulnerabilities`);
        }
      }
    }
  }
  ...
  try {
    fullScanCResult = await fetchCreateOrgFullScan(
      ...
      orgSlug,
      {
        ...
        repoName,
        branchName,
        scanType: reach.runReachabilityAnalysis ? SCAN_TYPE_SOCKET_TIER1 : SCAN_TYPE_SOCKET,
        workspace,
      },
      {
        cwd,
