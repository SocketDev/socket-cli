...
export class OAuthIntrospector {
  ...
  async verifyAccessToken(token: string): Promise<AuthInfo | undefined> {
    ...
    return {
      clientId:
        typeof introspection["client_id"] === "string" ? introspection["client_id"] : "unknown",
      extra: introspection,
...
export function getForwardedHeaderValue(header: string | string[] | undefined): string {
...
export function getRequestBaseUrl(
  ...
  const forwardedProto = trustProxy
    ...
    : "";
  const forwardedHost = trustProxy ? getForwardedHeaderValue(req.headers["x-forwarded-host"]) : "";
  const host =
    forwardedHost || getRequestHeaderValue(req.headers.host).trim() || `localhost:${fallbackPort}`;
...
export function getRequestHeaderValue(header: string | string[] | undefined): string {
...
export function parseJsonObject(responseText: string, context: string): Record<string, unknown> {
  let parsed: unknown;
  try {
