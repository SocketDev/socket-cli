...
export async function runHttpTransport(config: HttpTransportConfig): Promise<void> {
  ...
  if (introspector) {
    try {
      ...
      logger.info(`Enabled OAuth-backed MCP auth with issuer ${config.oauthIssuer}`);
    } catch (e) {
      // loadMetadata only throws Error subclasses (httpRequest /
      // parseJsonObject / explicit throws); read .message directly.
      logger.error(`Failed to initialize OAuth metadata: ${(e as Error).message}`);
      throw e;
    }
  }
  ...
  const destroySession = (id: string): void => destroySessionEntry(id, sessions, logger);

  const tickReaper = () =>
    reapIdleSessions(Date.now(), SESSION_TTL_MS, sessions, destroySession, logger);
  ...
  const allowedOrigins = ["https://mcp.socket.dev", "https://mcp.socket-staging.dev"] as const;
  ...
  const httpServer = createServer(async (req, res) => {
    ...
    const isValidOrigin = origin
      ? isLocalhostOrigin(origin) || (allowedOrigins as readonly string[]).includes(origin)
      : isAllowedHost;
    ...
    if (introspector && url.pathname === OAUTH_PROTECTED_RESOURCE_METADATA_PATH) {
      ...
      writeJson(
        res,
        200,
        buildProtectedResourceMetadata(baseUrl, metadata, config.oauthRequiredScopes),
      );
      return;
    }
    ...
    if (req.method === "POST") {
      ...
      req.on("end", () =>
        handleRequestSafely("POST", res, logger, async () => {
          ...
          await transport.handleRequest(authenticatedReq as McpHandleRequest, res, jsonData);
        }),
      );
      return;
    }
    ...
    if (req.method === "GET") {
      ...
      await handleRequestSafely("GET", res, logger, async () => {
        ...
        await session.transport.handleRequest(authenticatedReq as McpHandleRequest, res);
      });
      return;
    }
