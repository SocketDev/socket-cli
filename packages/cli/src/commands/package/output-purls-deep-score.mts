...
export function createMarkdownReport(data: PurlDataResponse): string {
  const {
    self: { alerts: selfAlerts, capabilities: selfCaps, purl, score: selfScore },
    transitively: { alerts, capabilities, dependencyCount, func, lowest, score },
  } = data;
  ...
  if (selfAlerts.length) {
    ...
    o.push("");
    o.push(mdTable(selfAlerts, ["severity", "name"], ["Severity", "Alert Name"]));
  } else {
