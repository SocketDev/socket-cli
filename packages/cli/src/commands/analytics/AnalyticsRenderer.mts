...
export function displayAnalyticsWithIocraft(data: FormattedData): void {
  try {
      ...
      children: [
        ...
        Box({
          ...
          children: [
            ...
            ...Object.entries(data.top_five_alert_types).map(
              ({ 0: type, 1: count }) =>
                Box({
        ...
        Box({
          ...
          children: [
            ...
            ...Object.entries(data.total_critical_alerts).map(
              ({ 0: date, 1: count }) =>
                Box({
        ...
        Box({
          ...
          children: [
            ...
            ...Object.entries(data.total_high_alerts).map(
              ({ 0: date, 1: count }) =>
                Box({
                  children: [Text({ children: `  ${date}: ${count}` })],
                }),
    ...
    logger.log(
      `Top 5 Alert Types: ${Object.keys(data.top_five_alert_types).length} types`,
    )
    logger.log(
      `Critical Alerts: ${Object.keys(data.total_critical_alerts).length} dates`,
    )
    logger.log(
      `High Alerts: ${Object.keys(data.total_high_alerts).length} dates`,
    )
  }
}
