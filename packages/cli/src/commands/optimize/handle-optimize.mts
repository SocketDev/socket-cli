...
export async function handleOptimize({
  ...
  debug(`Detected package manager: ${pkgEnvDetails.agent} v${pkgEnvDetails.agentVersion}`);
    ...
    await outputOptimizeResult(
      {
        ok: false,
        message: "Unsupported",
        cause: cmdPrefixMessage(CMD_NAME, `${agent} v${agentVersion} does not support overrides.`),
      },
      outputKind,
    );
    return;
