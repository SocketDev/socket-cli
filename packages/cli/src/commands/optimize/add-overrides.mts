...
export async function addOverrides(
  ...
  const { agent, lockName, lockSrc, npmExecPath, pkgPath: rootPath } = pkgEnvDetails;
  const {
    logger,
    pin,
    prod,
  ...
  await pEach(
    ...
    async ({ 1: data }: { 1: { name: string; package: string; version: string } }) => {
      ...
      for (const { 1: depObj } of depEntries) {
        ...
        const origSpec = hasOwn(depObj, origPkgName) ? (depObj[origPkgName] as string) : undefined;
        if (origSpec) {
  ...
  if (state.added.size > 0 || state.updated.size > 0) {
    pkgEnvDetails.editablePkgJson.update(Object.fromEntries(depEntries) as PackageJson);
    if (isWorkspaceRoot) {
