/**
 * Socket-basics version getter function. Uses direct process.env access so
 * esbuild define can inline values. IMPORTANT: esbuild's define plugin can only
 * replace direct process.env['KEY'] references. If we imported from env
...
export function getSocketBasicsVersion(): string {
