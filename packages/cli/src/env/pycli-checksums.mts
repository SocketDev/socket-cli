...
export function getPyCliChecksums(): PyCliChecksums {
  const checksums = process.env['INLINED_PYCLI_CHECKSUMS']
  if (!checksums) {
  ...
  try {
    return JSON.parse(checksums) as PyCliChecksums
  } catch {
    throw new Error(
      'Failed to parse PyCLI checksums. This indicates a build configuration error.',
    )
  }
}
...
export function requirePyCliChecksum(assetName: string): string | undefined {
  ...
  if (!sha256) {
    throw new Error(
      `Missing SHA-256 checksum for PyCLI asset: ${assetName}. ` +
