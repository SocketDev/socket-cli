...
export const rootPath =
  __dirname.endsWith("dist") || __dirname.endsWith("build") ? srcPath : path.resolve(srcPath, "..");
...
export function getSocketAppDataPath(): string | undefined {
  ...
  let dataHome: string | undefined = isWin32 ? ENV.LOCALAPPDATA : ENV.XDG_DATA_HOME;
  if (!dataHome) {
    ...
    if (isWin32) {
      ...
      dataHome = path.join(home, isDarwin ? "Library/Application Support" : ".local/share");
    }
  }
...
export function getSocketCachePath(): string {
  ...
  switch (platform) {
    ...
    case "win32": {
      const tempDir = ENV.TEMP || ENV.TMP || path.join(home, "AppData", "Local", "Temp");
