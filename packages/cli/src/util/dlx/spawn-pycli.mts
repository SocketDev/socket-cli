 ...
 * @param retryCount Internal retry counter to prevent unbounded recursion.
 */
export async function ensureSocketPyCli(pythonBin: string, retryCount = 0): Promise<void> {
  const MAX_RETRIES = 3;
...
export function getPythonCachePath(): string {
  ...
  return path.join(getDlxCachePath(), "python", `${version}-${tag}-${platform}-${arch}`);
}
...
export function getPythonStandaloneInfo(): { assetName: string; url: string } {
  ...
  if (platform === "darwin") {
    platformTriple = arch === "arm64" ? "aarch64-apple-darwin" : "x86_64-apple-darwin";
  } else if (platform === "linux") {
    platformTriple = arch === "arm64" ? "aarch64-unknown-linux-gnu" : "x86_64-unknown-linux-gnu";
  } else if (platform === "win32") {
    // Windows ARM64 can use native ARM64 Python for better performance.
    platformTriple = arch === "arm64" ? "aarch64-pc-windows-msvc" : "x86_64-pc-windows-msvc";
  } else {
    throw new InputError(
...
export async function spawnSocketPyCli(
  ...
  try {
    ...
    const spawnResult = await spawn(pythonBin, ["-m", "socketsecurity.socketcli", ...args], {
      ...dlxOptions,
      env: spawnEnvFinal,
      shell: WIN32,
      stdio: "inherit",
