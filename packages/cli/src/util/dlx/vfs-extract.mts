...
/**
 * Extract external tools from the SEA's VFS into node-smol's shared dlx
 * directory, `~/.socket/_dlx/<node-smol-hash>/`.
 *
 * Standalone binaries land directly in that directory. An npm package keeps its
 * tree, so its binary ends up at `node_modules/<packageName>/bin/<binaryName>`.
 *
 * `depth` only guards the retry recursion. Callers pass nothing.
 *
 * @returns Tool names mapped to their extracted paths, or undefined when
 *   extraction failed.
 */
