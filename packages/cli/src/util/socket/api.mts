...
import { ConfigError, buildErrorCause, getNetworkErrorDiagnostics } from "../error/errors.mts";
...
export { getCommandRequirements, getErrorMessageForHttpStatusCode, logPermissionsFor403 };

export type { CommandRequirements } from "./api-error-messages.mts";

// The Socket API server that should be used for operations.
export function getDefaultApiBaseUrl(): string | undefined {
  const baseUrl = getSocketCliApiBaseUrl() || getConfigValueOrUndef(CONFIG_KEY_API_BASE_URL);
...
export async function handleApiCall<T extends SocketSdkOperations>(
  ...
  if (sdkResult.success === false) {
    ...
    const causeWithEndpoint = description ? `${cause} (endpoint: ${description})` : cause;
...
export async function queryApi(path: string, apiToken: string) {
  ...
  if (!baseUrl) {
    throw new ConfigError("Socket API base URL is not configured.", CONFIG_KEY_API_BASE_URL);
  }
  /* c8 ignore stop */

  return await socketHttpRequest(`${baseUrl}${baseUrl.endsWith("/") ? "" : "/"}${path}`, {
    method: "GET",
    headers: {
 ...
 */
export async function queryApiSafeJson<T>(path: string, description = ""): Promise<CResult<T>> {
