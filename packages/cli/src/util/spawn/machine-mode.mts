...
const TOOLS: Record<string, ToolRules> = {
  ...
  pnpm: {
    ...
    subcommands: Object.fromEntries(PNPM_JSON_CMDS.map((c) => [c, ["--reporter=json"]])),
  ...
  yarn: {
    ...
    subcommands: Object.fromEntries(YARN_CLASSIC_JSON_CMDS.map((c) => [c, ["--json", "--silent"]])),
  },
  "yarn-berry": {
    env: {
    ...
    subcommands: Object.fromEntries(YARN_BERRY_JSON_CMDS.map((c) => [c, ["--json"]])),
  },
  zpm: {
    subcommands: {
...
export function applyMachineMode(input: MachineModeInput): MachineModeOutput {
  ...
  const subcommandArgs = input.subcommand ? rules.subcommands?.[input.subcommand] : undefined;
  if (subcommandArgs) {
