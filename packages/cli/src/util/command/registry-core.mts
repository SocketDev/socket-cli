...
export class CommandRegistry implements ICommandRegistry {
  ...
  list(parent?: string): CommandDefinition[] {
    ...
    const unique = Array.from(new Map(commands.map((cmd) => [cmd.name, cmd])).values());
   ...
   */
  async execute(commandName: string, args: string[]): Promise<CResult<unknown>> {
    const command = this.commands.get(commandName);
   ...
   */
  private async parseFlags(command: CommandDefinition, args: string[]): Promise<FlagValues> {
