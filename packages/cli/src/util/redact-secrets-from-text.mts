...
export const SECRET_REDACTION_PATTERNS: readonly SecretRedactionPattern[] = [
  {
    ...
    re: /(\b[A-Za-z0-9_-]{0,64}(?:api[_-]?key|access[_-]?key(?:[_-]?id)?|token|secret|credential|signature|sig|password|passwd)\b(?:\\?["'])?\s*[:=]\s*(?:\\?["'])?)[^\s"',;}&\\\]]+/giu,
  ...
  },
  {
    ...
    name: 'github-token',
    re: /(?:github_pat_|gh[pousr]_)[A-Za-z0-9_-]{8,}/giu,
    replace: SECRET_REDACTION_PLACEHOLDER,
  },
  {
  ...
  },
  {
    ...
    re: /((?:[?&]|%3F|%26)(?:(?!%3F|%26|%3D)[A-Za-z0-9_.%[\]-]){0,64}(?:api[_-]?key|access(?:[_-]|%5F|%2D)?key(?:(?:[_-]|%5F|%2D)?id)?|token|secret|credential|signature|sig|password|passwd)(?:\]|%5D)?(?:=|%3D))(?:(?!%26)[^&\s])+/giu,
    replace: `$1${SECRET_REDACTION_PLACEHOLDER}`,
  },
]
