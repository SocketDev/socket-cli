 ...
 *   failed.
 */
export async function extractBasicsTools(_cacheDir?: string): Promise<string | undefined> {
  if (!isSeaBinary()) {
...
export function getBasicsToolPaths(toolsDir: string): {
  ...
  return {
    ...
    trivy: normalizePath(path.join(baseDlxDir, "trivy", isPlatWin ? "trivy.exe" : "trivy")),
