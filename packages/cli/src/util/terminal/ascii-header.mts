...
import type { Palette, RGB, ShimmerSpec } from "@socketsecurity/lib-stable/effects/shimmer";
 ...
 */
export function renderShimmerFrame(frame: number, theme: HeaderTheme = "default"): string {
  ...
  for (let i = 0; i < ASCII_LOGO.length; i++) {
    ...
    const secondaryColors = frameColors(spec, lineLength, frame + slantOffset + 35);
    const merged: RGB[] = primaryColors.map((c, idx) => brighterRgb(c, secondaryColors[idx]!));
