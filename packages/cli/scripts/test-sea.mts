...
async function buildBlob(configPath) {
  ...
  const result = await spawn(process.execPath, ["--experimental-sea-config", configPath], {
    stdio: "inherit",
  });
...
export async function generateSeaConfig(entryPoint, outputPath, toolPaths, mode) {
  ...
  const configPath = path.join(path.dirname(outputPath), `sea-config-${mode}-${outputName}.json`);
  const blobPath = path.join(path.dirname(outputPath), `sea-blob-${mode}-${outputName}.blob`);
    ...
    mode === "vfs"
      ...
      : Object.fromEntries(
          Object.entries(toolPaths)
            ...
            .map(([toolName, toolPath]) => [`external-tools/${toolName}`, toolPath]),
        );
...
async function loadToolPaths() {
  ...
  try {
    ...
    logger.fail(`Failed to parse tool paths from ${toolPathsFile}: ${e.message}`);
    logger.fail("Run: node scripts/test-download-external-tools.mts");
...
async function runStandaloneMode(platform, toolPaths) {
  ...
  logger.log("=".repeat(60));
  logger.log("");
  logger.log(`Tools (uncompressed): ${(totalToolSize / 1024 / 1024).toFixed(2)} MB`);
  logger.log(`Base Node binary: ${(baseStats.size / 1024 / 1024).toFixed(2)} MB`);
...
async function runVfsMode(platform) {
  ...
  const uncompressedTotal =
    uncompressedToolsSize + baseStats.size / 1024 / 1024 + blobStats.size / 1024 / 1024;
  const savings = uncompressedTotal - finalSizeMB;
  const compressionRatio = ((1 - finalSizeMB / uncompressedTotal) * 100).toFixed(1);
  ...
  logger.log(`Final SEA binary: ${finalSizeMB.toFixed(2)} MB`);
  logger.log(`Uncompressed size (Node SEA assets): ${uncompressedTotal.toFixed(2)} MB`);
...
async function runWithToolsMode(platform, toolPaths) {
  ...
  // Generate SEA config.
  const outputName = path.basename(outputPath, path.extname(outputPath));
  const configPath = path.join(path.dirname(outputPath), `sea-config-test-${outputName}.json`);
  const blobPath = path.join(path.dirname(outputPath), `sea-blob-test-${outputName}.blob`);
  ...
  for (const [toolName, toolPath] of Object.entries(toolPaths)) {
    if (existsSync(toolPath)) {
      ...
      const stats = await fs.stat(toolPath);
      logger.log(`  Including ${toolName}: ${(stats.size / 1024 / 1024).toFixed(2)} MB`);
    }
  }
  ...
  const nodeBinary = await downloadNodeBinary(nodeVersion, process.platform, process.arch);
  const nodeStats = await fs.stat(nodeBinary);
  logger.log(`Node binary size: ${(nodeStats.size / 1024 / 1024).toFixed(2)} MB`);
  logger.log("");
  ...
  logger.log(`Output: ${outputPath}`);
  logger.log("");
  logger.log(`Compression: ${compressionRatio}% reduction from uncompressed size`);
  logger.log("");
