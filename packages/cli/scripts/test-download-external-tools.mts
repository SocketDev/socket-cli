...
async function downloadTool(toolName, platform) {
  ...
  const outputDir = path.join(__dirname, "../../build-infra/build/external-tools-test", platform);
  ...
  if (existsSync(binaryPath)) {
    ...
    logger.log(`Already exists: ${binaryName} (${(stats.size / 1024 / 1024).toFixed(2)} MB)`);
    return binaryPath;
  }
...
async function extractFromTarGz(archivePath, outputPath, binaryName) {
  ...
  const binaryFile = files.find(
    (f) => f.isFile() && (f.name === binaryName || f.name === `${binaryName}.exe`),
  );
  ...
  const sourcePath = path.join(binaryFile.parentPath || binaryFile.path, binaryFile.name);
  ...
  logger.log(`Extracted: ${binaryName} (${(stats.size / 1024 / 1024).toFixed(2)} MB)`);
}
