...
// fs.stat() calls read .size for WASM bundle size reporting; not existence
// checks.
/* oxlint-disable-next-line socket/no-file-scope-oxlint-disable -- legitimate file-scope: domain-grouped layout or test fixture; per-call would produce many redundant disables. */
/* oxlint-disable socket/prefer-exists-sync -- reads .size */
...
async function buildWasm() {
  ...
  logger.error('')
  try {
    await execCommand(process.execPath, [convertScript], { stdio: 'inherit' })
  } catch (e) {
    logger.error('')
  ...
  try {
    ...
    await execCommand(process.execPath, buildArgs, { stdio: 'inherit' })
  } catch (e) {
    logger.error('')
...
Examples:
  ...
  node scripts/wasm.mts --build

  # Fast dev build for iteration (3-5x faster)
  node scripts/wasm.mts --build --dev
