...
async function installExtractedBinary(extractedPath, binaryPath, isPlatWin) {
  if (extractedPath !== binaryPath && existsSync(extractedPath)) {
    try {
      await fs.rename(extractedPath, binaryPath)
    } catch (e) {
      await fs.copyFile(extractedPath, binaryPath)
      await safeDelete(extractedPath)
    }
  } else if (!existsSync(binaryPath)) {
    throw new Error(`Binary not found after extraction: ${extractedPath}`)
  }
  if (!isPlatWin) {
...
async function installPythonWheel(toolsDir, pythonBinPath) {
...
async function installPythonBasics(toolsDir, pythonBinPath) {
...
async function preparePythonRuntime(toolsDir, isPlatWin) {
  ...
  if (!isPlatWin) {
    ...
    for (let i = 0, { length } = binFiles; i < length; i += 1) {
      ...
      // oxlint-disable-next-line socket/prefer-exists-sync -- chmod.
  ...
  await installPythonWheel(toolsDir, pythonBinPath)
  await installPythonBasics(toolsDir, pythonBinPath)

  // Don't clean up - keep the whole python directory.
  // We'll include the entire directory in the tar.gz.
}

async function installStandaloneTool(
  toolName,
  toolsDir,
  binaryName,
  archivePath,
...
async function downloadToolArchive(toolName, assetName, config, archivePath) {
...
export async function downloadAndInstallTool(
  ...
  await downloadToolArchive(toolName, assetName, config, archivePath)
  ...
  if (toolName === 'python') {
    await preparePythonRuntime(toolsDir, isPlatWin)
    installed.push('python')
  } else {
    const extractedName = toolName === 'opengrep' ? 'opengrep-core' : toolName
    const extractedPath = normalizePath(
      path.join(toolsDir, extractedName + (isPlatWin ? '.exe' : '')),
    )
    await installExtractedBinary(extractedPath, binaryPath, isPlatWin)
