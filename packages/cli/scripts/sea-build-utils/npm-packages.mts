...
async function combineVfsArchives(
  ...
  if (existsSync(combinedTarGz)) {
    ...
    if (stats.size < 1024) {
      logger.warn(`Cached combined VFS tar.gz is too small (${stats.size} bytes), rebuilding…`);
  ...
  try {
    ...
    if (externalToolsTarGz && existsSync(externalToolsTarGz)) {
      ...
      const tarResult = await spawn("tar", ["-xzf", externalToolsTarGz, "-C", vfsDir]);
      if (tarResult && tarResult.code !== 0) {
    ...
    const tarResult = await spawn("tar", ["-czf", combinedTarGz, "-C", vfsDir, ...contents]);
    ...
    logger.success(`Combined VFS archive: ${(tarStats.size / 1024 / 1024).toFixed(2)} MB`);
    logger.error("");
...
async function downloadNpmPackage(packageSpec, targetDir, expectedIntegrity) {
  ...
  try {
    ...
    throw new Error(`Failed to download ${packageSpec} with Arborist: ${e.message}`);
  }
...
async function downloadNpmPackages() {
  ...
  const tarGzPath = normalizePath(path.join(npmPackagesDir, "npm-packages.tar.gz"));
  ...
  if (existsSync(tarGzPath)) {
    ...
    // Validate cached file is not empty or suspiciously small (> 1KB).
    if (stats.size < 1024) {
      logger.warn(`Cached npm packages tar.gz is too small (${stats.size} bytes), rebuilding…`);
  ...
  const tempDir = normalizePath(path.join(npmPackagesDir, `temp-${process.pid}-${Date.now()}`));
  ...
  try {
    ...
    const tarResult = await spawn("tar", ["-czf", tarGzPath, "-C", tempDir, "node_modules"]);
    ...
    logger.success(`npm packages packaged: ${(tarStats.size / 1024 / 1024).toFixed(2)} MB`);
    logger.error("");
