 ...
 *   `packages/cli/test/smoke.sh`'s organization section (45 commands).
 *
 *   Covers: list / policy security / policy license / quota; --json
 *   contract conformance; --org overrides; missing-org and invalid-org
 *   error paths (achieved via per-call `--config` injection instead of
 *   mutating the real config file).
 *
...
describe('socket organization (e2e)', () => {
  ...
  describe('--org <real-default-org> (auth required, scratch-isolated)', () => {
    ...
    it.skipIf(!RUN || !hasAuth)(
      'organization policy license --org <real> exits 0',
      async () => {
        if (!defaultOrg) {
          return
        }
