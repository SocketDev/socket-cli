 ...
 *   on `RUN_E2E_TESTS=1`. The destructive logout run additionally gates on
 *   RUN_E2E_DESTRUCTIVE=1 even though scratch-isolation means it's safe —
 *   operator opt-in stays consistent with the repos.e2e file.
 */
...
describe("socket logout (e2e)", () => {
  ...
  it.skipIf(!RUN || !RUN_DESTRUCTIVE)("logout (scratch-isolated) exits 0", async () => {
