 ...
 *   - npm/pnpm/yarn caches are pinned to scratch too Gated on `RUN_E2E_TESTS=1`.
 *     Auth-required tests additionally require a Socket API token to be
 *     present.
 */
...
describe("socket scan (e2e)", () => {
  ...
  describe("scan create (auth required, scratch-isolated)", () => {
    it.skipIf(!RUN || !hasAuth)("scan create . exits 0 with --json contract", async () => {
      const result = await executeCliInScratch(["scan", "create", ".", "--json"], {
        seedFiles: {
          "package.json": JSON.stringify({
            name: "socket-cli-e2e-scan",
            version: "0.0.0",
          }),
        },
      });
  ...
  describe("error paths — non-existent org", () => {
    it.skipIf(!RUN || !hasAuth)("scan create --org fake_org exits 1", async () => {
      const result = await executeCliInScratch(
