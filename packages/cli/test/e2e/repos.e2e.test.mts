 ...
 *   `packages/cli/test/smoke.sh`'s repos section (17 commands).
 *
 *   Most repos checks are help / dry-run / read-only and run under the
 *   normal `RUN_E2E_TESTS=1` gate. The create → update → view → del
 *   round-trip writes real org-side state via the Socket API, so it lives
 *   behind a second gate (`RUN_E2E_DESTRUCTIVE=1`) and uses a
 *   `cli-e2e-<pid>-<timestamp>` repo name so concurrent runs don't collide
 *   and a stale repo from a crashed run is easy to identify.
 *
 *   afterAll best-effort deletes the test repo, but the round-trip's own
 *   `del` step is the primary cleanup; afterAll only catches mid-run aborts.
 */
