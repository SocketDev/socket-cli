...
describe("socket repos (e2e)", () => {
  ...
  describe("error paths (auth required, read-only, scratch-isolated)", () => {
    it.skipIf(!RUN || !hasAuth)(
      ...
      async () => {
        const result = await executeCliInScratch(["repos", "view", "cli_donotcreate", "--json"]);
  ...
  describe("create → update → view → del round-trip (destructive)", () => {
    ...
    it.skipIf(!RUN || !hasAuth || !RUN_DESTRUCTIVE)("creates a uniquely-named repo", async () => {
    ...
    it.skipIf(!RUN || !hasAuth || !RUN_DESTRUCTIVE)("views the created repo", async () => {
      const result = await executeCliInScratch(["repos", "view", repoName]);
      expect(result.code).toBe(0);
    });

    it.skipIf(!RUN || !hasAuth || !RUN_DESTRUCTIVE)("deletes the created repo", async () => {
