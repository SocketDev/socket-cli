...
describe("socket analytics (e2e)", () => {
  ...
  describe("default / org / repo / time-window (auth required, scratch-isolated)", () => {
    ...
    it.skipIf(!RUN)("analytics repo socket-cli 30 --markdown exits 0", async () => {
  ...
  describe("--file output (auth required, scratch-isolated)", () => {
    it.skipIf(!RUN)("analytics --file <scratch>/out.txt --json exits 0", async () => {
      const result = await executeCliInScratch(["analytics", "--file", "out.txt", "--json"]);
      expect(result.code).toBe(0);
    });

    it.skipIf(!RUN)("analytics --file <scratch>/out.txt --markdown exits 0", async () => {
      const result = await executeCliInScratch(["analytics", "--file", "out.txt", "--markdown"]);
      expect(result.code).toBe(0);
    });
  });
  ...
  describe("error paths", () => {
    ...
    it.skipIf(!RUN)("analytics --file out.txt (no format flag) exits 2", async () => {
      const result = await executeCliCommand(["analytics", "--file", "out.txt"]);
      expect(result.code).toBe(2);
    });

    it.skipIf(!RUN)("analytics rainbow --json (unknown scope) exits 2", async () => {
      const result = await executeCliCommand(["analytics", "rainbow", "--json"]);
      expect(result.code).toBe(2);
    });

    it.skipIf(!RUN)("analytics repo veryunknownrepo --json (unknown repo) exits 1", async () => {
      const result = await executeCliInScratch(["analytics", "repo", "veryunknownrepo", "--json"]);
      expect(result.code).toBe(1);
    });

    it.skipIf(!RUN)("analytics repo 30 --markdown (no repo name) exits 2", async () => {
      const result = await executeCliCommand(["analytics", "repo", "30", "--markdown"]);
      expect(result.code).toBe(2);
    });

    it.skipIf(!RUN)("analytics org 25 --markdown (invalid time-window) exits 2", async () => {
      const result = await executeCliCommand(["analytics", "org", "25", "--markdown"]);
      expect(result.code).toBe(2);
    });

    it.skipIf(!RUN)("analytics 123 --json (invalid time-window) exits 2", async () => {
