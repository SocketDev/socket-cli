 ...
 *
 * Key Functions: - runWithConfig: Execute Socket CLI with isolated
 * configuration.
 *
 * Features: - Automatically appends --config {} if not present - Returns
 * structured result with exitCode, stdout, and stderr - Prevents test pollution
 * from user's local Socket configuration.
 *
 * Usage: - Use for testing CLI commands in isolation - Ensures reproducible
 * test results across environments - Prevents authentication token leakage in
 * tests.
 *
 * @example
