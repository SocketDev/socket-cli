/**
 * @file Mock setup utilities for Socket CLI tests. IMPORTANT: Mock setup helper
 *   functions DO NOT WORK with Vitest. Vitest requires vi.mock() to be called
 *   at the top level of test files for proper hoisting. When vi.mock() is
 *   called from within a function, the mock declarations are not hoisted
 *   correctly, resulting in "No export defined on mock" errors. Instead of
 *   using helper functions, explicitly declare mocks at the top of each test
 *   file.
 *
 * @example
 *   Correct pattern for mocking
 *   ```typescript
