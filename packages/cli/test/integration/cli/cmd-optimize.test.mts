 ...
 * end-to-end behavior.
 *
 * Test Coverage: - Command help and version information - Dry-run mode
 * validation (no file modifications) - Flag combinations (--pin, --prod,
 * --dry-run) - Output format support (JSON, markdown, text) - Custom directory
 * path handling - Error handling (missing package.json, invalid paths, missing
 * API tokens) - Edge cases (conflicting flags, unknown flags, comprehensive
 * flag combinations)
 *
 ...
 * logic - test/unit/commands/optimize/agent-installer.test.mts - Unit tests for
 * non-npm agents.
 */
...
async function revertFixtureChanges() {
  ...
  const relativePackageJson = path.relative(cwd, path.join(pnpmFixtureDir, PACKAGE_JSON));
  const relativePnpmLock = path.relative(cwd, path.join(pnpmFixtureDir, PNPM_LOCK_YAML));
  ...
  try {
    await spawn("git", ["checkout", "HEAD", "--", relativePackageJson, relativePnpmLock], {
      cwd,
      stdio: "ignore",
    });
  } catch {}
...
describe("socket optimize", async () => {
  ...
  cmdit(["optimize", FLAG_HELP, FLAG_CONFIG, "{}"], `should support ${FLAG_HELP}`, async (cmd) => {
    ...
    expect(stderr, "banner includes base command").toContain("`socket optimize`");
  });
  ...
  cmdit(
    ["optimize", FLAG_DRY_RUN, FLAG_PIN, ".", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["optimize", FLAG_DRY_RUN, FLAG_PROD, ".", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["optimize", FLAG_DRY_RUN, FLAG_PIN, FLAG_PROD, ".", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["optimize", FLAG_DRY_RUN, FLAG_JSON, ".", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["optimize", FLAG_DRY_RUN, FLAG_MARKDOWN, ".", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["optimize", FLAG_DRY_RUN, "./custom-path", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  describe("error handling and usability tests", () => {
    ...
    cmdit(
      ["optimize", ".", FLAG_DRY_RUN, "--unknown-flag", FLAG_CONFIG, '{"apiToken":"fake-token"}'],
    ...
    cmdit(
      ...
      async (cmd) => {
        ...
        expect(stdout).toContain("Optimize dependencies with @socketregistry overrides");
        expect(code).toBe(0);
      },
    );
    ...
    cmdit(
      ...
      async (cmd) => {
        ...
        const output = stdout + stderr;
        expect(output.length).toBeGreaterThan(0);
        expect(code, "should exit with non-zero code for version mismatch").toBeGreaterThan(0);
      },
    );
  });
});
