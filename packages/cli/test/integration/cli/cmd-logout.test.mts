...
describe("socket logout", async () => {
  cmdit(["logout", FLAG_HELP, FLAG_CONFIG, "{}"], `should support ${FLAG_HELP}`, async (cmd) => {
    ...
    expect(stderr, "banner includes base command").toContain("`socket logout`");
  });

  cmdit(
    ["logout", "mootools", FLAG_DRY_RUN, FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
