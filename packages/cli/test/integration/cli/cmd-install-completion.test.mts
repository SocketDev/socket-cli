 ...
 *
 * Tests the bash tab completion installation for Socket CLI commands. This
 * subcommand sets up shell completion to improve CLI usability.
 *
 * Test Coverage:
 *
...
describe("socket install completion", async () => {
  cmdit(
    ...
    async (cmd) => {
      ...
      expect(stderr, "banner includes base command").toContain("`socket install completion`");
    },
  );

  cmdit(
    ["install", "completion", FLAG_DRY_RUN, FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
