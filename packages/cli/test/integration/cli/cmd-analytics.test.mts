 ...
 * validation - Scope selection (org vs repo) - Time filter validation (7, 30,
 * 90 days) - Repository name requirement when scope is "repo" - Legacy flag
 * detection and rejection (--scope, --repo, --time) - Argument parsing
 * (positional vs flag-based) - Error handling (missing token, invalid time
 * values, missing repo names) - Exit codes for various scenarios.
 *
 * Platform-Specific Behavior: - Windows Node 24+ has known stderr assertion
...
describe("socket analytics", async () => {
  cmdit(["analytics", FLAG_HELP, FLAG_CONFIG, "{}"], `should support ${FLAG_HELP}`, async (cmd) => {
    ...
    expect(stderr, "banner includes base command").toContain("`socket analytics`");
  });
  ...
  cmdit(
    ["analytics", "repo", FLAG_DRY_RUN, FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["analytics", "repo", "daname", FLAG_DRY_RUN, FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["analytics", "org", "--time", "7", FLAG_DRY_RUN, FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["analytics", "org", "7", FLAG_DRY_RUN, FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["analytics", "repo", "slowpo", "30", FLAG_DRY_RUN, FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
