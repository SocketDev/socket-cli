...
describe("socket scan create", async () => {
  ...
  cmdit(
    ...
    async (cmd) => {
      ...
      expect(stderr, "banner includes base command").toContain("`socket scan create`");
    },
  );
  ...
  cmdit(
    ...
    async (cmd) => {
      ...
      expect(stdout).toMatchInlineSnapshot(`"[DryRun]: Bailing now"`);
      expect(code, "should exit with code 0 when --exclude-paths is used standalone").toBe(0);
    },
  );
  ...
  cmdit(
    ...
    async (cmd) => {
      ...
      expect(output).toContain("Reachability analysis flags require --reach to be enabled");
      expect(output).toContain("add --reach flag to use --reach-* options");
      expect(code, "should exit with non-zero code when validation fails").not.toBe(0);
    },
  );
  ...
  cmdit(
    ...
    async (cmd) => {
      const { code, stdout } = await spawnSocketCli(binCliPath, cmd);
      expect(stdout).toMatchInlineSnapshot(`"[DryRun]: Bailing now"`);
      expect(code, "should exit with code 0 when comma-separated values are used").toBe(0);
    },
  );
  ...
  cmdit(
    ...
    async (cmd) => {
      ...
      expect(code, "should exit with non-zero code when invalid ecosystem is provided").not.toBe(0);
    },
  );

  cmdit(["scann", "create", FLAG_HELP], "should suggest similar command for typos", async (cmd) => {
    const { code, stderr, stdout } = await spawnSocketCli(binCliPath, cmd);
    const output = stdout + stderr;
    expect(output).toContain('Unknown command "scann". Did you mean "scan"?');
    expect(code, "should exit with non-zero code when command is not found").toBe(2);
  });
  ...
  cmdit(
    ...
    async (cmd) => {
      ...
      expect(code, "should exit with non-zero code when no files found").not.toBe(0);
    },
  );
  ...
  cmdit(
    ...
    async (cmd) => {
      ...
      expect(code, "should exit with non-zero code when invalid ecosystem provided").not.toBe(0);
    },
  );
  ...
  cmdit(
    ...
    async (cmd) => {
      ...
      expect(code, "should exit with non-zero code when conflicting flags are used").not.toBe(0);
    },
  );
