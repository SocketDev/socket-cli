...
import { FLAG_CONFIG, FLAG_HELP, FLAG_JSON } from "../../../src/constants/cli.mts";
...
describe("socket whoami", () => {
  ...
  describe("not authenticated", () => {
    cmdit(["whoami", FLAG_CONFIG, "{}"], "should show not authenticated", async (cmd) => {
      const { code, stderr, stdout } = await spawnSocketCli(binCliPath, cmd, {
        env: {
          ...process.env,
  ...
  describe("token display", () => {
    cmdit(
      ["whoami", FLAG_CONFIG, '{"apiToken":"sktsec_abcdefghijklmnopqrstuvwxyz"}'],
  ...
  describe("error handling", () => {
    cmdit(["whoami", "--invalid-flag"], "should ignore invalid flags gracefully", async (cmd) => {
