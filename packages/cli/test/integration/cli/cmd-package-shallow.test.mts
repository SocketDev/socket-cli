...
describe("socket package shallow", async () => {
  cmdit(
    ["package", "shallow", FLAG_HELP, FLAG_CONFIG, "{}"],
    ...
    async (cmd) => {
      ...
      expect(stderr, "banner includes base command").toContain("`socket package shallow`");
    },
  );

  cmdit(
    ["package", "shallow", FLAG_DRY_RUN, FLAG_CONFIG, "{}"],
  ...
  cmdit(
    ["package", "shallow", "npm", "babel", FLAG_DRY_RUN, FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
    "should require args with just dry-run",
    async (cmd) => {
      const { code, stderr, stdout } = await spawnSocketCli(binCliPath, cmd);
