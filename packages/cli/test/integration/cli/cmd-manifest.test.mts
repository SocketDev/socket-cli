...
describe("socket manifest", async () => {
  cmdit(["manifest", FLAG_HELP, FLAG_CONFIG, "{}"], `should support ${FLAG_HELP}`, async (cmd) => {
    ...
    expect(stderr, "banner includes base command").toContain("`socket manifest`");
  });

  cmdit(
    ["manifest", "mootools", FLAG_DRY_RUN, FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
