 ...
 *
 * Tests reachability analysis in dry-run mode. This is one of three test files
 * for reach command (dry-run, validation, execution).
 *
...
describe("socket scan reach - dry-run tests", async () => {
  cmdit(
    ...
    async (cmd) => {
      ...
      expect(stderr, "banner includes base command").toContain("`socket scan reach`");
    },
  );

  cmdit(
    ["scan", "reach", FLAG_DRY_RUN, "--org", "fakeOrg", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
