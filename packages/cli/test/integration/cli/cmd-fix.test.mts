...
describe("socket fix", async () => {
  ...
  cmdit(["fix", FLAG_HELP, FLAG_CONFIG, "{}"], `should support ${FLAG_HELP}`, async (cmd) => {
    const { code, stderr, stdout } = await spawnSocketCli(binCliPath, cmd);
  ...
  cmdit(
    ["fix", FLAG_DRY_RUN, "--autopilot", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["fix", FLAG_DRY_RUN, "--auto-merge", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["fix", FLAG_DRY_RUN, "--test-script", "custom-test", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["fix", FLAG_DRY_RUN, "--limit", "5", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["fix", FLAG_DRY_RUN, "--min-satisfying", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["fix", "--range-style", "invalid-style", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["fix", FLAG_DRY_RUN, "--range-style", "pin", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["fix", FLAG_DRY_RUN, "--no-major-updates", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["fix", path.join(fixtureBaseDir, "nonexistent"), FLAG_CONFIG, '{"apiToken":"fake-token"}'],
    "should show helpful error when no package.json found",
    async (cmd) => {
      const { code, stderr, stdout } = await spawnSocketCli(binCliPath, cmd);
      const output = stdout + stderr;
      expect(output).toContain("Unable to resolve a Socket account organization");
      expect(code, "should exit with non-zero code").not.toBe(0);
    },
  );
  ...
  cmdit(
    ["fix", FLAG_DRY_RUN, "--autopilot", "--limit", "1", FLAG_CONFIG, '{"apiToken":"fakeToken"}'],
  ...
  cmdit(
    ["fix", FLAG_ID, "GHSA-35jh-r3h4-6jhm", FLAG_DRY_RUN, FLAG_CONFIG, '{"apiToken":"fake-token"}'],
  ...
  describe("output format handling", () => {
    cmdit(
      ["fix", "--range-style", "pin", FLAG_MARKDOWN, ".", FLAG_CONFIG, '{"apiToken":"fake-token"}'],
  ...
  describe("error handling and usability tests", () => {
    cmdit(
      ["fix", "/nonexistent/directory", FLAG_CONFIG, '{"apiToken":"fake-token"}'],
    ...
    cmdit(
      ["fix", FLAG_ID, "invalid-id-format", FLAG_CONFIG, '{"apiToken":"fake-token"}'],
    ...
    cmdit(
      ["fix", "--limit", "not-a-number", FLAG_CONFIG, '{"apiToken":"fake-token"}'],
    ...
    cmdit(
      ["fix", FLAG_ID, "GHSA-xxxx-xxxx-xxxx", ".", FLAG_CONFIG, '{"apiToken":"fake-token"}'],
    ...
    cmdit(
      ["fix", FLAG_JSON, FLAG_MARKDOWN, ".", FLAG_CONFIG, '{"apiToken":"fake-token"}'],
    ...
    cmdit(
      ["fix", FLAG_ID, "CVE-1234-invalid", ".", FLAG_CONFIG, '{"apiToken":"fake-token"}'],
