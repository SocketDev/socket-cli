...
const ENV_KEYS = ["GH_TOKEN", "GITHUB_TOKEN", "SOCKET_CLI_GITHUB_TOKEN"] as const;
...
describe("env/socket-cli-github-token", () => {
  it("uses SOCKET_CLI_GITHUB_TOKEN when set", async () => {
    ...
    const mod = await import("../../../src/env/socket-cli-github-token.mts?cache_bust=1");
  ...
  it("falls back to GITHUB_TOKEN when SOCKET_CLI_GITHUB_TOKEN is unset", async () => {
    ...
    const mod = await import("../../../src/env/socket-cli-github-token.mts?cache_bust=2");
  ...
  it("falls back to GH_TOKEN when GITHUB_TOKEN is also unset", async () => {
    ...
    const mod = await import("../../../src/env/socket-cli-github-token.mts?cache_bust=3");
    expect(mod.SOCKET_CLI_GITHUB_TOKEN).toBe("gh-test-fake-token");
  });

  it("returns empty string when no env var is set", async () => {
    const mod = await import("../../../src/env/socket-cli-github-token.mts?cache_bust=4");
