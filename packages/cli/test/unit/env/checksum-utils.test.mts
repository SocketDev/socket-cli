...
import {
  parseChecksums,
  requireChecksum,
} from '../../../src/env/checksum-utils.mts'
...
describe('parseChecksums', () => {
  ...
  it('throws an error referencing the tool name on malformed JSON', () => {
    expect(() => parseChecksums('{not json', 'Tool')).toThrow(
      /Tool.*not valid JSON/,
    )
  })
...
describe('requireChecksum', () => {
  ...
  it('returns the matching checksum when present', () => {
    expect(
      requireChecksum({ 'asset.tar.gz': 'sha-x' }, 'asset.tar.gz', 'Tool'),
    ).toBe('sha-x')
  })

  it('throws when checksums non-empty but asset missing', () => {
    expect(() =>
      requireChecksum({ 'a.tar.gz': 'sha-a' }, 'b.tar.gz', 'Tool'),
    ).toThrow(/Tool has no SHA-256 checksum.*b\.tar\.gz/)
  })

  it('lists known assets in the error', () => {
    expect(() =>
      requireChecksum(
        { 'a.tar.gz': 'sha-a', 'b.tar.gz': 'sha-b' },
        'c.tar.gz',
        'Tool',
      ),
