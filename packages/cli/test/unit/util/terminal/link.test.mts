 ...
 *
 * Purpose: Tests terminal hyperlink rendering. Validates OSC 8 hyperlink
 * support.
 *
 * Test Coverage: - Hyperlink rendering - OSC 8 support detection - Fallback to
 * plain URLs - Link text formatting - Terminal capability detection.
 *
 * Testing Approach: Tests hyperlink utilities for modern terminals.
 *
 * Related Files: - util/terminal/link.mts (implementation)
 */
...
import {
  ...
  socketPackageLink,
  webLink,
} from "../../../../src/util/terminal/link.mts";
...
describe("terminal-link utilities", () => {
  ...
  describe("githubRepoLink", () => {
    it("creates basic GitHub repo link", () => {
      ...
      expect(result).toBe("[SocketDev/socket-cli](https://github.com/SocketDev/socket-cli)");
    });

    it("creates GitHub repo link with path", () => {
      const result = githubRepoLink("SocketDev", "socket-cli", "blob/main/README.md");
      expect(result).toBe(
    ...
    it("creates GitHub repo link with path and custom text", () => {
      const result = githubRepoLink("SocketDev", "socket-cli", "releases", "View Releases");
      expect(result).toBe("[View Releases](https://github.com/SocketDev/socket-cli/releases)");
    });
  });
