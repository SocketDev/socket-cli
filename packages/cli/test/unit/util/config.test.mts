...
describe("util/config", () => {
  describe("updateConfigValue", () => {
    ...
    it("should return object for applying a change", () => {
      expect(updateConfigValue("defaultOrg", "fake_test_org")).toMatchInlineSnapshot(`
        {
    ...
    it("handles skipAskToPersistDefaultOrg=true correctly", () => {
      const result = updateConfigValue("skipAskToPersistDefaultOrg", "true" as unknown);
      expect(result.ok).toBe(true);
    });

    it("handles skipAskToPersistDefaultOrg=false correctly", () => {
      const result = updateConfigValue("skipAskToPersistDefaultOrg", "false" as unknown);
      expect(result.ok).toBe(true);
    });

    it("deletes skipAskToPersistDefaultOrg on non-bool value", () => {
      const result = updateConfigValue("skipAskToPersistDefaultOrg", "something" as unknown);
      expect(result.ok).toBe(true);
    });
  });
  ...
  describe("non-destructive config saving", () => {
    ...
    it("deletes skipAskToPersistDefaultOrg when value is unrecognized", () => {
      ...
      const result = updateConfigValue("skipAskToPersistDefaultOrg", "maybe" as unknown);
      expect(result.ok).toBe(true);
    });
  });
