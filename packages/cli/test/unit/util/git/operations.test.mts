...
describe("git utilities", () => {
  ...
  describe("gitBranch", () => {
    ...
    it("handles detached HEAD state", async () => {
      const { spawn } = vi.mocked(await import("@socketsecurity/lib-stable/process/spawn/child"));
      spawn.mockRejectedValueOnce(new Error("Not on a branch")).mockResolvedValueOnce({
        status: 0,
        stdout: "abc1234\n",
        stderr: "",
      } as unknown);
  ...
  describe("gitCommit", () => {
    it("creates a commit with message and files", async () => {
      ...
      const result = await gitCommit("Test commit", ["file1.txt", "file2.txt"], {
        cwd: "/test/dir",
      });
  ...
  describe("gitEnsureIdentity", () => {
    ...
    it("logs failure when config set rejects (lines 447-450)", async () => {
      ...
      await expect(gitEnsureIdentity("Test User", "test@example.com")).resolves.toBeUndefined();
    });
  });
  ...
  describe("gitResetAndClean", () => {
    it("calls gitResetHard and gitCleanFdx", async () => {
      ...
      expect(spawn).toHaveBeenCalledWith("git", ["reset", "--hard", "main"], expect.any(Object));
      expect(spawn).toHaveBeenCalledWith("git", ["clean", "-fdx"], expect.any(Object));
    });
  });
