...
describe("handleGitHubApiError", () => {
  describe("rate limit errors", () => {
    ...
    it("handles 403 with rate limit message", () => {
      const error = createRequestError(403, "API rate limit exceeded for user ID 12345");
  ...
  describe("abuse detection", () => {
    it("handles abuse detection rate limit", () => {
      const error = createRequestError(403, "You have exceeded a secondary rate limit");
    ...
    it("differentiates abuse detection from standard rate limit", () => {
      // Standard rate limit.
      const standardError = createRequestError(403, "API rate limit exceeded for user");
      ...
      const abuseError = createRequestError(403, "You have exceeded a secondary rate limit");
  ...
  describe("permission errors", () => {
    it("handles 403 permission denied (not rate limit)", () => {
      const error = createRequestError(403, "Resource not accessible by integration");
...
describe("isGraphqlRateLimitError", () => {
  ...
  it("returns true for rate limit message", () => {
    const error = createGraphqlError([{ message: "API rate limit exceeded for user" }]);
    expect(isGraphqlRateLimitError(error)).toBe(true);
  });

  it("returns false for other GraphQL errors", () => {
    const error = createGraphqlError([{ type: "NOT_FOUND", message: "Resource not found" }]);
...
describe("handleGraphqlError", () => {
  ...
  it("handles generic GraphQL error", () => {
    const error = createGraphqlError([{ type: "FORBIDDEN", message: "Must have admin rights" }]);
  ...
  it("handles multiple GraphQL errors", () => {
    const error = createGraphqlError([{ message: "First error" }, { message: "Second error" }]);
...
describe("withGitHubRetry", () => {
  ...
  it("does not retry on 4xx errors (except rate limits)", async () => {
    const operation = vi.fn().mockRejectedValue(createRequestError(404, "Not Found"));
  ...
  it("returns rate limit error immediately without retrying", async () => {
    const operation = vi.fn().mockRejectedValue(createRequestError(429, "Rate limit exceeded"));
  ...
  it("returns error after exhausting retries", async () => {
    const operation = vi.fn().mockRejectedValue(createRequestError(500, "Persistent server error"));
  ...
  it("respects custom max retries", async () => {
    const operation = vi.fn().mockRejectedValue(createRequestError(500, "Server Error"));
...
describe("enablePrAutoMerge", () => {
  it("returns enabled true when GraphQL mutation succeeds", async () => {
    const { enablePrAutoMerge } = await import("../../../../src/util/git/github.mts");
...
describe("fetchGhsaDetails", () => {
  it("returns empty map for empty input array", async () => {
    const { fetchGhsaDetails } = await import("../../../../src/util/git/github.mts");
...
describe("setGitRemoteGithubRepoUrl", () => {
  it("returns false when GITHUB_SERVER_URL is invalid", async () => {
    ...
    const { setGitRemoteGithubRepoUrl } = await import("../../../../src/util/git/github.mts");
