 ...
 *
 * Purpose: Tests git provider detection (GitHub, GitLab, Bitbucket). Validates
 * provider identification from remote URLs.
 *
 * Test Coverage: - GitHub detection - GitLab detection - Bitbucket detection -
 * Remote URL parsing - SSH vs HTTPS URLs - Provider-specific features.
 *
 * Testing Approach: Tests git provider identification and URL parsing logic.
 *
 * Related Files: - util/git/providers.mts (implementation)
 */
...
describe("provider-factory", () => {
  ...
  describe("createPrProvider", () => {
    it("returns GitLabProvider when GITLAB_HOST is set", async () => {
      const providerFactory = await import("../../../../src/util/git/provider-factory.mts");
...
describe("GitHubProvider", () => {
  ...
  describe("updatePr", () => {
    ...
    it("throws when PR details fetch fails", async () => {
      mockWithGitHubRetry.mockResolvedValueOnce({ ok: true, data: {} }).mockResolvedValueOnce({
        ok: false,
