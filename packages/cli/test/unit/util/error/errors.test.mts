...
describe("Error Classes", () => {
  ...
  describe("calculateStringSimilarity", () => {
    it("returns 1 for identical strings (early-return)", async () => {
      const { calculateStringSimilarity } = await import("../../../../src/util/error/errors.mts");
    ...
    it("returns a fractional value for partial overlap", async () => {
      const { calculateStringSimilarity } = await import("../../../../src/util/error/errors.mts");
      const score = calculateStringSimilarity("fetch request failed", "fetch request succeeded");
  ...
  describe("captureException / captureExceptionSync", () => {
    it('captureExceptionSync returns "" when Sentry is not configured', async () => {
      const { captureExceptionSync } = await import("../../../../src/util/error/errors.mts");
      const result = captureExceptionSync(new Error("boom"));
      expect(result).toBe("");
    });

    it('captureException returns "" when Sentry is not configured', async () => {
      const { captureException } = await import("../../../../src/util/error/errors.mts");
...
describe("getErrorMessageOr", () => {
  ...
  it("should use different fallback messages", () => {
    expect(getErrorMessageOr(undefined, "Custom fallback 1")).toBe("Custom fallback 1");
    expect(getErrorMessageOr(undefined, "Custom fallback 2")).toBe("Custom fallback 2");
  });
});
...
describe("formatErrorWithDetail", () => {
  ...
  it("should return base message when error has no message", () => {
    const error = new Error("");
    expect(formatErrorWithDetail("Operation failed", error)).toBe("Operation failed");
  });
...
describe("buildErrorCause", () => {
  ...
  it("should include reason when messages are sufficiently different", async () => {
    const result = await buildErrorCause(400, "Request failed", "Missing required field: name");
  ...
  it("should handle 429 with message but no reason", async () => {
    const result = await buildErrorCause(429, "Too many requests", "No error message returned");
