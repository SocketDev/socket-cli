...
describe("defineHandoffCommand", () => {
  ...
  describe("spawn mode dispatch", () => {
    ...
    it('uses spawnSfw when spawnMode is "auto"', async () => {
      ...
      const runPromise = cmd.run(["install", "cowsay"], { url: import.meta.url } as ImportMeta, {
        parentName: "socket",
      });
      setImmediate(() => child.emit("exit", 0, undefined));
      const mockExit = vi.spyOn(process, "exit").mockImplementation((() => {}) as unknown);
      try {
        await runPromise;
        expect(mockSpawnSfw).toHaveBeenCalledWith(["npm", "install", "cowsay"], {
          stdio: "inherit",
        });
  ...
  describe("binaryPicker", () => {
    it("uses binaryPicker output as the first arg to sfw", async () => {
      ...
      const runPromise = cmd.run(["install", "flask"], { url: import.meta.url } as ImportMeta, {
        parentName: "socket",
        invokedAs: "pip3",
      });
      setImmediate(() => child.emit("exit", 0, undefined));
      ...
      try {
        await runPromise;
        expect(mockSpawnSfwDlx).toHaveBeenCalledWith(["pip3", "install", "flask"], {
          stdio: "inherit",
        });
      } finally {
        mockExit.mockRestore();
