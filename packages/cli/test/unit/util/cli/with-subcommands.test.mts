...
describe("meow-with-subcommands", () => {
  ...
  describe("meowOrExit", () => {
    ...
    it("exits with error when --version flag is set but config does not declare version", async () => {
      ...
      const exitSpy = vi.spyOn(process, "exit").mockImplementation((() => {}) as unknown);
      try {
        expect(() =>
          meowOrExit(
            {
        ...
        expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("Unknown flag"));
      } finally {
        exitSpy.mockRestore();
      }
    });
  ...
  describe("findBestCommandMatch", () => {
    ...
    it("returns undefined when nothing close matches", () => {
      const result = findBestCommandMatch("completelyunrelated", subcommands, aliases);
  ...
  describe("getTokenOrigin", () => {
    ...
    it('returns "(config)" when token from persisted config (line 56)', () => {
      withTokenEnvCleared(() => {
        mockGetConfigValueOrUndef.mockReturnValueOnce("sktsec_persisted_xxxxxxxxxxxx");
  ...
  describe("meowWithSubcommands", () => {
    ...
    it("applies SOCKET_API_TOKEN override (line 362)", async () => {
      ...
      try {
        ...
        expect(mockOverrideConfigApiToken).toHaveBeenCalledWith("sktsec_test_xxxxxxxxxxxx");
      } finally {
