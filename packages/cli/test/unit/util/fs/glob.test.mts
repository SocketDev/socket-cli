...
const mockSupportedFiles: SocketSdkSuccessResult<'getReportSupportedFiles'>['data'] =
  {
    ...
    python: {
      ...
      // The API returns this pattern lowercase; real projects ship `Pipfile`.
...
describe('util/fs/glob', () => {
  ...
  describe('createSupportedFilesFilter', () => {
    ...
    it('filter matches a manifest whose real name differs in case', () => {
      const filter = createSupportedFilesFilter(mockSupportedFiles)

      expect(filter('/project/Pipfile')).toBe(true)
    })
  ...
  describe('isReportSupportedFile', () => {
    ...
    it('matches a manifest whose real name differs in case', () => {
      expect(
        isReportSupportedFile('/project/Pipfile', mockSupportedFiles),
      ).toBe(true)
    })
