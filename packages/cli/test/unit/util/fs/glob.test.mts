...
describe("util/fs/glob", () => {
  ...
  describe("isReportSupportedFile", () => {
    it("returns true for supported files", () => {
      expect(isReportSupportedFile("/project/package.json", mockSupportedFiles)).toBe(true);
      expect(isReportSupportedFile("/project/requirements.txt", mockSupportedFiles)).toBe(true);
    });

    it("returns false for unsupported files", () => {
      expect(isReportSupportedFile("/project/index.js", mockSupportedFiles)).toBe(false);
      expect(isReportSupportedFile("/project/README.md", mockSupportedFiles)).toBe(false);
    });

    it("works with nested paths", () => {
      expect(isReportSupportedFile("/project/subdir/nested/package.json", mockSupportedFiles)).toBe(
        true,
      );
    });
  });
  ...
  describe("ignorePatternToMinimatch", () => {
    it("returns special-cased patterns verbatim with negation prefix preserved", async () => {
      const { ignorePatternToMinimatch } = await import("../../../../src/util/fs/glob.mts");
  ...
  describe("workspacePatternToGlobPattern", () => {
    it("returns empty for empty input", async () => {
      const { workspacePatternToGlobPattern } = await import("../../../../src/util/fs/glob.mts");
      expect(workspacePatternToGlobPattern("")).toBe("");
    });

    it("appends /*/package.json for trailing-slash workspaces", async () => {
      const { workspacePatternToGlobPattern } = await import("../../../../src/util/fs/glob.mts");
      expect(workspacePatternToGlobPattern("packages/")).toBe("packages//*/package.json");
    });
    ...
    it("appends /package.json for plain workspaces", async () => {
      const { workspacePatternToGlobPattern } = await import("../../../../src/util/fs/glob.mts");
      expect(workspacePatternToGlobPattern("packages/cli")).toBe("packages/cli/package.json");
    });
  });

  describe("ignoreFileLinesToGlobPatterns", () => {
    it("skips blank and comment lines", async () => {
      const { ignoreFileLinesToGlobPatterns } = await import("../../../../src/util/fs/glob.mts");
    ...
    it("preserves negation patterns with relative path joined", async () => {
      const { ignoreFileLinesToGlobPatterns } = await import("../../../../src/util/fs/glob.mts");
      const result = ignoreFileLinesToGlobPatterns(["!keep"], "/repo/sub/.gitignore", "/repo");
  ...
  describe("ignoreFileToGlobPatterns", () => {
    it("splits on \\r?\\n and threads through ignoreFileLinesToGlobPatterns", async () => {
      const { ignoreFileToGlobPatterns } = await import("../../../../src/util/fs/glob.mts");
  ...
  describe("getWorkspaceGlobs", () => {
    it("reads pnpm-workspace.yaml packages list for PNPM agent (lines 49-56)", async () => {
      ...
      safeReadFile.mockResolvedValueOnce('packages:\n  - "packages/*"\n  - "apps/*"\n');
      const { getWorkspaceGlobs } = await import("../../../../src/util/fs/glob.mts");
