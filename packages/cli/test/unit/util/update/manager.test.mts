...
import { checkForUpdates, scheduleUpdateCheck } from "../../../../src/util/update/manager.mts";
  ...
  describe("checkForUpdates", () => {
    describe("parameter validation", () => {
      it("returns false for empty package name", async () => {
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith(
          expect.stringContaining("checkForUpdates options.name requires a non-empty string"),
        );
      });
      ...
      it("returns false for empty version", async () => {
        ...
        expect(result).toBe(false);
        expect(mockLogger.warn).toHaveBeenCalledWith(
          expect.stringContaining("checkForUpdates options.version requires a non-empty string"),
        );
      });
      ...
      it("returns false for negative TTL", async () => {
        ...
        expect(result).toBe(false);
        expect(mockLogger.warn).toHaveBeenCalledWith(
          expect.stringContaining("checkForUpdates options.ttl must be >= 0 (saw: -1)"),
        );
      });
      ...
      it("warns about invalid auth info but continues", async () => {
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("Invalid auth info"));
      });
    ...
    describe("error handling", () => {
      ...
      it("returns false when fetch fails and no cache", async () => {
        ...
        expect(result).toBe(false);
        expect(mockLogger.log).toHaveBeenCalledWith("No version information available");
      });
