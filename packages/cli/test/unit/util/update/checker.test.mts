...
describe("update/checker", () => {
  ...
  describe("NetworkUtils.fetch", () => {
    ...
    it("rejects on HTTP error status", async () => {
      ...
      await expect(NetworkUtils.fetch("https://registry.npmjs.org/nonexistent")).rejects.toThrow(
        "HTTP 404",
      );
    });
    ...
    it("rejects on invalid JSON response", async () => {
      ...
      await expect(NetworkUtils.fetch("https://registry.npmjs.org/test")).rejects.toThrow(
    ...
    it("rejects on timeout", async () => {
      ...
      await expect(NetworkUtils.fetch("https://registry.npmjs.org/test", {}, 1000)).rejects.toThrow(
  ...
  describe("NetworkUtils.getLatestVersion", () => {
    ...
    it("throws when registryUrl is explicit empty string (line 222-226)", async () => {
      await expect(NetworkUtils.getLatestVersion("test", { registryUrl: "" })).rejects.toThrow(
    ...
    it("throws error when version is missing from response", async () => {
      ...
      await expect(NetworkUtils.getLatestVersion("test-package")).rejects.toThrow(
        /responded without a \.version string/,
      );
    });
  });
  ...
  describe("checkForUpdates", () => {
    it("throws error for empty package name", async () => {
      await expect(checkForUpdates({ name: "", version: "1.0.0" })).rejects.toThrow(
        /checkForUpdates options\.name requires a non-empty string/,
      );
    });

    it("throws error for empty version", async () => {
      await expect(checkForUpdates({ name: "test", version: "" })).rejects.toThrow(
    ...
    it("throws error when registry fetch fails", async () => {
      ...
      await expect(checkForUpdates({ name: "test-package", version: "1.0.0" })).rejects.toThrow();
    });
