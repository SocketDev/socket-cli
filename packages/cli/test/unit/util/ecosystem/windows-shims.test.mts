...
vi.mock(import("@socketsecurity/lib-stable/constants/platform"), () => mockWin32);
...
describe("windows-shims", () => {
  describe("resolveBinPathSync", () => {
    ...
    it("returns the input path when the file content has no shim pattern", () => {
      ...
      expect(resolveBinPathSync("/usr/bin/some-tool")).toBe("/usr/bin/some-tool");
    });

    it("extracts the absolute npm-cli.js path from a node-style shim", () => {
      mockExistsSync.mockReturnValue(true);
      mockReadFileSync.mockReturnValue('node "/usr/lib/node_modules/npm/bin/npm-cli.js" "$@"\n');
    ...
    it("extracts the pnpm shim path", () => {
      ...
      expect(resolveBinPathSync("/usr/local/bin/pnpm")).toBe("/opt/pnpm/dist/pnpm.cjs");
    });

    it("extracts the yarn shim path (.mjs extension)", () => {
      mockExistsSync.mockReturnValue(true);
      mockReadFileSync.mockReturnValue('node "/opt/yarn/lib/yarn.mjs" "$@"\n');
      expect(resolveBinPathSync("/usr/local/bin/yarn")).toBe("/opt/yarn/lib/yarn.mjs");
    });
    ...
    it("returns the input path when readFileSync throws", () => {
      ...
      expect(resolveBinPathSync("/usr/local/bin/npm")).toBe("/usr/local/bin/npm");
    });
  });
  ...
  describe("preferWindowsCmdShim (POSIX)", () => {
    it("returns the input path verbatim on POSIX (WIN32 = false)", () => {
      ...
      expect(preferWindowsCmdShim("/usr/local/bin/npm", "npm")).toBe("/usr/local/bin/npm");
    });
  });
  ...
  describe("preferWindowsCmdShim (Windows)", () => {
    ...
    it("returns the input when basename does not match binName (line 79-80)", () => {
      mockWin32.WIN32 = true;
      try {
        expect(preferWindowsCmdShim("/usr/local/bin/wrong", "npm")).toBe("/usr/local/bin/wrong");
      } finally {
        mockWin32.WIN32 = false;
      }
    });
