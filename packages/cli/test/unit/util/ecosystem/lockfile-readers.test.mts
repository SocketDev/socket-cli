...
import { LOCKS, readLockFileByAgent } from "../../../../src/util/ecosystem/lockfile-readers.mts";
...
describe("lockfile-readers", () => {
  ...
  describe("readLockFileByAgent (npm/pnpm/etc.)", () => {
    it("returns the utf8 content for npm", async () => {
      ...
      const result = await reader("/repo/package-lock.json", "/usr/bin/npm", "/repo");
    ...
    it("returns undefined when the underlying read throws (catch branch)", async () => {
      ...
      const result = await reader("/repo/pnpm-lock.yaml", "/usr/bin/pnpm", "/repo");
