...
vi.mock(import("../../../../src/constants/agents.mts"), async (importOriginal) => {
...
describe("package-environment", () => {
  ...
  describe("detectPackageEnvironment", () => {
    it("detects npm environment with package-lock.json", async () => {
      const { findUp } = await import("../../../../src/util/fs/find-up.mts");
    ...
    it("detects browser targets from browserslist", async () => {
      ...
      mockFindUp.mockImplementation(async (files) => {
        if (Array.isArray(files) && files.some((f) => f.includes("package-lock.json"))) {
          return "/project/package-lock.json";
        }
        return undefined;
      });
      ...
      expect(result.browsers || mockBrowserslist()).toEqual(["chrome 90", "firefox 88"]);
    });
  });
  ...
  describe("detectAndValidatePackageEnvironment", () => {
    ...
    it("lowers pkgMinNodeVersion from browserslist node targets (lines 387-399)", async () => {
      ...
      const mockBrowserslist = (await import("browserslist")).default as unknown;
      mockBrowserslist.mockReturnValue(["node 16.0.0", "node 18.0.0", "chrome 120"]);
