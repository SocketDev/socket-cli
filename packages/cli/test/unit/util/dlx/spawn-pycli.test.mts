...
const mockGetBasicsToolPaths = vi.hoisted(() => vi.fn(() => ({ python: "/basics/python" })));
...
describe("ensurePythonDlx", () => {
  ...
  it("throws when MAX_RETRIES exceeded", async () => {
    await expect(ensurePythonDlx(3)).rejects.toThrow(/could not acquire the Python install lock/);
  });
  ...
  it("waits for concurrent download and returns when binary appears", async () => {
    stubFastTimers();
    try {
      mockFsWriteFile.mockRejectedValue(Object.assign(new Error("EEXIST"), { code: "EEXIST" }));
...
describe("spawnSocketPyCli", () => {
  ...
  it("runs local resolution path via spawnNode", async () => {
    ...
    expect(mockSpawnNode).toHaveBeenCalledWith(["/local/py", "scan"], expect.any(Object));
