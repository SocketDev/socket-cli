...
describe("telemetry/integration", () => {
  ...
  describe("trackCliError", () => {
    it("tracks cli_error event with error details", async () => {
      const error = new Error("Test error");
      await trackCliError(["node", "socket", "scan"], Date.now() - 500, error, 1);
    ...
    it("normalizes non-Error values", async () => {
      await trackCliError(["node", "socket", "scan"], Date.now(), "string error", 1);
  ...
  describe("sanitizeArgv (via buildContext)", () => {
    ...
    it("strips arguments after wrapper CLI commands", async () => {
      await trackCliStart(["node", "socket", "npm", "install", "@my/private-package"]);
    ...
    it("redacts hex tokens", async () => {
      await trackCliStart(["node", "socket", "scan", "abc123def456789012345678901234567890"]);
