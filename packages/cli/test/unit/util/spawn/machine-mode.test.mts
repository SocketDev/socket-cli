...
import { applyMachineMode } from "../../../../src/util/spawn/machine-mode.mts";
...
describe("applyMachineMode", () => {
  ...
  describe("vltpkg", () => {
    it("uniformly forwards --view=json to every subcommand", () => {
      expect(applyMachineMode({ args: [], subcommand: "ls", tool: "vlt" }).args).toContain(
        "--view=json",
      );
      expect(
        applyMachineMode({
      ...
      expect(applyMachineMode({ args: [], subcommand: "anything", tool: "vlt" }).args).toContain(
        "--view=json",
      );
    });
  });
  ...
  describe("go", () => {
    it("adds -json for list/test/build", () => {
      expect(applyMachineMode({ args: [], subcommand: "list", tool: "go" }).args).toContain(
        "-json",
      );
      expect(applyMachineMode({ args: [], subcommand: "test", tool: "go" }).args).toContain(
        "-json",
      );
      expect(applyMachineMode({ args: [], subcommand: "build", tool: "go" }).args).toContain(
        "-json",
      );
    });
