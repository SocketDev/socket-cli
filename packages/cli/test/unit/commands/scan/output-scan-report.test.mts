...
vi.mock(import("../../../../src/commands/scan/generate-report.mts"), async (orig) => {
...
describe("output-scan-report", () => {
  ...
  describe("outputScanReport", () => {
    ...
    it("should write JSON to file when filepath is specified", async () => {
      ...
      expect(mockWriteFile).toHaveBeenCalledWith("/tmp/report.json", expect.any(String));
    });
    ...
    it("should output markdown format", async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining("# Scan Policy Report"));
    });
