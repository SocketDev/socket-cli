...
const mockGetGithubApiUrl = vi.hoisted(() => vi.fn(() => "https://api.github.com"));
vi.mock(import("@socketsecurity/lib-stable/env/github"), async (importOriginal) => {
...
describe("setup-scan-config", () => {
  ...
  describe("configureScan", () => {
    ...
    it("sets report=true when user picks alwaysReport=yes", async () => {
      mockInput.mockResolvedValueOnce("r").mockResolvedValueOnce("w").mockResolvedValueOnce("b");
  ...
  describe("configureGithub", () => {
    ...
    it("saves orgGithub when user provides one", async () => {
      mockSelect.mockResolvedValueOnce("yes");
      mockInput.mockResolvedValueOnce("https://custom.api").mockResolvedValueOnce("my-gh-org");
      const config: unknown = {};
      const result = await configureGithub(config, "/cwd");
      expect(result.ok).toBe(true);
    ...
    it('clears all setting when user picks "(leave default)"', async () => {
      ...
      mockInput.mockResolvedValueOnce("repos").mockResolvedValueOnce("").mockResolvedValueOnce("");
