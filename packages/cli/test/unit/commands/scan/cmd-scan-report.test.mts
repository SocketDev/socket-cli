...
// Import after mocks.
const { cmdScanReport } = await import("../../../../src/commands/scan/cmd-scan-report.mts");
...
describe("cmd-scan-report", () => {
  ...
  describe("run", () => {
    ...
    it("should pass --fold flag to handleScanReport", async () => {
      await cmdScanReport.run([testScanId, "--fold", "version"], importMeta, context);
    ...
    it("should pass --report-level flag to handleScanReport", async () => {
      await cmdScanReport.run([testScanId, "--report-level", "error"], importMeta, context);
    ...
    it("should pass output file path to handleScanReport", async () => {
      await cmdScanReport.run([testScanId, "./output.json"], importMeta, context);
    ...
    it("should pass --org flag to determineOrgSlug", async () => {
      ...
      await cmdScanReport.run([testScanId, "--org", "custom-org"], importMeta, context);
    ...
    it("should fail if both --json and --markdown are provided", async () => {
      await cmdScanReport.run([testScanId, "--json", "--markdown"], importMeta, context);
    ...
    it("should pass --no-interactive to determineOrgSlug", async () => {
      await cmdScanReport.run([testScanId, "--no-interactive"], importMeta, context);
    ...
    it("should show fold and report level in dry-run", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("pkg"));
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("monitor"));
    });

    it("should show license policy in dry-run", async () => {
      await cmdScanReport.run(["--dry-run", testScanId, "--license"], importMeta, context);

      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("includeLicense"));
    });

    it("should show short flag in dry-run", async () => {
      await cmdScanReport.run(["--dry-run", testScanId, "--short"], importMeta, context);

      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("short"));
    });
  });
});
