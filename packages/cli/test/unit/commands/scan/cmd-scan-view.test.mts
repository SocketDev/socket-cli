...
vi.mock(import("../../../../src/commands/scan/handle-scan-view.mts"), () => ({
  handleScanView: mockHandleScanView,
}));

vi.mock(import("../../../../src/commands/scan/stream-scan.mts"), () => ({
...
// Import after mocks.
const { cmdScanView } = await import("../../../../src/commands/scan/cmd-scan-view.mts");
...
describe("cmd-scan-view", () => {
  ...
  describe("run", () => {
    ...
    it("should call handleScanView with scan ID", async () => {
      ...
      expect(mockHandleScanView).toHaveBeenCalledWith("test-org", testScanId, "", "text");
    });
    ...
    it("should pass --org flag to determineOrgSlug", async () => {
      ...
      await cmdScanView.run([testScanId, "--org", "custom-org"], importMeta, context);

      expect(mockDetermineOrgSlug).toHaveBeenCalledWith("custom-org", true, false);
      expect(mockHandleScanView).toHaveBeenCalledWith("custom-org", testScanId, "", "text");
    });

    it("should support --json output mode", async () => {
      await cmdScanView.run([testScanId, "--json"], importMeta, context);

      expect(mockHandleScanView).toHaveBeenCalledWith("test-org", testScanId, "", "json");
    });

    it("should support --markdown output mode", async () => {
      await cmdScanView.run([testScanId, "--markdown"], importMeta, context);

      expect(mockHandleScanView).toHaveBeenCalledWith("test-org", testScanId, "", "markdown");
    });

    it("should fail if both --json and --markdown are provided", async () => {
      await cmdScanView.run([testScanId, "--json", "--markdown"], importMeta, context);
    ...
    it("should call streamScan with --json --stream flags", async () => {
      await cmdScanView.run([testScanId, "--json", "--stream"], importMeta, context);
    ...
    it("should pass --no-interactive to determineOrgSlug", async () => {
      await cmdScanView.run([testScanId, "--no-interactive"], importMeta, context);
    ...
    it("should show stream mode in dry-run with --json --stream", async () => {
      await cmdScanView.run(["--dry-run", testScanId, "--json", "--stream"], importMeta, context);

      // Dry-run output routes to stderr when --json is set so the
      // primary payload stays pipe-safe on stdout.
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("stream"));
    });
  });
});
