...
describe("reachability-flags", () => {
  describe("reachabilityFlags", () => {
    ...
    it("has reachDisableAnalysisSplitting flag", () => {
      ...
      expect(reachabilityFlags["reachDisableAnalysisSplitting"]!.type).toBe("boolean");
      expect(reachabilityFlags["reachDisableAnalysisSplitting"]!.default).toBe(false);
    });
    ...
    it("has reachUseOnlyPregeneratedSboms flag", () => {
      ...
      expect(reachabilityFlags["reachUseOnlyPregeneratedSboms"]!.type).toBe("boolean");
      expect(reachabilityFlags["reachUseOnlyPregeneratedSboms"]!.default).toBe(false);
    });

    it("has reachUseUnreachableFromPrecomputation flag", () => {
      expect(reachabilityFlags["reachUseUnreachableFromPrecomputation"]).toBeDefined();
      expect(reachabilityFlags["reachUseUnreachableFromPrecomputation"]!.type).toBe("boolean");
      expect(reachabilityFlags["reachUseUnreachableFromPrecomputation"]!.default).toBe(false);
    });

    it("all flags have descriptions", () => {
      const flagNames = Object.keys(reachabilityFlags) as Array<keyof typeof reachabilityFlags>;
      for (let i = 0, { length } = flagNames; i < length; i += 1) {
        ...
        expect(reachabilityFlags[flagName]!.description!.length).toBeGreaterThan(0);
      }
    });
  });
});
