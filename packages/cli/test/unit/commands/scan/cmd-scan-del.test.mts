...
vi.mock(import("../../../../src/commands/scan/handle-delete-scan.mts"), () => ({
...
// Import after mocks.
const { cmdScanDel } = await import("../../../../src/commands/scan/cmd-scan-del.mts");
...
describe("cmd-scan-del", () => {
  ...
  describe("run", () => {
    ...
    it("should show delete operation in dry-run", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringMatching(/delet/i));
    });
    ...
    it("should pass correct command path context to handler", async () => {
      mockHandleDeleteScan.mockImplementationOnce(async (orgSlug, scanId, outputKind) => {
    ...
    it("should call handler when all validations pass", async () => {
      ...
      expect(mockHandleDeleteScan).toHaveBeenCalledTimes(1);
      expect(mockHandleDeleteScan).toHaveBeenCalledWith("valid-org", testScanId, "text");
    });
    ...
    it("should validate org slug even with default org", async () => {
      ...
      expect(mockHandleDeleteScan).toHaveBeenCalledWith("my-org", testScanId, "text");
    });

    it("should format dry-run output with org/scan path", async () => {
      await cmdScanDel.run(["--dry-run", testScanId], importMeta, context);

      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringMatching(/test-org/));
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining(testScanId));
    });
    ...
    it("should handle scan ID with different UUID format", async () => {
      ...
      expect(mockHandleDeleteScan).toHaveBeenCalledWith("test-org", anotherScanId, "text");
    });

    it("should handle org slug with special characters", async () => {
      mockDetermineOrgSlug.mockResolvedValueOnce(["org-with-dashes", "org-with-dashes"]);

      await cmdScanDel.run([testScanId, "--org", "org-with-dashes"], importMeta, context);

      expect(mockHandleDeleteScan).toHaveBeenCalledWith("org-with-dashes", testScanId, "text");
    });
