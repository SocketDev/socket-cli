...
vi.mock(import("../../../../src/commands/scan/handle-diff-scan.mts"), () => ({
...
// Import after mocks.
const { cmdScanDiff } = await import("../../../../src/commands/scan/cmd-scan-diff.mts");
...
describe("cmd-scan-diff", () => {
  ...
  describe("run", () => {
    ...
    it("should support --dry-run flag", async () => {
      await cmdScanDiff.run(["--dry-run", testScanId1, testScanId2], importMeta, context);
    ...
    it("should pass --depth flag to handleDiffScan", async () => {
      await cmdScanDiff.run([testScanId1, testScanId2, "--depth", "5"], importMeta, context);
    ...
    it("should pass --org flag to determineOrgSlug", async () => {
      ...
      await cmdScanDiff.run([testScanId1, testScanId2, "--org", "custom-org"], importMeta, context);
    ...
    it("should support --json output mode", async () => {
      await cmdScanDiff.run([testScanId1, testScanId2, "--json"], importMeta, context);
    ...
    it("should support --markdown output mode", async () => {
      await cmdScanDiff.run([testScanId1, testScanId2, "--markdown"], importMeta, context);
    ...
    it("should pass --no-interactive to determineOrgSlug", async () => {
      await cmdScanDiff.run([testScanId1, testScanId2, "--no-interactive"], importMeta, context);
    ...
    it("should show both scan IDs in dry-run", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining(testScanId1));
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining(testScanId2));
    });
