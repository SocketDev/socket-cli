...
vi.mock(import("../../../../src/commands/scan/create-scan-from-github.mts"), () => ({
...
describe("handleCreateGithubScan", () => {
  ...
  it("handles all repositories flag", async () => {
    ...
    expect(mockCreate).toHaveBeenCalledWith(expect.objectContaining({ all: true, repos: "" }));
  });
  ...
  it("handles interactive mode", async () => {
    ...
    expect(mockCreate).toHaveBeenCalledWith(expect.objectContaining({ interactive: true }));
  });
