...
vi.mock(import("../../../../src/commands/scan/handle-create-github-scan.mts"), () => ({
...
describe("cmd-scan-github", () => {
  ...
  describe("run", () => {
    ...
    it("should support --dry-run flag", async () => {
      ...
      await cmdScanGithub.run(["--dry-run", "--github-token", "test-token"], importMeta, context);

      expect(mockHandleCreateGithubScan).not.toHaveBeenCalled();
      // Now that outputDryRunUpload is mocked, assert on it directly.
      expect(mockOutputDryRunUpload).toHaveBeenCalledWith("GitHub scan", expect.any(Object));
    });
    ...
    it("outputs dry-run details for --all (scope: all repositories)", async () => {
      ...
      await cmdScanGithub.run(
        ["--github-token", "test-token", "--all", "--dry-run", "--no-interactive"],
        importMeta,
        context,
      );
    ...
    it("outputs dry-run details for --repos (repositories list)", async () => {
      ...
      await cmdScanGithub.run(
        ["--github-token", "test-token", "--repos", "foo,bar", "--dry-run", "--no-interactive"],
        importMeta,
        context,
      );
