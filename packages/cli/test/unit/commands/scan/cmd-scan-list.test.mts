...
// Import after mocks.
const { cmdScanList } = await import("../../../../src/commands/scan/cmd-scan-list.mts");
...
describe("cmd-scan-list", () => {
  ...
  describe("run", () => {
    ...
    it("should support --dry-run flag", async () => {
      ...
      await cmdScanList.run(["--dry-run", "--org", "test-org"], importMeta, context);
    ...
    it("should show query parameters in --dry-run mode", async () => {
      ...
      expect(mockHandleListScans).not.toHaveBeenCalled();
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("Query parameters"));
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("page: 2"));
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("perPage: 50"));
    });
    ...
    it("should pass --page flag to handleListScans", async () => {
      ...
      await cmdScanList.run(["--org", "test-org", "--page", "3"], importMeta, context);
    ...
    it("should pass --per-page flag to handleListScans", async () => {
      ...
      await cmdScanList.run(["--org", "test-org", "--per-page", "50"], importMeta, context);
    ...
    it("should pass --sort flag to handleListScans", async () => {
      ...
      await cmdScanList.run(["--org", "test-org", "--sort", "name"], importMeta, context);
    ...
    it("should pass --direction flag to handleListScans", async () => {
      ...
      await cmdScanList.run(["--org", "test-org", "--direction", "asc"], importMeta, context);
    ...
    it("should pass --branch flag to handleListScans", async () => {
      ...
      await cmdScanList.run(["--org", "test-org", "--branch", "develop"], importMeta, context);
    ...
    it("should accept repo as positional argument", async () => {
      ...
      await cmdScanList.run(["--org", "test-org", "my-repo"], importMeta, context);
    ...
    it("should accept branch as second positional argument", async () => {
      ...
      await cmdScanList.run(["--org", "test-org", "my-repo", "develop"], importMeta, context);
    ...
    it("should support --json output mode", async () => {
      ...
      await cmdScanList.run(["--org", "test-org", "--json"], importMeta, context);
    ...
    it("should support --markdown output mode", async () => {
      ...
      await cmdScanList.run(["--org", "test-org", "--markdown"], importMeta, context);
    ...
    it("should fail when both --json and --markdown are set", async () => {
      ...
      await cmdScanList.run(["--org", "test-org", "--json", "--markdown"], importMeta, context);
    ...
    it("should support --no-interactive flag", async () => {
      ...
      await cmdScanList.run(["--org", "test-org", "--no-interactive"], importMeta, context);
    ...
    it("throws InputError when --page is not a positive integer", async () => {
      ...
      await expect(
        cmdScanList.run(["--org", "test-org", "--page", "not-a-number"], importMeta, context),
    ...
    it("throws InputError when --per-page is not a positive integer", async () => {
      ...
      await expect(
        cmdScanList.run(["--org", "test-org", "--per-page", "oops"], importMeta, context),
