...
vi.mock(import("../../../../src/commands/scan/fetch-create-org-full-scan.mts"), () => ({
...
vi.mock(import("../../../../src/commands/scan/finalize-tier1-scan.mts"), () => ({
...
vi.mock(import("../../../../src/commands/scan/output-create-new-scan.mts"), () => ({
...
vi.mock(import("../../../../src/commands/manifest/detect-manifest-actions.mts"), () => ({
  detectManifestActions: mockDetectManifestActions,
}));
vi.mock(import("../../../../src/commands/manifest/generate_auto_manifest.mts"), () => ({
...
describe("handleCreateNewScan", () => {
  ...
  it("creates scan successfully with found files", async () => {
    ...
    const { getPackageFilesForScan } = await import("../../../../src/util/fs/path-resolve.mts");
    ...
    mockFetchCreateOrgFullScan.mockResolvedValue(createSuccessResult({ id: "scan-123" }));
    ...
    expect(outputCreateNewScan).toHaveBeenCalledWith(createSuccessResult({ id: "scan-123" }), {
      interactive: false,
      outputKind: "json",
    });
  });
  ...
  it("handles auto-manifest mode", async () => {
    const { readOrDefaultSocketJson } = await import("../../../../src/util/socket/json.mts");
  ...
  it("handles no eligible files found", async () => {
    ...
    // biome-ignore lint/correctness/noUnusedVariables: imported for mocking.
    const { getPackageFilesForScan } = await import("../../../../src/util/fs/path-resolve.mts");
    const { checkCommandInput } = await import("../../../../src/util/validation/check-input.mts");
  ...
  it("handles reachability analysis", async () => {
    ...
    mockFetchCreateOrgFullScan.mockResolvedValue(createSuccessResult({ id: "scan-789" }));
  ...
  it("handles scan report generation", async () => {
    ...
    mockGetPackageFilesForScan.mockResolvedValue(["/test/project/package.json"]);
    mockCheckCommandInput.mockReturnValue(true);
    mockFetchCreateOrgFullScan.mockResolvedValue(createSuccessResult({ id: "scan-report-123" }));
  ...
  it("handles fetch supported files failure", async () => {
    ...
    mockFetchSupportedScanFileNames.mockResolvedValue(createErrorResult(error.message));

    await handleCreateNewScan(mockConfig);

    expect(outputCreateNewScan).toHaveBeenCalledWith(createErrorResult(error.message), {
      interactive: false,
      outputKind: "json",
    });
  });
  ...
  it("handles reachability analysis failure", async () => {
    ...
    mockGetPackageFilesForScan.mockResolvedValue(["/test/project/package.json"]);
    mockCheckCommandInput.mockReturnValue(true);
    mockPerformReachabilityAnalysis.mockResolvedValue(createErrorResult("Reachability failed"));
    ...
    expect(mockOutputCreateNewScan).toHaveBeenCalledWith(expect.objectContaining({ ok: false }), {
      interactive: false,
      outputKind: "json",
    });
  });
  ...
  it("handles report mode with missing scan ID", async () => {
    ...
    mockFetchCreateOrgFullScan.mockResolvedValue(createSuccessResult({ name: "scan" }));
  ...
  describe("basics flag", () => {
    ...
    it("reaches the basics-findings code path when factsPath exists", async () => {
      ...
      try {
        ...
        mockGetPackageFilesForScan.mockResolvedValue([path.join(tmpDir, "package.json")]);
        mockCheckCommandInput.mockReturnValue(true);
        mockFetchCreateOrgFullScan.mockResolvedValue(createSuccessResult({ id: "scan-123" }));
