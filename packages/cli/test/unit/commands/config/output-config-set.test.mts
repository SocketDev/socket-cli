 ...
 *
 * Purpose: Tests the output formatting for config set results.
 *
 * Test Coverage: - outputConfigSet function - JSON output format - Text output
 * format - Markdown output format - Error handling.
 *
 * Related Files: - src/commands/config/output-config-set.mts (implementation)
 */
  ...
  describe("outputConfigSet", () => {
    ...
    describe("Text output", () => {
      it("outputs OK and message on success", async () => {
        ...
        expect(mockLogger.log).toHaveBeenCalledWith("Config key updated successfully");
      });
      ...
      it("outputs error with fail message", async () => {
        ...
        expect(mockLogger.fail).toHaveBeenCalledWith(expect.stringContaining("Invalid key"));
        expect(process.exitCode).toBe(1);
      });
    });
