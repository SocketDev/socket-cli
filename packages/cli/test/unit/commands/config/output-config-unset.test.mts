  ...
  describe("outputConfigUnset", () => {
    ...
    describe("Text output", () => {
      it("outputs OK and message on success", async () => {
        ...
        expect(mockLogger.log).toHaveBeenCalledWith("Config key removed successfully");
      });
      ...
      it("outputs additional data when provided", async () => {
        ...
        expect(mockLogger.log).toHaveBeenCalledWith("Previous value was: old-value");
      });
      ...
      it("outputs error with fail message", async () => {
        ...
        expect(mockLogger.fail).toHaveBeenCalledWith(expect.stringContaining("Cannot unset key"));
        expect(process.exitCode).toBe(1);
      });
    });
