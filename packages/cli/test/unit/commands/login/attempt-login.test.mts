...
const mockFailMsgWithBadge = vi.hoisted(() => vi.fn((msg, cause) => `[fail] ${msg}: ${cause}`));
...
describe("attemptLogin", () => {
  ...
  it("happy path with no enterprise orgs persists default org and applies login", async () => {
    ...
    expect(mockUpdateConfigValue).toHaveBeenCalledWith(expect.any(String), "my-org");
    expect(mockApplyLogin).toHaveBeenCalled();
    expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining("API credentials"));
  });
  ...
  it("prompts to enforce when there are multiple enterprise orgs", async () => {
    ...
    expect(mockApplyLogin).toHaveBeenCalledWith(expect.any(String), ["a"], undefined, undefined);
  });
  ...
  it("runs tab completion installer when user accepts", async () => {
    ...
    expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining("Tab completion"));
  });
  ...
  it("warns when config is in read-only mode (flag override)", async () => {
    ...
    expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("read-only"));
  });
  ...
  it("reports refreshed when token matches the previously persisted one", async () => {
    ...
    expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining("refreshed"));
  });
  ...
  it("reports updated when a different token was previously persisted", async () => {
    ...
    expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining("updated"));
  });
  ...
  it("handles enterprise org with no name (fallback to undefined label)", async () => {
    ...
    mockGetEnterpriseOrgs.mockReturnValueOnce([{ id: "a" }, { id: "b", name: "Beta" }]);
    mockSelect.mockResolvedValueOnce("").mockResolvedValueOnce(false);

    await attemptLogin(undefined, undefined);

    // Empty enforced selection means no enforced orgs.
    expect(mockApplyLogin).toHaveBeenCalledWith(expect.any(String), [], undefined, undefined);
  });
