...
vi.mock(import("../../../../src/commands/login/attempt-login.mts"), () => ({
...
// Import after mocks.
const { cmdLogin, CMD_NAME } = await import("../../../../src/commands/login/cmd-login.mts");
...
describe("cmd-login", () => {
  ...
  describe("command metadata", () => {
    ...
    it("should have correct description", () => {
      expect(cmdLogin.description).toBe("Setup Socket CLI with an API token and defaults");
    });
  ...
  describe("run", () => {
    ...
    describe("help flag", () => {
      it("should display help text with --help flag", async () => {
        await expect(cmdLogin.run(["--help"], importMeta, context)).rejects.toThrow();
    ...
    describe("login execution", () => {
      ...
      it("should pass API base URL when provided", async () => {
        await cmdLogin.run(["--api-base-url=https://api.example.com"], importMeta, context);

        expect(mockAttemptLogin).toHaveBeenCalledWith("https://api.example.com", "");
      });

      it("should pass API proxy when provided", async () => {
        await cmdLogin.run(["--api-proxy=http://localhost:8080"], importMeta, context);

        expect(mockAttemptLogin).toHaveBeenCalledWith("", "http://localhost:8080");
      });

      it("should pass both API base URL and proxy when provided", async () => {
        await cmdLogin.run(
          ["--api-base-url=https://api.example.com", "--api-proxy=http://localhost:8080"],
          importMeta,
          context,
        );
