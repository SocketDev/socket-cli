  ...
  describe("run", () => {
    ...
    describe("help flag", () => {
      it("should display help text with --help flag", async () => {
        ...
        await expect(cmdNpm.run(["--help"], importMeta, context)).rejects.toThrow();
    ...
    describe("dry-run behavior", () => {
      ...
      it("should show dry-run output with npm install command", async () => {
        await cmdNpm.run(["--dry-run", "install", "lodash"], importMeta, context);
      ...
      it("should filter Socket flags in dry-run output", async () => {
        await cmdNpm.run(["--dry-run", "--config", "{}", "install", "lodash"], importMeta, context);
    ...
    describe("flag filtering", () => {
      ...
      it("should filter out --config flag when forwarding to sfw", async () => {
        ...
        await cmdNpm.run(["--config", "{}", "install", "lodash"], importMeta, context);
      ...
      it("should preserve npm flags while filtering Socket flags", async () => {
        ...
        expect(mockSpawnSfw).toHaveBeenCalledWith(["npm", "install", "--save-dev", "lodash"], {
          stdio: "inherit",
        });
      });
      ...
      it("should handle --no-banner flag", async () => {
        ...
        await cmdNpm.run(["--no-banner", "install", "lodash"], importMeta, context);
    ...
    describe("command structure", () => {
      ...
      it("should forward npm install with version specifier", async () => {
        ...
        expect(mockSpawnSfw).toHaveBeenCalledWith(["npm", "install", "lodash@4.17.21"], {
          stdio: "inherit",
        });
      });
      ...
      it("should forward npm install with global flag", async () => {
        ...
        expect(mockSpawnSfw).toHaveBeenCalledWith(["npm", "install", "-g", "cowsay"], {
          stdio: "inherit",
        });
      });
      ...
      it("should forward npm exec command", async () => {
        ...
        expect(mockSpawnSfw).toHaveBeenCalledWith(["npm", "exec", "cowsay", "hello"], {
          stdio: "inherit",
        });
      });
      ...
      it("should forward npm install with multiple packages", async () => {
        ...
        await cmdNpm.run(["install", "lodash", "express", "react"], importMeta, context);
    ...
    describe("exit handler callback", () => {
      ...
      it("should call process.exit with numeric exit code", async () => {
        ...
        expect(mockTrackSubprocessExit).toHaveBeenCalledWith(NPM, expect.any(Number), 42);
      ...
      it("should call process.kill with signal", async () => {
        ...
        expect(mockTrackSubprocessExit).toHaveBeenCalledWith(NPM, expect.any(Number), undefined);
    ...
    describe("command name constant", () => {
      it("should use NPM constant as command name", async () => {
        const { CMD_NAME } = await import("../../../../src/commands/npm/cmd-npm.mts");
