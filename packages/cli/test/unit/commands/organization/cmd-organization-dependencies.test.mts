...
vi.mock(import("../../../../src/commands/organization/handle-dependencies.mts"), () => ({
...
describe("cmd-organization-dependencies", () => {
  ...
  describe("run", () => {
    ...
    it("should pass --limit flag to handleDependencies", async () => {
      ...
      await cmdOrganizationDependencies.run(["--limit", "20"], importMeta, context);
    ...
    it("should pass --offset flag to handleDependencies", async () => {
      ...
      await cmdOrganizationDependencies.run(["--offset", "10"], importMeta, context);
    ...
    it("should fail when both --json and --markdown flags are set", async () => {
      ...
      await cmdOrganizationDependencies.run(["--json", "--markdown"], importMeta, context);
    ...
    it("should validate negative offset value", async () => {
      ...
      await expect(
        cmdOrganizationDependencies.run(["--offset", "-1"], importMeta, context),
    ...
    it("should validate non-numeric limit value", async () => {
      ...
      await expect(
        cmdOrganizationDependencies.run(["--limit", "invalid"], importMeta, context),
      ).rejects.toThrow(/--limit must be a non-negative integer \(saw: "invalid"\)/);
    ...
    it("should validate non-numeric offset value", async () => {
      ...
      await expect(
        cmdOrganizationDependencies.run(["--offset", "invalid"], importMeta, context),
      ).rejects.toThrow(/--offset must be a non-negative integer \(saw: "invalid"\)/);
    ...
    it("should show query parameters in dry-run mode", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("limit: 50"));
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("offset: 0"));
    });
    ...
    it("should show custom query parameters in dry-run mode", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("limit: 100"));
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("offset: 25"));
    });

    it("should handle limit of zero by using default", async () => {
      mockHasDefaultApiToken.mockReturnValueOnce(true);

      await cmdOrganizationDependencies.run(["--limit", "0"], importMeta, context);
    ...
    it("shows fallback limit=50 in dry-run when --limit=0", async () => {
      ...
      await cmdOrganizationDependencies.run(["--dry-run", "--limit", "0"], importMeta, context);
