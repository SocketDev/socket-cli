...
const binModule = await import("@socketsecurity/lib-stable/bin/which");
...
describe("cmd-pip", () => {
  ...
  describe("binary detection (getPipBinName)", () => {
    it("should use pip when invoked as socket pip and pip exists", async () => {
      ...
      expect(mockSpawnSfwDlx).toHaveBeenCalledWith(["pip", "install", "flask"], expect.any(Object));
    });
    ...
    it("should fallback to pip3 when pip does not exist", async () => {
      ...
      mockWhichReal.mockResolvedValueOnce(undefined).mockResolvedValueOnce("/usr/bin/pip3");
    ...
    it("should fallback to pip when pip3 does not exist but requested", async () => {
      ...
      mockWhichReal.mockResolvedValueOnce(undefined).mockResolvedValueOnce("/usr/bin/pip");
    ...
    it("should use requested binary when neither pip nor pip3 exist", async () => {
      ...
      expect(mockWhichReal).toHaveBeenCalledWith("pip", { nothrow: true });
      expect(mockWhichReal).toHaveBeenCalledWith("pip3", { nothrow: true });
      expect(mockSpawnSfwDlx).toHaveBeenCalledWith(["pip", "install", "scipy"], expect.any(Object));
    });
  });
  ...
  describe("spawn behavior", () => {
    ...
    it("should call spawnSfwDlx with correct arguments", async () => {
      ...
      expect(mockSpawnSfwDlx).toHaveBeenCalledWith(["pip", "install", "django", "--upgrade"], {
        stdio: "inherit",
      });
    });

    it("should forward all arguments to sfw", async () => {
      const argv = ["install", "-r", "requirements.txt", "--no-cache-dir", "--user"];
      const importMeta = { url: import.meta.url } as ImportMeta;
      const context: CliCommandContext = {
        parentName: "socket",
      };
  ...
  describe("common pip operations", () => {
    ...
    it("should handle pip list", async () => {
      ...
      expect(mockSpawnSfwDlx).toHaveBeenCalledWith(["pip", "list"], expect.any(Object));
    });
    ...
    it("should handle pip freeze", async () => {
      ...
      expect(mockSpawnSfwDlx).toHaveBeenCalledWith(["pip", "freeze"], expect.any(Object));
    });
