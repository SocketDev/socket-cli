...
describe("cmd-manifest-conda", () => {
  ...
  describe("run", () => {
    ...
    it("should show warning about conda support", async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("Conda"));
    });
    ...
    it("should pass --file flag to handler", async () => {
      await cmdManifestConda.run(["--file", "custom-env.yml"], importMeta, context);
    ...
    it("should pass --out flag to handler", async () => {
      await cmdManifestConda.run(["--out", "custom-requirements.txt"], importMeta, context);
