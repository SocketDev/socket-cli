...
vi.mock(import("../../../../src/commands/manifest/convert-gradle-to-maven.mts"), () => ({
  convertGradleToMaven: mockConvertGradleToMaven,
}));
vi.mock(import("../../../../src/commands/manifest/handle-manifest-conda.mts"), () => ({
...
describe("generateAutoManifest", () => {
  ...
  it("logs socket.json defaults when verbose", async () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("socket.json"), sockJson);
  });
  ...
  it("runs sbt converter when sbt is detected and not disabled", async () => {
    ...
    expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining("Scala sbt build"));
  });
  ...
  it("runs conda handler when conda is detected and not disabled", async () => {
    ...
    expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining("environment.yml"));
  });
