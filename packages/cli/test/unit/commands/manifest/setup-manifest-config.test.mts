...
const mockReadSocketJsonSync = vi.hoisted(() => vi.fn(() => ({ ok: true, data: {} })));
const mockWriteSocketJson = vi.hoisted(() => vi.fn(async () => ({ ok: true, data: undefined })));
...
describe("setup-manifest-config", () => {
  ...
  describe("askForStdout", () => {
    it('passes "yes" default for true', async () => {
      ...
      expect(mockSelect).toHaveBeenCalledWith(expect.objectContaining({ default: "yes" }));
    });

    it('passes "no" default for false', async () => {
      mockSelect.mockResolvedValueOnce("no");
      await askForStdout(false);
      expect(mockSelect).toHaveBeenCalledWith(expect.objectContaining({ default: "no" }));
    });

    it("passes empty default when undefined", async () => {
      mockSelect.mockResolvedValueOnce("");
      await askForStdout(undefined);
      expect(mockSelect).toHaveBeenCalledWith(expect.objectContaining({ default: "" }));
    });
  });
  ...
  describe("askForEnabled", () => {
    ...
    it("passes default based on enabled value", async () => {
      ...
      expect(mockSelect).toHaveBeenCalledWith(expect.objectContaining({ default: "enable" }));

      mockSelect.mockResolvedValueOnce(false);
      await askForEnabled(false);
      expect(mockSelect).toHaveBeenLastCalledWith(expect.objectContaining({ default: "disable" }));

      mockSelect.mockResolvedValueOnce(undefined);
      await askForEnabled(undefined);
      expect(mockSelect).toHaveBeenLastCalledWith(expect.objectContaining({ default: "" }));
    });
  });
  ...
  describe("askForInputFile", () => {
    ...
    it("passes empty default by default", async () => {
      ...
      expect(mockInput).toHaveBeenCalledWith(expect.objectContaining({ default: "" }));
    });
  });
  ...
  describe("askForOutputFile", () => {
    it("passes default name", async () => {
      ...
      expect(mockInput).toHaveBeenCalledWith(expect.objectContaining({ default: "default.txt" }));
    });
  });
  ...
  describe("askForBin", () => {
    it("passes default bin", async () => {
      ...
      expect(mockInput).toHaveBeenCalledWith(expect.objectContaining({ default: "./gradlew" }));
    });
  });
  ...
  describe("setupGradle", () => {
    ...
    it("cancels when gradle-opts prompt is aborted", async () => {
      mockInput.mockResolvedValueOnce("./gradlew").mockResolvedValueOnce(undefined);
      const result = await setupGradle({});
      expect(result).toEqual({ ok: true, data: { canceled: true } });
    });

    it("cancels when verbose prompt is aborted", async () => {
      mockInput.mockResolvedValueOnce("./gradlew").mockResolvedValueOnce("--info");
      mockSelect.mockResolvedValueOnce(undefined);
      const result = await setupGradle({});
      expect(result).toEqual({ ok: true, data: { canceled: true } });
    });
    ...
    it("saves bin/gradleOpts/verbose when user provides them", async () => {
      mockInput.mockResolvedValueOnce("/usr/bin/gradle").mockResolvedValueOnce("--debug --info");
      mockSelect.mockResolvedValueOnce("yes");
      const config: unknown = {};
  ...
  describe("setupSbt", () => {
    ...
    it("saves all values when user provides them", async () => {
      mockInput.mockResolvedValueOnce("/usr/bin/sbt").mockResolvedValueOnce("-Dsbt.opts=foo");
      mockSelect
  ...
  describe("setupManifestConfig", () => {
    ...
    it("sort comparator orders detected before undetected (lines 97-103)", async () => {
      ...
      const detectedIdx = choices.findIndex((c: unknown) => c.name.includes("Conda"));
      const undetectedIdx = choices.findIndex((c: unknown) => c.name.includes("sbt"));
