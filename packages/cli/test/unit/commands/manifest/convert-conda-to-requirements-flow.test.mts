...
describe("convertCondaToRequirements (file)", () => {
  ...
  it("reads from a file path", async () => {
    const result = await convertCondaToRequirements("environment.yml", "/proj", false);
  ...
  it("logs verbose target path", async () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("target:"));
  });
});
...
describe("convertCondaToRequirements (stdin)", () => {
  ...
  it("logs verbose stdin info when --verbose", async () => {
    ...
    await promise;
    expect(mockLogger.info).toHaveBeenCalledWith("[VERBOSE] reading input from stdin");
  });
