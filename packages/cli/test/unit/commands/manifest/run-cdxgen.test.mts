...
describe("detectNodejsCdxgenSources", () => {
  ...
  it("detects a package-lock.json", async () => {
    mockFindUp.mockImplementation((name: string) =>
      Promise.resolve(name === "package-lock.json" ? "/x/package-lock.json" : undefined),
    );
    const result = await detectNodejsCdxgenSources("/tmp/project");
    expect(result.hasLockfile).toBe(true);
  });
...
describe("runCdxgen", () => {
  ...
  it("warns when output BOM has empty components array", async () => {
    ...
    expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining("bom.json created"));
    expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('empty "components"'));
  });
  ...
  it("warns with non-pre-build lifecycle hint when components empty", async () => {
    ...
    expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("--type"));
  });

  it("does not warn when components are present", async () => {
    mockExistsSync.mockReturnValue(true);
    mockReadFile.mockResolvedValueOnce(JSON.stringify({ components: [{ name: "foo" }] }));
  ...
  it("expands Array-valued flags into multiple --key value pairs", async () => {
    const { argvObjectToArray } = await import("../../../../src/commands/manifest/run-cdxgen.mts");
  ...
  it("emits --key value for string values", async () => {
    const { argvObjectToArray } = await import("../../../../src/commands/manifest/run-cdxgen.mts");
    expect(argvObjectToArray({ output: "out.json" } as unknown)).toEqual(["--output", "out.json"]);
  });
});
