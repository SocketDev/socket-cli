...
// Import after mocks.
const { cmdManifestAuto } = await import("../../../../src/commands/manifest/cmd-manifest-auto.mts");
...
describe("cmd-manifest-auto", () => {
  ...
  describe("run", () => {
    ...
    it("should fail when no targets detected", async () => {
      ...
      expect(mockLogger.fail).toHaveBeenCalledWith(expect.stringContaining("unable to discover"));
      expect(mockGenerateAutoManifest).not.toHaveBeenCalled();
    });
    ...
    it("should generate manifests when targets detected", async () => {
      ...
      expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining("2 targets"));
    });
