...
const {
  ...
  return {
    ...
    mockMeowOrExit: vi.fn((args: unknown) => {
...
// Mock SOCKET_CLI_API_TOKEN environment variable.
vi.mock(import("../../../../src/env/socket-cli-api-token.mts"), () => ({
...
// Import after mocks.
const { cmdWhoami } = await import("../../../../src/commands/whoami/cmd-whoami.mts");
...
describe("cmd-whoami", () => {
  ...
  describe("command metadata", () => {
    it("should have correct description", () => {
      expect(cmdWhoami.description).toBe("Check Socket CLI authentication status");
    });
  ...
  describe("run", () => {
    ...
    describe("authenticated with token from environment variable", () => {
      ...
      it("should display authenticated status in text format", async () => {
        ...
        expect(mockLogger.success).toHaveBeenCalledWith("Authenticated with Socket");
    ...
    describe("not authenticated (no token)", () => {
      ...
      it("should display unauthenticated status in text format", async () => {
        ...
        expect(mockGetDefaultApiToken).toHaveBeenCalled();
        expect(mockLogger.fail).toHaveBeenCalledWith("Not authenticated with Socket");
        expect(mockLogger.log).toHaveBeenCalledWith("");
        expect(mockLogger.log).toHaveBeenCalledWith("To authenticate, run one of:");
    ...
    describe("output format flags", () => {
      ...
      it("should default to text format when no format flag provided", async () => {
        ...
        expect(mockLogger.success).toHaveBeenCalledWith("Authenticated with Socket");
        expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining("Token:"));
      });
