...
// Import after mocks.
const { cmdCargo } = await import("../../../../src/commands/cargo/cmd-cargo.mts");
...
describe("cmd-cargo", () => {
  ...
  describe("command metadata", () => {
    it("should have correct description", () => {
      expect(cmdCargo.description).toBe("Run cargo with Socket Firewall security");
    });
    ...
    it("renders help text via the meow help callback", async () => {
      ...
      mockFilterFlags.mockReturnValue([]);
      const runPromise = cmdCargo.run([], { url: import.meta.url } as ImportMeta, {
        parentName: "socket",
      });
  ...
  describe("run", () => {
    ...
    describe("flag filtering", () => {
      it("should filter out Socket CLI flags and forward cargo flags", async () => {
        ...
        expect(mockSpawnSfwDlx).toHaveBeenCalledWith(["cargo", "build", "--release"], {
          stdio: "inherit",
        });
      });
    });
    ...
    describe("command forwarding", () => {
      it("should forward cargo commands to sfw with correct args", async () => {
        ...
        expect(mockSpawnSfwDlx).toHaveBeenCalledWith(["cargo", "install", "ripgrep"], {
          stdio: "inherit",
        });
