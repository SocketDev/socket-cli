...
describe("fetchRepoAnalytics", () => {
  it("fetches repository analytics successfully", async () => {
    const { mockHandleApi, mockSdk } = await setupSdkMockSuccess("getRepoAnalytics", {
      commits: 450,
      contributors: 12,
      issues: 85,
      pullRequests: 120,
  ...
  it("handles API call failure", async () => {
    await setupSdkMockError("getRepoAnalytics", "Repository analytics unavailable", 404);
  ...
  it("handles different time ranges", async () => {
    ...
    for (let i = 0, { length } = timeRanges; i < length; i += 1) {
      ...
      expect(mockSdk.getRepoAnalytics).toHaveBeenCalledWith("test-repo", time.toString());
    }
  });
