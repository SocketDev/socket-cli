...
describe("pr-lifecycle-logger", () => {
  ...
  describe("logPrEvent", () => {
    it("logs created event with success logger", async () => {
      ...
      expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining("PR #123"));
    ...
    it("logs merged event with success logger", async () => {
      ...
      expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining("PR #456"));
      expect(mockLogger.success).toHaveBeenCalledWith(
        expect.stringContaining("GHSA-abcd-efgh-ijkl"),
      );
      expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining("Branch cleaned up"));
    });

    it("logs closed event with info logger", async () => {
      logPrEvent("closed", 789, "GHSA-test-test-test");

      expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("PR #789"));
      expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("GHSA-test-test-test"));
    });

    it("logs updated event with info logger", async () => {
      logPrEvent("updated", 111, "GHSA-update-test", "Updated from base branch");

      expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("PR #111"));
      expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("GHSA-update-test"));
    ...
    it("logs superseded event with warn logger", async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("PR #222"));
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("GHSA-supersede"));
    });

    it("logs failed event with error logger", async () => {
      logPrEvent("failed", 333, "GHSA-fail-test", "API error");

      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("PR #333"));
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("GHSA-fail-test"));
    ...
    it("handles missing details parameter", async () => {
      ...
      expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining("PR #444"));
      expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining("GHSA-no-details"));
      // Should not include a colon when no details.
      expect(mockLogger.success).toHaveBeenCalledWith(expect.not.stringContaining(": undefined"));
    });
    ...
    it("applies color coding to symbols", async () => {
      ...
      expect(mockLogger.success).toHaveBeenCalledWith(expect.stringContaining("[green]✓[/green]"));
    });
  });
});
