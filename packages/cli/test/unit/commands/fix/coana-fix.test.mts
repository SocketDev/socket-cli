...
const mockCheckCiEnvVars = vi.hoisted(() => vi.fn(() => ({ missing: [], present: [] })));
...
const mockGitCheckoutBranch = vi.hoisted(() => vi.fn(() => Promise.resolve(true)));
const mockGitCommit = vi.hoisted(() => vi.fn(() => Promise.resolve(true)));
const mockGitPushBranch = vi.hoisted(() => vi.fn(() => Promise.resolve(true)));
const mockGitRemoteBranchExists = vi.hoisted(() => vi.fn(() => Promise.resolve(false)));
const mockGitResetAndClean = vi.hoisted(() => vi.fn(() => Promise.resolve(true)));
const mockCleanupStaleBranch = vi.hoisted(() => vi.fn(() => Promise.resolve(true)));
...
describe("coanaFix (coverage)", () => {
  ...
  describe("local mode info messages", () => {
    ...
    it("writes the local-mode output file when outputFile is provided", async () => {
      ...
      expect(mockWriteFile).toHaveBeenCalledWith("/tmp/out.json", '{"fixed": true}', "utf8");
      expect(result.ok).toBe(true);
    });
  ...
  describe("CI mode per-GHSA PR creation paths", () => {
    ...
    it("closes superseded PRs before creating a new one", async () => {
      ...
      expect(mockLogPrEvent).toHaveBeenCalledWith("superseded", 1, "GHSA-AAAA-BBBB-CCCC");
