 ...
 *
 * Purpose: Tests environment variable instruction generation for CI-based
 * automated fix workflows. Validates that the helper functions correctly format
 * and document required environment variables for enabling automatic pull
 * request creation in CI environments.
 *
 * Test Coverage: - Environment variable instruction generation with exact var
 * names - Instruction formatting and consistency validation - CI environment
 * variable checking.
 *
 ...
 * helper functions - src/commands/fix/handle-fix.mts - Main fix command handler
...
vi.mock(import("@socketsecurity/lib-stable/env/socket-cli"), async (importOriginal) => {
...
vi.mock(import("../../../../src/env/socket-cli-git-user-email.mts"), () => mockGitEmail);

// Mock SOCKET_CLI_GIT_USER_NAME.
const mockGitUser = vi.hoisted(() => ({ SOCKET_CLI_GIT_USER_NAME: "" }));
vi.mock(import("../../../../src/env/socket-cli-git-user-name.mts"), () => mockGitUser);

// Mock GITHUB_REPOSITORY.
const mockGithubRepo = vi.hoisted(() => ({ GITHUB_REPOSITORY: "" }));
vi.mock(import("../../../../src/env/github-repository.mts"), () => mockGithubRepo);
...
describe("env-helpers", () => {
  ...
  describe("checkCiEnvVars", () => {
    it("should return all missing when no env vars are set", () => {
      ...
      expect(result.missing).toContain("SOCKET_CLI_GITHUB_TOKEN (or GITHUB_TOKEN)");
    });
    ...
    it("should return GitHub token as present when set", () => {
      ...
      expect(result.present).toContain("SOCKET_CLI_GITHUB_TOKEN (or GITHUB_TOKEN)");
      expect(result.missing).not.toContain("SOCKET_CLI_GITHUB_TOKEN (or GITHUB_TOKEN)");
    });
  ...
  describe("getFixEnv", () => {
    ...
    it("should warn when CI is set but other vars are missing", async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("CI mode detected"));
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("Missing:"));
    });
    ...
    it("should log debug message when not in CI but some vars are set", async () => {
      ...
      expect(mockDebug).toHaveBeenCalledWith(expect.stringContaining("isCi is false"));
    });
