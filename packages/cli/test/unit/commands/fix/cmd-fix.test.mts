...
describe("cmd-fix", () => {
  ...
  describe("run", () => {
    ...
    it("should fail if org slug cannot be resolved", async () => {
      ...
      expect(mockHandleFix).not.toHaveBeenCalled();
      expect(mockLogger.fail).toHaveBeenCalledWith(expect.stringContaining("Unable to resolve"));
    });
    ...
    it("should pass comma-separated --id values to handleFix", async () => {
      await cmdFix.run(["--id", "CVE-2021-23337,GHSA-xxxx-yyyy-zzzz"], importMeta, context);
    ...
    it("should pass --show-affected-direct-dependencies flag to handleFix", async () => {
      await cmdFix.run(["--show-affected-direct-dependencies"], importMeta, context);
    ...
    describe("misplaced vulnerability identifier detection", () => {
      ...
      it.each([
        ...
        async (_label, input, expectedSuggestion) => {
          ...
          expect(mockHandleFix).not.toHaveBeenCalled();
          expect(mockLogger.fail).toHaveBeenCalledWith(
            expect.stringContaining("looks like a vulnerability identifier, not a directory path"),
          );
          expect(mockLogger.fail).toHaveBeenCalledWith(
    ...
    describe("target directory validation", () => {
      ...
      it("lets a real directory flow through to handleFix", async () => {
        ...
        expect(mockHandleFix).toHaveBeenCalledWith(expect.objectContaining({ cwd: realDir }));
    ...
    it("should show all vulnerabilities in dry-run with --all", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("all vulnerabilities"));
    });
    ...
    it("should show compute-only mode in dry-run with --no-apply-fixes", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("compute fixes only"));
    });
    ...
    it("should show auto-discovered targets in dry-run when no --id or --all", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("auto-discovered"));
    });
