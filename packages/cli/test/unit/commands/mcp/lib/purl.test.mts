...
describe("buildPurl", () => {
  describe("npm ecosystem", () => {
    ...
    it("throws on a bare @something without a slash (rejected by packageurl-js)", () => {
      ...
      expect(() => buildPurl("npm", "@notscoped", "1.2.3")).toThrow(/Invalid purl: npm/);
    });
  ...
  describe("maven ecosystem", () => {
    it("splits groupId:artifactId on `:`", () => {
      expect(buildPurl("maven", "org.springframework:spring-core", "6.1.0")).toBe(
  ...
  describe("golang ecosystem", () => {
    it("splits a deep module path on the last slash", () => {
      ...
      expect(buildPurl("golang", "github.com/socketdev/socket-cli", "v1.0.0")).toBe(
        "pkg:golang/github.com/socketdev/socket-cli@v1.0.0",
      );
    });

    it("treats a single-segment module as bare name", () => {
      expect(buildPurl("golang", "context", "v1.2.3")).toBe("pkg:golang/context@v1.2.3");
    });
  });

  describe("other ecosystems (no special handling)", () => {
    it("builds a bare pypi PURL", () => {
      expect(buildPurl("pypi", "requests", "2.31.0")).toBe("pkg:pypi/requests@2.31.0");
    });
    ...
    it("preserves real cargo versions other than 1.0.0", () => {
      expect(buildPurl("cargo", "serde", "1.0.193")).toBe("pkg:cargo/serde@1.0.193");
    });
  ...
  describe("ecosystem case folding", () => {
    it("lowercases the ecosystem so NPM works the same as npm", () => {
      expect(buildPurl("NPM", "left-pad", "1.3.0")).toBe("pkg:npm/left-pad@1.3.0");
    });
  ...
  describe("version handling", () => {
    ...
    it("preserves a pre-release version", () => {
      expect(buildPurl("npm", "foo", "4.2.0-beta.1")).toBe("pkg:npm/foo@4.2.0-beta.1");
    });
  });
});
