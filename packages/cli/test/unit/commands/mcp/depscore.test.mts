...
const { DEPSCORE_TOOL_DESCRIPTION, DEPSCORE_TOOL_NAME, DepscoreInputSchema, runDepscore } =
...
describe("runDepscore — SDK setup", () => {
  it("returns isError when SDK setup fails", async () => {
    ...
    const result = await runDepscore({ packages: [{ depname: "lodash" }] }, { apiToken: "test_x" });
  ...
  it("caches SDK clients per token (one setupSdk per distinct token)", async () => {
    ...
    await runDepscore({ packages: [{ depname: "a" }] }, { apiToken: "test_token_1" });
    await runDepscore({ packages: [{ depname: "b" }] }, { apiToken: "test_token_1" });
    expect(mockSetupSdk).toHaveBeenCalledTimes(1);
    await runDepscore({ packages: [{ depname: "c" }] }, { apiToken: "test_token_2" });
...
describe("runDepscore — payload shaping", () => {
  ...
  it("defaults ecosystem to npm when caller omits it", async () => {
    await runDepscore({ packages: [{ depname: "noeco" }] }, { apiToken: "test_a" });
    const purl = mockBatchPackageFetch.mock.calls[0]![0].components[0].purl;
    expect(purl).toContain("pkg:npm/noeco");
  });

  it("defaults version to unknown when caller omits it", async () => {
    await runDepscore({ packages: [{ depname: "unknownversion" }] }, { apiToken: "test_a" });
    const purl = mockBatchPackageFetch.mock.calls[0]![0].components[0].purl;
...
describe("runDepscore — response handling", () => {
  it("formats a single artifact with score breakdown", async () => {
    ...
    const result = await runDepscore({ packages: [{ depname: "lodash" }] }, { apiToken: "test_a" });
  ...
  it("omits overall and uuid from the score breakdown", async () => {
    ...
    const result = await runDepscore({ packages: [{ depname: "foo" }] }, { apiToken: "test_a" });
  ...
  it("returns an error when the response only has _type rows", async () => {
    mockBatchPackageFetch.mockResolvedValue(makeOk([{ _type: "summary", count: 0 }]));
    const result = await runDepscore({ packages: [{ depname: "foo" }] }, { apiToken: "test_a" });
    expect(result.isError).toBe(true);
...
describe("runDepscore — error paths", () => {
  it("surfaces a 401 with a re-authenticate message", async () => {
    mockBatchPackageFetch.mockResolvedValue(makeErr(401, "Unauthorized", "invalid token"));
    const result = await runDepscore({ packages: [{ depname: "foo" }] }, { apiToken: "test_bad" });
    expect(result.isError).toBe(true);
  ...
  it("surfaces a 403 with a permissions message", async () => {
    mockBatchPackageFetch.mockResolvedValue(makeErr(403, "Forbidden", "missing scope"));
    const result = await runDepscore(
