...
const { runStdioTransport } = await import("../../../../src/commands/mcp/transport-stdio.mts");
...
describe("runStdioTransport", () => {
  it("logs the start message before connecting", async () => {
    ...
    expect(mockLogger.info).toHaveBeenNthCalledWith(1, "Starting Socket MCP server in stdio mode");
  });
  ...
  it("logs the ready message after connect resolves", async () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith("Starting Socket MCP server in stdio mode");
