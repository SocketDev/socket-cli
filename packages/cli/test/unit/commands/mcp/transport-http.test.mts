...
const { runHttpTransport } = await import("../../../../src/commands/mcp/transport-http.mts");
...
async function startServer(
  ...
  const config = {
    ...
    oauthRequiredScopes: overrides.oauthRequiredScopes ?? (["packages:list"] as const),
    port,
...
describe("runHttpTransport — session reuse", () => {
  it("handles initialize with a stale Mcp-Session-Id header (creates new session)", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.headers["mcp-session-id"]).toBeTypeOf("string");
    expect(res.headers["mcp-session-id"]).not.toBe("stale-session-that-doesnt-exist");
  });
...
describe("runHttpTransport — routing", () => {
  ...
  it("logs the host when origin is absent on initialize", async () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining(`from localhost:${port}`));
  });
...
describe("runHttpTransport — OAuth disabled", () => {
  it("serves /.well-known/oauth-protected-resource as 404 when OAuth is off", async () => {
    const { port } = await startServer();
    const res = await httpRequest(`http://127.0.0.1:${port}/.well-known/oauth-protected-resource`, {
      headers: { origin: `http://localhost:${port}` },
    });
