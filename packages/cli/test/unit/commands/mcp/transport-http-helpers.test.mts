...
describe("getRequestBaseUrl", () => {
  it("uses Host header when trustProxy=false (default)", () => {
    const url = getRequestBaseUrl(makeReq({ headers: { host: "example.com" } }), 3000, false);
...
describe("parseJsonObject", () => {
  ...
  it("throws with context on malformed JSON", () => {
    expect(() => parseJsonObject("{not valid", "ctx")).toThrow(/ctx returned invalid JSON/);
  });

  it("throws when payload is a JSON array", () => {
    expect(() => parseJsonObject("[1,2,3]", "ctx")).toThrow(/expected a JSON object/);
  });

  it("throws when payload is null", () => {
    expect(() => parseJsonObject("null", "ctx")).toThrow(/expected a JSON object/);
  });
  ...
  it("throws when payload is a primitive string", () => {
    expect(() => parseJsonObject('"hello"', "ctx")).toThrow(/expected a JSON object/);
  });
  ...
  it("throws with String(error) when caught value is not an Error", () => {
    ...
    try {
      expect(() => parseJsonObject("{}", "ctx")).toThrow(/ctx returned invalid JSON: plain string/);
    } finally {
...
describe("buildProtectedResourceMetadata", () => {
  it("packages issuer + resource + scopes + name", () => {
    ...
    const result = buildProtectedResourceMetadata(baseUrl, metadata, ["a:read", "b:write"]);
...
describe("splitScopes", () => {
  ...
  it("splits on tabs and other whitespace", () => {
    expect(splitScopes("a:read\tb:write\nc:exec")).toEqual(["a:read", "b:write", "c:exec"]);
  });
...
describe("destroySessionEntry", () => {
  ...
  it("swallows synchronous throws from transport.close()", () => {
    ...
    const sessions = new Map<string, SessionLike>([["s1", fakeSession({ transportClose })]]);
    const log = { info: vi.fn() };
  ...
  it("swallows async rejections from server.close() (the .catch arm)", async () => {
    ...
    const sessions = new Map<string, SessionLike>([["s1", fakeSession({ serverClose })]]);
    const log = { info: vi.fn() };
    destroySessionEntry("s1", sessions, log);
...
describe("handleRequestSafely", () => {
  ...
  it("logs and writes a 500 JSON-RPC envelope when the handler throws", async () => {
    ...
    expect(log.error).toHaveBeenCalledWith(expect.stringContaining("transport boom"));
  ...
  it("coerces non-Error throws via the template literal", async () => {
    ...
    expect(log.error).toHaveBeenCalledWith("Error processing POST request: plain string error");
  });
});

describe("module-level constants", () => {
  it("exposes the OAuth well-known path", () => {
    expect(OAUTH_WELL_KNOWN_PATH).toBe("/.well-known/oauth-authorization-server");
  });

  it("exposes the protected-resource metadata path", () => {
    expect(OAUTH_PROTECTED_RESOURCE_METADATA_PATH).toBe("/.well-known/oauth-protected-resource");
  });
});
