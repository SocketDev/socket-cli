...
vi.mock(import("../../../../src/commands/mcp/handle-mcp.mts"), () => ({
...
describe("cmdMcp — mode selection", () => {
  it("defaults to stdio mode", async () => {
    ...
    expect(mockHandleMcp).toHaveBeenCalledWith(expect.objectContaining({ http: false }));
  });

  it("switches to HTTP mode with --http", async () => {
    await cmdMcp.run(["--http"], importMeta, context);
    expect(mockHandleMcp).toHaveBeenCalledWith(expect.objectContaining({ http: true }));
  });
...
describe("cmdMcp — port flag", () => {
  it("defaults port to 3000 when no flag or env is set", async () => {
    await cmdMcp.run([], importMeta, context);
    expect(mockHandleMcp).toHaveBeenCalledWith(expect.objectContaining({ port: 3000 }));
  });

  it("honors --port", async () => {
    await cmdMcp.run(["--port", "5151"], importMeta, context);
    expect(mockHandleMcp).toHaveBeenCalledWith(expect.objectContaining({ port: 5151 }));
  });
  ...
  it("falls back to MCP_PORT env when no --port flag is given", async () => {
    ...
    await cmdMcp.run([], importMeta, context);
    expect(mockHandleMcp).toHaveBeenCalledWith(expect.objectContaining({ port: 8081 }));
  });
...
describe("cmdMcp — trust-proxy", () => {
  it("forwards --trust-proxy=true", async () => {
    ...
    expect(mockHandleMcp).toHaveBeenCalledWith(expect.objectContaining({ trustProxy: true }));
  });
  ...
  it("defaults to false when neither flag nor env are set", async () => {
    await cmdMcp.run([], importMeta, context);
    expect(mockHandleMcp).toHaveBeenCalledWith(expect.objectContaining({ trustProxy: false }));
  });
});
...
describe("cmdMcp — OAuth flags", () => {
  ...
  it("parses --oauth-required-scopes as whitespace-separated", async () => {
    await cmdMcp.run(["--http", "--oauth-required-scopes", "a:read  b:write"], importMeta, context);
    expect(mockHandleMcp).toHaveBeenCalledWith(
      expect.objectContaining({
