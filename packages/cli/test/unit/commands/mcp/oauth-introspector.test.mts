...
vi.mock(import("@socketsecurity/lib-stable/http-request/request"), async (importOriginal) => {
...
describe("OAuthIntrospector — loadMetadata", () => {
  it("fetches and returns valid metadata on success", async () => {
    mockHttpRequest.mockResolvedValueOnce(fakeResponse({ status: 200, body: validMetadata }));
  ...
  it("memoizes the metadata promise (one fetch across many calls)", async () => {
    mockHttpRequest.mockResolvedValue(fakeResponse({ status: 200, body: validMetadata }));
    const intro = newIntrospector();
  ...
  it("throws on non-2xx status with the body in the message", async () => {
    mockHttpRequest.mockResolvedValueOnce(fakeResponse({ status: 500, text: "boom" }));
    const intro = newIntrospector();
  ...
  it("throws on 4xx status with the body in the message", async () => {
    mockHttpRequest.mockResolvedValueOnce(fakeResponse({ status: 404, text: "not found" }));
    const intro = newIntrospector();
    await expect(intro.loadMetadata()).rejects.toThrow(
  ...
  it("throws when authorization_endpoint is missing", async () => {
    ...
    mockHttpRequest.mockResolvedValueOnce(fakeResponse({ status: 200, body: partial }));
    const intro = newIntrospector();
    await expect(intro.loadMetadata()).rejects.toThrow(
  ...
  it("throws when token_endpoint is wrong type", async () => {
    ...
    const intro = newIntrospector();
    await expect(intro.loadMetadata()).rejects.toThrow(/missing required field: token_endpoint/);
  });

  it("clears the cached promise after a failure so the next call retries", async () => {
    mockHttpRequest.mockResolvedValueOnce(fakeResponse({ status: 500, text: "transient" }));
    const intro = newIntrospector();
    await expect(intro.loadMetadata()).rejects.toThrow();
  ...
  it("throws when the response body is not valid JSON", async () => {
    mockHttpRequest.mockResolvedValueOnce(fakeResponse({ status: 200, text: "not-json{" }));
    const intro = newIntrospector();
    await expect(intro.loadMetadata()).rejects.toThrow(
...
describe("OAuthIntrospector — verifyAccessToken", () => {
  ...
  it("returns null when introspection says inactive", async () => {
    setupMetadata();
    mockHttpRequest.mockResolvedValueOnce(fakeResponse({ status: 200, body: { active: false } }));
    const intro = newIntrospector();
  ...
  it("throws on non-2xx introspection status", async () => {
    setupMetadata();
    mockHttpRequest.mockResolvedValueOnce(fakeResponse({ status: 500, text: "broken" }));
    const intro = newIntrospector();
  ...
  it("sends a Basic-auth header derived from clientId:clientSecret", async () => {
    ...
    expect(introCall![1].headers["content-type"]).toBe("application/x-www-form-urlencoded");
...
describe("OAuthIntrospector — authenticateRequest", () => {
  ...
  function newIntrospectorWithMetadataPrimed(): InstanceType<typeof OAuthIntrospector> {
    const intro = newIntrospector();
