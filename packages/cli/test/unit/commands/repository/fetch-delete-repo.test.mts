...
describe("fetchDeleteRepo", () => {
  it("deletes repository successfully", async () => {
    ...
    const { mockHandleApi, mockSdk } = await setupSdkMockSuccess("deleteRepository", mockData);

    const result = await fetchDeleteRepo("test-org", "deleted-repo");

    expect(mockSdk.deleteRepository).toHaveBeenCalledWith("test-org", "deleted-repo");
