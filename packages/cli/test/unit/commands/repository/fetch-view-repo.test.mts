...
describe("fetchViewRepo", () => {
  it("views repository successfully", async () => {
    ...
    const { mockHandleApi, mockSdk } = await setupSdkMockSuccess("getRepository", mockData);
  ...
  it("handles private repository access", async () => {
    ...
    expect(result.ok).toBe(true);
    expect(mockSdk.getRepository).toHaveBeenCalledWith("private-org", "secret-project");
  });
