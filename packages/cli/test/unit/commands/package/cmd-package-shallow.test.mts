...
vi.mock(import("../../../../src/commands/package/handle-purls-shallow-score.mts"), () => ({
...
describe("cmd-package-shallow", () => {
  ...
  describe("run", () => {
    ...
    it("should support --dry-run flag", async () => {
      await cmdPackageShallow.run(["--dry-run", "npm", "webtorrent"], importMeta, context);
    ...
    it("should handle multiple packages with same ecosystem", async () => {
      await cmdPackageShallow.run(["npm", "webtorrent", "babel"], importMeta, context);
    ...
    it("should handle purl format with pkg: prefix", async () => {
      await cmdPackageShallow.run(["pkg:npm/webtorrent@1.9.1"], importMeta, context);
    ...
    it("should handle mixed purl formats with same ecosystem", async () => {
      await cmdPackageShallow.run(["npm", "webtorrent@1.0.1", "babel"], importMeta, context);
    ...
    it("should handle multiple purls from different ecosystems", async () => {
      await cmdPackageShallow.run(["npm/webtorrent", "golang/babel"], importMeta, context);
    ...
    it("should pass --json flag to handlePurlsShallowScore", async () => {
      await cmdPackageShallow.run(["npm", "webtorrent", "--json"], importMeta, context);
    ...
    it("should pass --markdown flag to handlePurlsShallowScore", async () => {
      await cmdPackageShallow.run(["npm", "webtorrent", "--markdown"], importMeta, context);
    ...
    it("should handle different ecosystems", async () => {
      await cmdPackageShallow.run(["maven", "webtorrent", "babel"], importMeta, context);
    ...
    it("should show query parameters in dry-run mode", async () => {
      await cmdPackageShallow.run(["--dry-run", "npm", "webtorrent", "babel"], importMeta, context);
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("count: 2"));
    });

    it("should fail with invalid first parameter", async () => {
      await cmdPackageShallow.run(["not-an-ecosystem-or-purl"], importMeta, context);
