...
describe("outputPurlsShallowScore", () => {
  ...
  it("defaults exit code to 1 when result.code is missing", async () => {
    outputPurlsShallowScore(["pkg:npm/x"], { ok: false, message: "fail" } as unknown, "text");
    expect(process.exitCode).toBe(1);
  });

  it("logs JSON for failed result in JSON mode", async () => {
    outputPurlsShallowScore(["pkg:npm/x"], { ok: false, message: "fail" } as unknown, "json");
  ...
  it("logs text report for successful result in text mode", async () => {
    ...
    expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining("Shallow Package Score"));
  });
