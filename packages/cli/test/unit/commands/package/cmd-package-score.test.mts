...
vi.mock(import("../../../../src/commands/package/handle-purl-deep-score.mts"), () => ({
...
describe("cmd-package-score", () => {
  ...
  describe("run", () => {
    ...
    it("should support --dry-run flag", async () => {
      ...
      await cmdPackageScore.run(["--dry-run", "npm", "babel-cli"], importMeta, context);
    ...
    it("should call handlePurlDeepScore with ecosystem and package name", async () => {
      ...
      expect(mockHandlePurlDeepScore).toHaveBeenCalledWith("pkg:npm/babel-cli", "text");
    });

    it("should handle purl format with pkg: prefix", async () => {
      mockHasDefaultApiToken.mockReturnValueOnce(true);

      await cmdPackageScore.run(["pkg:npm/babel-cli@1.0.0"], importMeta, context);

      expect(mockHandlePurlDeepScore).toHaveBeenCalledWith("pkg:npm/babel-cli@1.0.0", "text");
    });
    ...
    it("should pass --json flag to handlePurlDeepScore", async () => {
      ...
      await cmdPackageScore.run(["npm", "babel-cli", "--json"], importMeta, context);

      expect(mockHandlePurlDeepScore).toHaveBeenCalledWith("pkg:npm/babel-cli", "json");
    });

    it("should pass --markdown flag to handlePurlDeepScore", async () => {
      mockHasDefaultApiToken.mockReturnValueOnce(true);

      await cmdPackageScore.run(["npm", "babel-cli", "--markdown"], importMeta, context);

      expect(mockHandlePurlDeepScore).toHaveBeenCalledWith("pkg:npm/babel-cli", "markdown");
    });

    it("should fail when both --json and --markdown flags are set", async () => {
      mockHasDefaultApiToken.mockReturnValueOnce(true);

      await cmdPackageScore.run(["npm", "babel-cli", "--json", "--markdown"], importMeta, context);
    ...
    it("should handle package with version", async () => {
      ...
      expect(mockHandlePurlDeepScore).toHaveBeenCalledWith("pkg:npm/eslint@1.0.0", "text");
    });
    ...
    it("should handle different ecosystems", async () => {
      ...
      expect(mockHandlePurlDeepScore).toHaveBeenCalledWith("pkg:pypi/requests", "text");
    });
