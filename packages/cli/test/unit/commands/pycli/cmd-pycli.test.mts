...
describe("cmd-pycli", () => {
  ...
  describe("command metadata", () => {
    it("should have correct description", () => {
      expect(cmdPyCli.description).toBe("Run Socket Python CLI (socketsecurity) directly");
    });
  ...
  describe("run", () => {
    ...
    it("should pass arguments to spawnSocketPyCli", async () => {
      ...
      await cmdPyCli.run(["--generate-license", "--repo", "owner/repo", "."], importMeta, context);
    ...
    it("shows wrapper help when --help is passed and skips spawn", async () => {
      ...
      try {
        await cmdPyCli.run(["--help"], importMeta, context).catch(() => undefined);
        expect(mockSpawnSocketPyCli).not.toHaveBeenCalled();
      } finally {
        exitSpy.mockRestore();
      }
    ...
    it("should log info message when invoking Python CLI", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith("Invoking Socket Python CLI…");
    });
