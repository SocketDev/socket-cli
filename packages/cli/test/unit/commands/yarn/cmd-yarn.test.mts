  ...
  describe("run", () => {
    ...
    describe("help flag", () => {
      it("should display help text with --help flag", async () => {
        ...
        await expect(cmdYarn.run(["--help"], importMeta, context)).rejects.toThrow();
    ...
    describe("dry-run behavior", () => {
      ...
      it("should show dry-run output with yarn install command", async () => {
        await cmdYarn.run(["--dry-run", "install", "lodash"], importMeta, context);
    ...
    describe("flag filtering", () => {
      ...
      it("should filter out --config flag when forwarding to sfw", async () => {
        ...
        await cmdYarn.run(["--config", "{}", "install", "lodash"], importMeta, context);
      ...
      it("should preserve yarn flags while filtering Socket flags", async () => {
        ...
        await cmdYarn.run(["--config", "{}", "install", "--dev", "lodash"], importMeta, context);

        // yarn's --dev should be preserved.
        expect(mockSpawnSfwDlx).toHaveBeenCalledWith(["yarn", "install", "--dev", "lodash"], {
          stdio: "inherit",
        });
      });
      ...
      it("should handle --no-banner flag", async () => {
        ...
        await cmdYarn.run(["--no-banner", "install", "lodash"], importMeta, context);
    ...
    describe("command structure", () => {
      ...
      it("should forward yarn install with version specifier", async () => {
        ...
        expect(mockSpawnSfwDlx).toHaveBeenCalledWith(["yarn", "add", "lodash@4.17.21"], {
          stdio: "inherit",
        });
      });
      ...
      it("should forward yarn global add command", async () => {
        ...
        expect(mockSpawnSfwDlx).toHaveBeenCalledWith(["yarn", "global", "add", "cowsay"], {
          stdio: "inherit",
        });
      });
      ...
      it("should forward yarn add with multiple packages", async () => {
        ...
        await cmdYarn.run(["add", "lodash", "express", "react"], importMeta, context);
    ...
    describe("exit handler callback", () => {
      ...
      it("should call process.exit with numeric exit code", async () => {
        ...
        expect(mockTrackSubprocessExit).toHaveBeenCalledWith(YARN, expect.any(Number), 42);
      ...
      it("should call process.kill with signal", async () => {
        ...
        expect(mockTrackSubprocessExit).toHaveBeenCalledWith(YARN, expect.any(Number), undefined);
    ...
    describe("command name constant", () => {
      it("should use YARN constant as command name", async () => {
        const { CMD_NAME } = await import("../../../../src/commands/yarn/cmd-yarn.mts");
