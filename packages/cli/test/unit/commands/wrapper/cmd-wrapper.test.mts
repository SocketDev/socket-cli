...
vi.mock(import("../../../../src/commands/wrapper/add-socket-wrapper.mts"), () => ({
  addSocketWrapper: mockAddSocketWrapper,
}));

vi.mock(import("../../../../src/commands/wrapper/remove-socket-wrapper.mts"), () => ({
  removeSocketWrapper: mockRemoveSocketWrapper,
}));

vi.mock(import("../../../../src/commands/wrapper/check-socket-wrapper-setup.mts"), () => ({
  checkSocketWrapperSetup: mockCheckSocketWrapperSetup,
}));

vi.mock(import("../../../../src/commands/wrapper/postinstall-wrapper.mts"), () => ({
...
// Import after mocks.
const { cmdWrapper } = await import("../../../../src/commands/wrapper/cmd-wrapper.mts");
...
describe("cmd-wrapper", () => {
  ...
  describe("command metadata", () => {
    it("should have correct description", () => {
      expect(cmdWrapper.description).toBe("Enable or disable the Socket npm/pnpm exec wrapper");
    });
  ...
  describe("run", () => {
    ...
    it("should fail when no shell config files exist", async () => {
      ...
      expect(mockLogger.fail).toHaveBeenCalledWith(expect.stringContaining("bash profile"));
    });
    ...
    it("should output JSON when --json flag is set", async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining('"action"'));
      expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining('"enabled"'));
    });
    ...
    it("should show disabled status in JSON output", async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining('"disabled"'));
    });
    ...
    it("shows skipped-files section in markdown when files are already configured", async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining("## Skipped Files"));
    });
  });
});
