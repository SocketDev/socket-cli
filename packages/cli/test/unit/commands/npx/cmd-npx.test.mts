  ...
  describe("run", () => {
    ...
    describe("help flag", () => {
      it("should display help text with --help flag", async () => {
        ...
        await expect(cmdNpx.run(["--help"], importMeta, context)).rejects.toThrow();
    ...
    describe("dry-run behavior", () => {
      ...
      it("should show dry-run output with pnpm exec command", async () => {
        ...
        const hasArgs = logCalls.some(
          (call) => typeof call === "string" && (call.includes("cowsay") || call.includes("hello")),
        );
        expect(hasArgs).toBe(true);
      });

      it("should filter Socket flags in dry-run output", async () => {
        await cmdNpx.run(["--dry-run", "--config", "{}", "cowsay", "hello"], importMeta, context);
    ...
    describe("flag filtering", () => {
      ...
      it("should filter out --config flag when forwarding to sfw", async () => {
        ...
        await cmdNpx.run(["--config", "{}", "cowsay", "hello"], importMeta, context);
      ...
      it("should filter out multiple Socket CLI flags", async () => {
        ...
        await cmdNpx.run(["--config", "{}", "--no-banner", "cowsay", "hello"], importMeta, context);
      ...
      it("should handle --no-banner flag", async () => {
        ...
        await cmdNpx.run(["--no-banner", "cowsay", "hello"], importMeta, context);
    ...
    describe("command structure", () => {
      ...
      it("should forward pnpm exec with version specifier", async () => {
        ...
        expect(mockSpawnSfw).toHaveBeenCalledWith(["npx", "cowsay@1.6.0", "hello"], {
          stdio: "inherit",
        });
      });
      ...
      it("should forward pnpm exec with --yes flag", async () => {
        ...
        expect(mockSpawnSfw).toHaveBeenCalledWith(["npx", "--yes", "cowsay", "hello"], {
          stdio: "inherit",
        });
      });
      ...
      it("should forward pnpm exec with --package flag", async () => {
        ...
        expect(mockSpawnSfw).toHaveBeenCalledWith(["npx", "--package=cowsay", "cowsay", "hello"], {
          stdio: "inherit",
        });
      });
    });
    ...
    describe("exit handler callback", () => {
      ...
      it("should call process.exit with numeric exit code", async () => {
        ...
        expect(mockTrackSubprocessExit).toHaveBeenCalledWith(NPX, expect.any(Number), 42);
      ...
      it("should call process.kill with signal", async () => {
        ...
        expect(mockTrackSubprocessExit).toHaveBeenCalledWith(NPX, expect.any(Number), undefined);
