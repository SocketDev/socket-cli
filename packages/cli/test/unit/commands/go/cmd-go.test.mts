...
import { cmdGo } from '../../../../src/commands/go/cmd-go.mts'
...
describe('go firewall integration', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.run.mockResolvedValue({ code: 0, signal: noChildSignal })
    mocks.which.mockResolvedValue('/example/bin/go')
  ...
  it.each([
    ...
    await cmdGo.run(args, import.meta, context)
    expect(mocks.run).toHaveBeenCalledWith(['go', ...args], {
      stdio: 'inherit',
    })
  })
  ...
  it.each(['SIGTERM', 'SIGINT'] as const)(
    ...
    async signal => {
      ...
      await cmdGo.run([], import.meta, context)
  ...
  it('retains failure status when firewall setup rejects', async () => {
    ...
    await expect(cmdGo.run([], import.meta, context)).rejects.toBeInstanceOf(
      Error,
    )
