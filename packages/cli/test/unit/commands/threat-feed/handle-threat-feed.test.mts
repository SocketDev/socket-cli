 ...
 * pipeline.
 *
 * Related Files: - src/commands/handleThreatFeed.mts (implementation)
 */
...
vi.mock(import("../../../../src/commands/threat-feed/fetch-threat-feed.mts"), () => ({
  fetchThreatFeed: vi.fn(),
}));
vi.mock(import("../../../../src/commands/threat-feed/output-threat-feed.mts"), () => ({
...
describe("handleThreatFeed", () => {
  ...
  it("handles different ecosystems", async () => {
    ...
    for (let i = 0, { length } = ecosystems; i < length; i += 1) {
      ...
      expect(fetchThreatFeed).toHaveBeenCalledWith(expect.objectContaining({ ecosystem }));
    }
  });
  ...
  it("handles different filter types", async () => {
    ...
    for (let i = 0, { length } = filters; i < length; i += 1) {
      ...
      expect(fetchThreatFeed).toHaveBeenCalledWith(expect.objectContaining({ filter }));
    }
  });
