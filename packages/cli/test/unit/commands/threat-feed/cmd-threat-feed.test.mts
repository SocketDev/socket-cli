...
vi.mock(import("../../../../src/commands/threat-feed/handle-threat-feed.mts"), () => ({
...
// Import after mocks.
const { cmdThreatFeed } = await import("../../../../src/commands/threat-feed/cmd-threat-feed.mts");
...
describe("cmd-threat-feed", () => {
  ...
  describe("run", () => {
    ...
    it("throws InputError on negative --per-page", async () => {
      ...
      await expect(cmdThreatFeed.run(["--per-page", "-1"], importMeta, context)).rejects.toThrow(
    ...
    it("warns about extra positional args beyond the recognized slots", async () => {
      ...
      await cmdThreatFeed.run(["npm", "lodash", "extra-arg-1", "extra-arg-2"], importMeta, context);

      expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("excessive args"));
    });
    ...
    it("should show dry-run output with all parameters", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("threat feed data"));
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("npm"));
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining("test-pkg"));
    });
