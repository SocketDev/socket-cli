...
│   └── nested@1.0.0`;
  ...
  describe("lsStdoutIncludes", () => {
    const createEnvDetails = (agent: string): EnvDetails => ({ agent }) as unknown as EnvDetails;

    it("uses human format for bun agent", () => {
      const stdout = `├── lodash@4.17.21`;
      expect(lsStdoutIncludes(createEnvDetails("bun"), stdout, "lodash")).toBe(true);
    });

    it("uses human format for yarn/berry agent", () => {
      const stdout = `├── lodash@4.17.21`;
      expect(lsStdoutIncludes(createEnvDetails("yarn/berry"), stdout, "lodash")).toBe(true);
    });

    it("uses human format for yarn/classic agent", () => {
      const stdout = `├── lodash@4.17.21`;
      expect(lsStdoutIncludes(createEnvDetails("yarn/classic"), stdout, "lodash")).toBe(true);
    });

    it("uses query format for npm agent", () => {
      const stdout = `{"name":"lodash"}`;
      expect(lsStdoutIncludes(createEnvDetails("npm"), stdout, "lodash")).toBe(true);
    });

    it("uses query format for pnpm agent", () => {
      const stdout = `{"name":"lodash"}`;
      expect(lsStdoutIncludes(createEnvDetails("pnpm"), stdout, "lodash")).toBe(true);
    });

    it("uses query format for unknown agent", () => {
      const stdout = `{"name":"lodash"}`;
      expect(lsStdoutIncludes(createEnvDetails("unknown"), stdout, "lodash")).toBe(true);
    });

    it("returns false when package not found in bun format", () => {
      const stdout = `├── express@4.18.0`;
      expect(lsStdoutIncludes(createEnvDetails("bun"), stdout, "lodash")).toBe(false);
    });

    it("returns false when package not found in npm format", () => {
      const stdout = `{"name":"express"}`;
      expect(lsStdoutIncludes(createEnvDetails("npm"), stdout, "lodash")).toBe(false);
    });
  });
});
