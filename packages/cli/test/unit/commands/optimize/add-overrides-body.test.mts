...
async function loadAddOverrides(opts: {
  ...
  vi.doMock(import("@socketsecurity/registry-stable"), () => ({
    getManifestData: (agent?: string) => (agent === "npm" ? manifestEntries : []),
  }));
  ...
  vi.doMock(import("../../../../src/util/fs/glob.mts"), () => ({
    globWorkspace: opts.globWorkspace ?? vi.fn(async () => []),
    isReportSupportedFile: vi.fn(),
  }));
  vi.doMock(import("../../../../src/commands/optimize/get-dependency-entries.mts"), () => ({
    getDependencyEntries: opts.getDependencyEntries ?? vi.fn(() => []),
  }));
  vi.doMock(import("../../../../src/commands/optimize/get-overrides-by-agent.mts"), () => ({
    getOverridesData: opts.getOverridesData ?? vi.fn(() => ({ overrides: {}, type: "npm" })),
    getOverridesDataNpm: opts.getOverridesDataNpm ?? vi.fn(() => ({ overrides: {}, type: "npm" })),
    getOverridesDataYarnClassic:
      opts.getOverridesDataYarnClassic ?? vi.fn(() => ({ overrides: {}, type: "yarn" })),
  }));
  vi.doMock(import("../../../../src/commands/optimize/lockfile-includes-by-agent.mts"), () => ({
    lockSrcIncludes: opts.lockSrcIncludes ?? vi.fn(() => false),
  }));
  vi.doMock(import("../../../../src/commands/optimize/deps-includes-by-agent.mts"), () => ({
    lsStdoutIncludes: opts.lsStdoutIncludes ?? vi.fn(() => false),
  }));
  vi.doMock(import("../../../../src/commands/optimize/ls-by-agent.mts"), () => ({
    listPackages: opts.listPackages ?? vi.fn(async () => ""),
  }));
  vi.doMock(import("../../../../src/commands/optimize/update-manifest-by-agent.mts"), () => ({
    updateManifest: vi.fn(),
  }));
  vi.doMock(import("../../../../src/util/npm/package-arg.mts"), () => ({
    safeNpa: opts.safeNpa ?? vi.fn(() => undefined),
  }));
  vi.doMock(import("../../../../src/util/process/cmd.mts"), () => ({
    cmdPrefixMessage: (name: string, msg: string) => `[${name}] ${msg}`,
  }));
  vi.doMock(import("../../../../src/util/semver.mts"), () => ({
    getMajor: opts.getMajor ?? vi.fn((v: string) => parseInt(v, 10)),
  }));
  if (opts.fetchPackageManifest) {
    vi.doMock(import("@socketsecurity/lib-stable/packages/manifest"), async (importOriginal) => {
...
describe("addOverrides body (manifestNpmOverrides loop)", () => {
  ...
  it("skips entries when getMajor returns undefined (line 122-124)", async () => {
    const addOverrides = await loadAddOverrides({
      manifestEntries: [["pkg-a", { name: "pkg-a", package: "pkg-a-orig", version: "invalid" }]],
  ...
  it("adds an alias when origPkgName found in deps (line 158-167)", async () => {
    ...
    const addOverrides = await loadAddOverrides({
      manifestEntries: [["pkg-a", { name: "pkg-a", package: "pkg-a-orig", version: "1.2.3" }]],
