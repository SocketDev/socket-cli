...
vi.mock(import("@socketsecurity/lib-stable/constants/node"), async (importOriginal) => {
  const actual = await importOriginal();
  return {
    ...actual,
...
describe("agent installer utilities", () => {
  ...
  describe("runAgentInstall", () => {
    ...
    it("uses spinner when provided", async () => {
      const { Spinner } = vi.mocked(await import("@socketsecurity/lib-stable/spinner/spinner"));
      const mockSpinner = {
        start: vi.fn(),
        stop: vi.fn(),
      };
    ...
    it("handles unknown agent", async () => {
      ...
      expect(spawn).toHaveBeenCalledWith("/usr/bin/unknown-agent", ["install"], expect.any(Object));
    });
