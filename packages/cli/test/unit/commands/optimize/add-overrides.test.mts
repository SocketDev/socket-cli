...
vi.mock(import("../../../../src/util/fs/glob.mts"), () => ({
  globWorkspace: mockGlobWorkspace,
  isReportSupportedFile: vi.fn(),
}));

const mockGetDependencyEntries = vi.hoisted(() => vi.fn(() => []));
vi.mock(import("../../../../src/commands/optimize/get-dependency-entries.mts"), () => ({
  getDependencyEntries: mockGetDependencyEntries,
}));

const mockGetOverridesData = vi.hoisted(() => vi.fn(() => ({ overrides: {}, type: "npm" })));
const mockGetOverridesDataNpm = vi.hoisted(() => vi.fn(() => ({ overrides: {}, type: "npm" })));
const mockGetOverridesDataYarnClassic = vi.hoisted(() =>
  vi.fn(() => ({ overrides: {}, type: "yarn" })),
);
vi.mock(import("../../../../src/commands/optimize/get-overrides-by-agent.mts"), () => ({
...
vi.mock(import("../../../../src/commands/optimize/lockfile-includes-by-agent.mts"), () => ({
  lockSrcIncludes: vi.fn(() => false),
}));

vi.mock(import("../../../../src/commands/optimize/deps-includes-by-agent.mts"), () => ({
  lsStdoutIncludes: vi.fn(() => false),
}));

vi.mock(import("../../../../src/commands/optimize/ls-by-agent.mts"), () => ({
  listPackages: vi.fn(async () => ""),
}));

vi.mock(import("../../../../src/commands/optimize/update-manifest-by-agent.mts"), () => ({
  updateManifest: vi.fn(),
}));

const mockSafeNpa = vi.hoisted(() => vi.fn());
vi.mock(import("../../../../src/util/npm/package-arg.mts"), () => ({
  safeNpa: mockSafeNpa,
}));
...
vi.mock(import("../../../../src/util/semver.mts"), () => ({
  getMajor: mockGetMajor,
}));

const mockGetManifestData = vi.hoisted(() => vi.fn(() => []));
vi.mock(import("@socketsecurity/registry-stable"), () => ({
