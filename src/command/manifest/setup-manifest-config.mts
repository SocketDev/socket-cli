...
export async function setupConda(
  ...
  return await setupCondaOutput()

  async function setupCondaOutput(): Promise<CResult<{ canceled: boolean }>> {
...
export async function setupManifestConfig(
  ...
  const result = await setupSelectedManifest()

  async function setupSelectedManifest(): Promise<
    CResult<{ canceled: boolean }>
  > {
    let selectedResult: CResult<{ canceled: boolean }>
    switch (targetEco) {
      case 'conda': {
        if (!manifest.conda) {
          manifest.conda = {}
        }
        selectedResult = await setupConda(manifest.conda)
        break
      }
      case 'gradle': {
        if (!manifest.gradle) {
        ...
        selectedResult = await setupGradle(manifest.gradle)
        break
      }
      case 'maven': {
        if (!manifest.maven) {
        ...
        selectedResult = await setupMaven(manifest.maven)
        break
      }
      case 'sbt': {
        if (!manifest.sbt) {
        ...
        selectedResult = await setupSbt(manifest.sbt)
        break
      }
      default: {
        selectedResult = canceledByUser()
      }
    }
