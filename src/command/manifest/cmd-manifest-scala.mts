...
export async function run(
  ...
  const manifestSettings = sockJson.defaults?.manifest?.sbt ?? {}
  ...
  out = resolveScalaOutput()

  function resolveScalaOutput(): string {
    ...
    if (stdout) {
      ...
      if (manifestSettings.outfile) {
        out = manifestSettings.outfile
