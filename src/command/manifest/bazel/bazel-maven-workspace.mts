...
export async function processWorkspaceForMaven(config: {
  ...
  for (
    ...
    await processMavenCandidate(repoName)
  }
  if (verbose) {
    for (
  ...
  async function processMavenCandidate(repoName: string) {
    ...
    await writeMavenCandidate(repoName, result)
  }

  async function writeMavenCandidate(
    repoName: string,
