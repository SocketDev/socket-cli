...
// `arrayToLower` / `toLower` helpers are kept together at the top, alphabetical
// anchor for the cdxgen flag mapping below; `run` is the command entry point
// and lives near its config + cmdManifestCdxgen export, not interleaved with
// helpers.
/* oxlint-disable-next-line socket/no-file-scope-oxlint-disable -- legitimate file-scope: domain-grouped layout or test fixture; per-call would produce many redundant disables. */
/* oxlint-disable socket/sort-source-methods -- grouped by phase */
...
export async function run(
  ...
  const { pathArgs, unknowns } = partitionCdxgenPositionals(yargv._ as string[])
...
export function partitionCdxgenPositionals(positionals: string[]): {
  pathArgs: string[]
  unknowns: string[]
} {
