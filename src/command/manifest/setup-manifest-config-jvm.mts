...
export async function setupGradle(
  ...
  return await setupGradleOutput()

  async function setupGradleOutput(): Promise<CResult<{ canceled: boolean }>> {
...
export async function setupSbt(
  ...
  if (config.facts === false) {
    const pomOptions = await setupSbtPomOutput()
    if (!pomOptions.ok || pomOptions.data.canceled) {
      return pomOptions
    }
  } else {
    const factsOptions = await setupFactsOptions(config)
  ...
  return await setupSbtVerbose()

  async function setupSbtVerbose(): Promise<CResult<{ canceled: boolean }>> {
    const verbose = await askForVerboseFlag(config.verbose)
    /* c8 ignore start - interactive prompt cancellation, undefined return, requires raw inquirer mock setup */
    if (verbose === undefined) {
      return canceledByUser()
  ...
  async function setupSbtPomOutput(): Promise<CResult<{ canceled: boolean }>> {
