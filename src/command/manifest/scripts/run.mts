...
function assertMavenExtensionBuilt(jarPath: string): void {
  ...
  throw new Error(
    `Maven manifest extension jar not found at ${jarPath}. It is bundled in the published CLI; for local dev build it with: bash src/command/manifest/scripts/maven-extension/build-jar.sh`,
  )
}
...
async function runSbt(
  ...
  return await withTmpDir('socket-sbt-facts-', async globalBase => {
    ...
    const javaHome = cfg.env?.['JAVA_HOME'] ?? getEnvValue('JAVA_HOME')
    const javaHomeOpt =
