...
export async function convertSbtToMaven({
  ...
  try {
    ...
    return await processSbtPoms(output.stdout)
  } catch (e) {
    return reportConversionError(e)
  }

  async function processSbtPoms(
    stdout: string,
  ): Promise<CResult<ManifestResult>> {
    const poms: string[] = []
    ...
    return await copySbtPoms(poms)
  }

  async function copySbtPoms(poms: string[]): Promise<CResult<ManifestResult>> {
  ...
  function reportConversionError(e: unknown): CResult<ManifestResult> {
    const summary =
