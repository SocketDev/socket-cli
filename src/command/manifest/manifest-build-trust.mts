...
export function resolveGradleInvocation({
  ...
  const manifestSettings = socketJson?.defaults?.manifest?.gradle ?? {}
  const socketJsonBin = readBuildToolBin(manifestSettings.bin)
  const socketJsonOpts = manifestSettings.gradleOpts
...
export function resolveMavenInvocation({
  ...
  const cliBinPath = readBuildToolBin(cliBin)
  const manifestSettings = socketJson?.defaults?.manifest?.maven ?? {}
  const socketJsonBin = readBuildToolBin(manifestSettings.bin)
  const socketJsonOpts = manifestSettings.mavenOpts
...
export function resolveSbtInvocation({
  ...
  const pathBin = 'sbt'
  const cliBinPath = readBuildToolBin(cliBin)
  const manifestSettings = socketJson?.defaults?.manifest?.sbt ?? {}
  const socketJsonBin = readBuildToolBin(manifestSettings.bin)
  const socketJsonOpts = manifestSettings.sbtOpts
