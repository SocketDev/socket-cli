...
import { spawnCdxgen, spawnSynp } from '../../util/dlx/spawn.mjs'
...
export async function runCdxgen(argvObj: ArgvObject): Promise<DlxSpawnResult> {
  ...
  if (
    ...
    try {
      const synpResult = await spawnSynp(['--source-file', `./${YARN_LOCK}`], {
        ...dlxOpts,
        agent,
      })
  ...
  const cdxgenResult = await spawnCdxgen(argvObjectToArray(argvMutable), {
    ...dlxOpts,
    agent,
  })
