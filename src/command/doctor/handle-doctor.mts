...
import { ensurePnpmWorkspaceMinReleaseAge } from '../../core/optimize/update-pnpm-workspace-yaml.mts'
import { cmdPrefixMessage } from '../../util/process/cmd.mts'

import type { MinReleaseAgeOutcome } from '../../core/optimize/update-pnpm-workspace-yaml.mts'
...
export async function handleDoctor({
  ...
  if (!pkgEnvCResult.ok || !pkgEnvCResult.data) {
    outputDoctorEnvironmentFailure(pkgEnvCResult, outputKind)
    return
  }
...
export function outputDoctorEnvironmentFailure(
  pkgEnvCResult: Awaited<
    ReturnType<typeof detectAndValidatePackageEnvironment>
  >,
  outputKind: OutputKind,
): void {
