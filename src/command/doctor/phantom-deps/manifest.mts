...
export function collectEntryPoints(pkg: Record<string, unknown>): Entry[] {
  ...
  collectManifestBinEntries(pkg['bin'], out, seen)

  const exportsField = pkg['exports']
  collectManifestExportEntries(exportsField, out, seen)
...
export function collectManifestBinEntries(
  bin: unknown,
  out: Entry[],
  seen: EntrySeen,
): void {
...
export function collectManifestExportEntries(
...
export function parseManifest(raw: string): Manifest | undefined {
  ...
  if (typeof peerMeta === 'object' && peerMeta !== null) {
    const entries = Object.entries(peerMeta)
    for (let i = 0, { length } = entries; i < length; i += 1) {
      const { 0: peer, 1: cfg } = entries[i]!
      const optional = isObject(cfg) && cfg['optional'] === true
      if (optional) {
