...
export function fsResolve(
  ...
  return resolveDirectoryManifest(root, joined, surface, depth, exts)
}
...
export function resolveDirectoryManifest(
  root: string,
  joined: string,
  surface: Surface,
  depth: number,
  ...
  if (existsSync(pkgJsonPath)) {
    try {
      ...
      const entry = isObject(pkg) ? manifestEntry(pkg, surface) : undefined
      if (entry) {
...
export function resolveGraphTargets(
  root: string,
  file: string,
  occurrences: ReturnType<typeof extract>,
): string[] {
  ...
  for (let i = 0, { length } = occurrences; i < length; i += 1) {
    ...
    if (classifySpecifier(occ.spec).kind === 'relative') {
      ...
      const resolved = fsResolve(root, path.dirname(file), occ.spec, surface, 0)
      if (resolved) {
...
export function walk(root: string, entryPoints: readonly Entry[]): Walk {
  ...
  while (queue.length > 0) {
    ...
    const targets = resolveGraphTargets(root, file, occurrences)
  ...
  const references = collectGraphReferences(parsed, flags)
