...
export async function run(
  ...
  if (['enable', 'enabled', 'on'].includes(arg ?? '')) {
    enable = true
    disable = false
  } else if (['disable', 'disabled', 'off'].includes(arg ?? '')) {
    enable = false
    disable = true
  }
