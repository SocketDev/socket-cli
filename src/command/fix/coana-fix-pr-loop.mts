...
export async function runGhsaFixLoop(
  ...
  const repoInfo = fixEnv.repoInfo
  let count = 0
  ...
  for (let i = 0, { length } = unprocessedIds; i < length; i += 1) {
    ...
    try {
      ...
      const existingPrs = await getSocketFixPrs(repoInfo.owner, repoInfo.repo, {
        ghsaId,
      ...
      if (await hasExistingOpenPr()) {
        continue
      }

      async function hasExistingOpenPr(): Promise<boolean> {
        ...
        if (existingOpenPrs.length > 0) {
          const { 0: firstPr } = existingOpenPrs
      ...
      await completePrCreation()

      async function completePrCreation() {
        if (prResult.ok) {
