      ...
      expect(stdout).toMatchInlineSnapshot(`
        "Create a new Socket scan and report
          ...
          Reachability Options (when --reach is used)
            --dynamic-sbom-inference  Enables dynamic SBOM inference for full application reachability analysis. Recursively generates a Socket facts SBOM for every independent Gradle, sbt, and Maven build root, and implies --auto-manifest. For Gradle, sbt, and Maven, only dependencies present in these generated Socket facts SBOM files are used for analysis; other ecosystems are analyzed as normal.
