...
export class SCMComments {
  static checkForSocketComments({
    comments
  }: {
  ...
  static parseIgnoreCommand(line: string) {
  ...
  static processSecurityComment({
    ...
    for (const ignoreComment of ignoreComments) {
      const parsed = SCMComments.parseIgnoreCommand(
  ...
  // OK
  static getIgnoreOptions({ comments }: { comments: SocketComments }) {
  ...
  static removeAlerts({
    ...
    const { ignoreAll, ignoreCommands } = SCMComments.getIgnoreOptions({
      comments
    })
    ...
    for (const alert of newAlerts) {
      if (ignoreAll) {
        ...
        if (
          ignoreCommands.includes(purl) ||
          ignoreCommands.includes(purlStar)
        ) {
          console.log(
            `Alerts for ${alert.pkg_name}@${alert.pkg_version} ignored`
          )
        } else {
          console.log(
