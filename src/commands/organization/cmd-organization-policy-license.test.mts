...
describe('socket organization policy license', async () => {
    ...
    async cmd => {
      ...
      expect(`\n   ${stderr}`).toMatchInlineSnapshot(`
        ...
        undefined Invalid input
          Unable to determine organization slug
        Suggested actions:
  ...
  cmdit(
    ...
    async cmd => {
      ...
      expect(code, 'dry-run should exit with code 1 if validation fails').toBe(
        1,
      )
    },
  )
  ...
  cmdit(
    ...
    async cmd => {
      ...
      expect(code, 'dry-run should exit with code 0 if validation passes').toBe(
        0,
      )
    },
  )
