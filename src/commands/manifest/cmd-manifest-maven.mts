...
// Unlike gradle/sbt, Maven projects already ship a `pom.xml` that Socket scans
// natively, so there is no pom-generation path here: `socket manifest maven`
// only emits a Socket facts file describing the resolved dependency graph.
const config: CliCommandConfig = {
  commandName: 'maven',
  description: '[beta] Generate a Socket facts file for a Maven project',
  hidden: false,
  flags: {
    ...commonFlags,
    bin: {
      type: 'string',
      description: 'Location of the mvn binary to use, default: mvn (on PATH)',
    },
    includeConfigs: {
      type: 'string',
      description:
    ...
    excludeConfigs: {
      type: 'string',
      description:
        'Comma-separated glob patterns; Maven dependency scopes matching any pattern are skipped (applied after --include-configs)',
    },
    ignoreUnresolved: {
      type: 'boolean',
      description:
    ...
    mavenOpts: {
      type: 'string',
      description: 'Additional options to pass on to mvn, see `mvn --help`',
    },
    verbose: {
      type: 'boolean',
...
async function run(
  ...
  if (!bin) {
    if (sockJson.defaults?.manifest?.maven?.bin) {
      ...
      // `mvn` is more likely to be resolved against the PATH env (Maven
      // wrappers are far less universal than gradlew).
      bin = 'mvn'
    }
  }
  if (!mavenOpts) {
