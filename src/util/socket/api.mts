...
export async function handleApiCall<T extends SocketSdkOperations>(
  ...
  startApiCallSpinner(spinner, description)
  ...
  try {
    ...
    spinner?.stop()
    logApiCallResponse(sdkResult, spinner, description)
  } catch (e) {
    spinner?.stop()
...
export function logApiCallResponse(
  sdkResult: { success?: unknown | undefined },
  spinner: HandleApiCallOptions['spinner'],
 ...
 */
export function startApiCallSpinner(
  spinner: HandleApiCallOptions['spinner'],
  description?: string | undefined,
): void {
  if (description) {
