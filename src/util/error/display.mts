...
export function formatErrorForDisplay(
  ...
  const details = getErrorDisplayDetails(error, { ...opts, verbose, showStack })
 ...
 */
export function formatErrorStack(error: Error): string | undefined {
...
export function formatNetworkErrorMessage(error: NetworkError): string {
  const message = error.statusCode
    ...
    : error.message
  return appendCauseChain(message, error.cause)
}

export function formatRateLimitErrorMessage(error: RateLimitError): string {
...
export function getErrorDisplayDetails(
  error: unknown,
  config: ErrorDisplayOptions & { verbose: boolean; showStack: boolean },
): { body?: string | undefined; message: string; title: string } {
  ...
  if (error instanceof RateLimitError) {
    ...
    message = formatRateLimitErrorMessage(error)
  } else if (error instanceof AuthError) {
    title = 'Authentication error'
    message = appendCauseChain(error.message, error.cause)
  } else if (error instanceof NetworkError) {
    title = 'Network error'
    message = formatNetworkErrorMessage(error)
    ...
    body = config.showStack ? formatErrorStack(error) : undefined
