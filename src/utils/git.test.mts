...
import { parseGitRemoteUrl, getBaseBranch, gitBranch, getRepoInfo, detectDefaultBranch, gitCommit, gitCheckoutBranch, gitCreateBranch, gitDeleteBranch, gitPushBranch, gitCleanFdx, gitResetHard, gitEnsureIdentity } from './git.mts'
...
describe('git utilities', () => {
  ...
  describe('gitBranch', () => {
    it('returns current branch name', async () => {
      const { spawn } = vi.mocked(await import('@socketsecurity/registry/lib/spawn'))
      spawn.mockResolvedValue({ status: 0, stdout: 'feature-branch\n', stderr: '' } as any)
      
      const result = await gitBranch()
      expect(result).toBe('feature-branch\n')
      expect(spawn).toHaveBeenCalledWith('git', ['symbolic-ref', '--short', 'HEAD'], expect.any(Object))
    })

    it('handles detached HEAD state', async () => {
      const { spawn } = vi.mocked(await import('@socketsecurity/registry/lib/spawn'))
      spawn.mockRejectedValueOnce(new Error('Not on a branch'))
        .mockResolvedValueOnce({ status: 0, stdout: 'abc1234\n', stderr: '' } as any)
    ...
    it('handles spawn errors', async () => {
      const { spawn, isSpawnError } = vi.mocked(await import('@socketsecurity/registry/lib/spawn'))
  ...
  describe('gitCommit', () => {
    it('creates a commit with message and files', async () => {
      ...
      const result = await gitCommit('Test commit', ['file1.txt', 'file2.txt'], { cwd: '/test/dir' })
      expect(result).toBe(true)
      expect(spawn).toHaveBeenCalledWith('git', ['add', 'file1.txt', 'file2.txt'], expect.any(Object))
      expect(spawn).toHaveBeenCalledWith('git', ['commit', '-m', 'Test commit'], expect.any(Object))
    })
    ...
    it('handles commit without files', async () => {
      ...
      expect(spawn).not.toHaveBeenCalledWith('git', expect.arrayContaining(['add']), expect.any(Object))
    })
  })
  ...
  describe('gitCreateBranch', () => {
    it('creates a new branch', async () => {
      ...
      expect(result).toBe(true)
      expect(spawn).toHaveBeenCalledWith('git', ['show-ref', '--quiet', 'refs/heads/new-feature'], expect.any(Object))
      expect(spawn).toHaveBeenCalledWith('git', ['branch', 'new-feature'], expect.any(Object))
    })
  })
  ...
  describe('gitDeleteBranch', () => {
    it('deletes a local branch', async () => {
      ...
      expect(result).toBe(true)
      expect(spawn).toHaveBeenCalledWith('git', ['branch', '-D', 'old-feature'], expect.any(Object))
    })
  })
  ...
  describe('gitPushBranch', () => {
    it('pushes a branch to remote', async () => {
      ...
      expect(result).toBe(true)
      expect(spawn).toHaveBeenCalledWith('git', ['push', '--force', '--set-upstream', 'origin', 'feature'], expect.any(Object))
    })
  })
  ...
  describe('gitCleanFdx', () => {
    it('cleans untracked files', async () => {
      ...
      expect(result).toBe(true)
      expect(spawn).toHaveBeenCalledWith('git', ['clean', '-fdx'], expect.any(Object))
    })
  })
  ...
  describe('gitResetHard', () => {
    it('resets to a specific ref', async () => {
      ...
      expect(result).toBe(true)
      expect(spawn).toHaveBeenCalledWith('git', ['reset', '--hard', 'origin/main'], expect.any(Object))
    })
  })
  ...
  describe('gitEnsureIdentity', () => {
    it('sets git user name and email', async () => {
      ...
      expect(spawn).toHaveBeenCalledWith('git', ['config', '--get', 'user.email'], expect.any(Object))
      expect(spawn).toHaveBeenCalledWith('git', ['config', '--get', 'user.name'], expect.any(Object))
    })
  })
})
