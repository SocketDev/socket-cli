...
const mockGetOrgTelemetryConfig = vi.hoisted(() =>
  vi.fn(() =>
    Promise.resolve({
...
describe('TelemetryService', () => {
  ...
  describe('initialization', () => {
    it('fetches telemetry configuration on creation', async () => {
      ...
      expect(mockGetOrgTelemetryConfig).toHaveBeenCalledWith('test-org')
    })
    ...
    it('uses default config when SDK setup fails', async () => {
      ...
      expect(client).toBeDefined()
      expect(mockGetOrgTelemetryConfig).not.toHaveBeenCalled()
    })

    it('uses default config when config fetch fails', async () => {
      mockGetOrgTelemetryConfig.mockResolvedValueOnce({
  ...
  describe('flushing', () => {
    ...
    it('handles flush timeout', async () => {
      ...
      // Start the flush (returns a promise).
      const flushPromise = client.flush()

      // Advance timers past the timeout (2000ms).
  ...
  describe('destroy', () => {
    ...
    it('handles flush timeout during destroy', async () => {
      ...
      // Start the destroy (returns a promise).
  ...
  describe('TelemetryService lifecycle', () => {
    it('should create singleton instance per org', async () => {
      const client1 = await TelemetryService.getTelemetryClient('org1')
      const client2 = await TelemetryService.getTelemetryClient('org1')

      expect(client1).toBe(client2)
    })

    it('should flush pending events before finalization', async () => {
      const client = await TelemetryService.getTelemetryClient('org1')
    ...
    it('should handle multiple flush calls gracefully', async () => {
      const client = await TelemetryService.getTelemetryClient('org1')
    ...
    it('should not throw when flushing with no events', async () => {
      const client = await TelemetryService.getTelemetryClient('org1')

      await expect(client.flush()).resolves.not.toThrow()
    })

    it('should cleanup interval timer on finalization', async () => {
      const client = await TelemetryService.getTelemetryClient('org1')
      const instance = TelemetryService.getCurrentInstance()
