 ...
 *
 * Architecture:
 * - Uses update-checker for registry lookups
 * - Uses update-store for persistent caching
 * - Uses update-notifier for user messaging
...
async function checkForUpdates(
  ...
  if (!isFresh) {
    ...
    try {
      ...
      // Debug: Failed to fetch latest version (silent failure).
