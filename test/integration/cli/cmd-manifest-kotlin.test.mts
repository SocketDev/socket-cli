/**
 * Integration tests for `socket manifest kotlin` command.
 *
 ...
 *
 * Related Files: - src/command/manifest/cmd-manifest-kotlin.mts - Command
 * definition - src/command/manifest/handle-manifest-kotlin.mts - Kotlin
 * manifest logic.
 */
      ...
      expect(stdout).toMatchInlineSnapshot(`
        "[beta] Generate a Socket facts file (or \`pom.xml\` with --pom) for a Kotlin project
          ...
          Usage
              ...
              The default binary is \`CWD/gradlew\`. A socket.json that points \`bin\`
              ...
              yourself to override the defaults without trusting socket.json.
  ...
  cmdit(
    ...
    async cmd => {
      ...
      expect(`\n   ${stderr}`).toMatchInlineSnapshot(`
        ...
        [DryRun]: Would execute generate .socket.facts.json from Kotlin project

          Command: gradlew
          Arguments: [PROJECT] --bin gradle
