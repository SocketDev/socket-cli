 ...
 * environments).
 *
 * Related Files: - src/command/config/cmd-config-list.mts - Command
 * definition.
 *
 * - Src/commands/config/handle-config-list.mts - Config listing logic -
 *   src/command/config/output-config-list.mts - Output formatting.
 */
  ...
  cmdit(
    ...
    async cmd => {
      ...
      expect(`\n   ${stderr}`).toMatchInlineSnapshot(`
        ...
        [DryRun]: Would fetch configuration settings

          Query parameters:
            showFullTokens: no (masked)
