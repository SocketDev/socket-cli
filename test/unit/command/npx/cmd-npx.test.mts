...
import { cmdNpx } from '../../../../src/command/npx/cmd-npx.mts'
...
describe('npx firewall integration', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.run.mockResolvedValue({ code: 0, signal: noChildSignal })
    mocks.which.mockResolvedValue('/example/bin/npx')
  ...
  it.each([
    ...
    await cmdNpx.run(args, import.meta, context)
    expect(mocks.run).toHaveBeenCalledWith(['npx', ...args], {
      stdio: 'inherit',
    })
  })
  ...
  it.each(['SIGTERM', 'SIGINT'] as const)(
    ...
    async signal => {
      ...
      await cmdNpx.run([], import.meta, context)
  ...
  it('retains failure status when firewall setup rejects', async () => {
    ...
    await expect(cmdNpx.run([], import.meta, context)).rejects.toBeInstanceOf(
      Error,
    )
