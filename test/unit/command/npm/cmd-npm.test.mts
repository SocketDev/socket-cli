...
import { cmdNpm } from '../../../../src/command/npm/cmd-npm.mts'
...
describe('npm firewall integration', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.run.mockResolvedValue({ code: 0, signal: noChildSignal })
    mocks.which.mockResolvedValue('/example/bin/npm')
  ...
  it.each([
    ...
    await cmdNpm.run(args, import.meta, context)
    expect(mocks.run).toHaveBeenCalledWith(['npm', ...args], {
      stdio: 'inherit',
    })
  })
  ...
  it.each(['SIGTERM', 'SIGINT'] as const)(
    ...
    async signal => {
      ...
      await cmdNpm.run([], import.meta, context)
  ...
  it('retains failure status when firewall setup rejects', async () => {
    ...
    await expect(cmdNpm.run([], import.meta, context)).rejects.toBeInstanceOf(
      Error,
    )
