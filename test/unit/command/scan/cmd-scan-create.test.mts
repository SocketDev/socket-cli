...
vi.mock(import('../../../../src/command/scan/suggest-org-slug.mts'), () => ({
  suggestOrgSlug: mockSuggestOrgSlug,
}))

vi.mock(import('../../../../src/command/scan/suggest_target.mts'), () => ({
  suggestTarget: mockSuggestTarget,
}))

vi.mock(
  import('../../../../src/command/scan/validate-reachability-target.mts'),
  () => ({
...
describe('cmd-scan-create', () => {
  ...
  describe('run', () => {
    ...
    describe('--make-default-branch validation', () => {
      it('rejects an attached branch name', async () => {
        await cmdScanCreate.run(
