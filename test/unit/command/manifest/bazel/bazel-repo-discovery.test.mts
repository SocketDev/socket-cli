/**
 * Unit tests for Maven hub discovery: show_extension parsing, probe
 * classification, and the show_extension outcome classifier.
 */
...
import {
  ...
  probeCandidate,
} from '../../../../../src/command/manifest/bazel/bazel-repo-discovery.mts'
import type {
  ProbeResult,
  ProbeStatus,
  RepoProbe,
...
describe('bazel-repo-discovery', () => {
  ...
  describe('classifyShowExtensionResult', () => {
    ...
    // confirmed against a live `bazel mod show_extension` run; the strings
    // below are representative shapes except where marked verbatim.
    ...
    it('classifies the real Bazel "no module ... exists in the dependency graph" arg error (exit 2) as not-defined', () => {
      ...
      // without rules_jvm_external (verified on real Bazel: exit code 2).
      // This is the dominant no-Maven case and must never be escalated to
      // indeterminate / hardFailure.
      expect(
        classifyShowExtensionResult(
          probeResult({
            code: 2,
    ...
    it('classifies the real Bazel unbound-name MODULE.bazel failure (exit 2) as indeterminate', () => {
      // Verbatim stderr from `bazel mod show_extension --enable_bzlmod` on a
      // MODULE.bazel fragment with an unbound name (verified on real Bazel:
      // exit 2). A genuine eval failure: we cannot conclude maven is absent,
      // so it is indeterminate even though the unbound-name text also trips
      // the not-in-graph "not defined" branch (eval-failure is checked first).
      expect(
        classifyShowExtensionResult(
          probeResult({
            code: 2,
