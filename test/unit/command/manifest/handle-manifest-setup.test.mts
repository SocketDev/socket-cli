 ...
 *
 * Related Files: - src/command/manifest/handle-manifest-setup.mts - Command
 * handler - src/command/manifest/setup-manifest-config.mts - Configuration
 * setup logic - src/command/manifest/output-manifest-setup.mts - Output
 * formatting.
 */

import { beforeEach, describe, expect, it, vi } from 'vitest'

import { handleManifestSetup } from '../../../../src/command/manifest/handle-manifest-setup.mts'
...
vi.mock(
  import('../../../../src/command/manifest/output-manifest-setup.mts'),
  () => ({
    outputManifestSetup: mockOutputManifestSetup,
  }),
)
vi.mock(
  import('../../../../src/command/manifest/setup-manifest-config.mts'),
  () => ({
...
describe('handleManifestSetup', () => {
  ...
  it('sets up manifest config successfully', async () => {
    const { setupManifestConfig } =
      await import('../../../../src/command/manifest/setup-manifest-config.mts')
    const { outputManifestSetup } =
      await import('../../../../src/command/manifest/output-manifest-setup.mts')
