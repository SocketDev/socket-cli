/**
 * Unit tests for handleThreatFeed.
 *
 * Purpose: Tests the handler that orchestrates threat feed retrieval. Validates
 * feed fetching, filtering, and output formatting.
 *
 ...
 * pipeline.
 *
 * Related Files: - src/command/handleThreatFeed.mts (implementation)
 */

import { beforeEach, describe, expect, it, vi } from 'vitest'

import { handleThreatFeed } from '../../../../src/command/threat-feed/handle-threat-feed.mts'

// Mock the dependencies.
vi.mock(
  import('../../../../src/command/threat-feed/fetch-threat-feed.mts'),
  () => ({
    fetchThreatFeed: vi.fn(),
  }),
)
vi.mock(
  import('../../../../src/command/threat-feed/output-threat-feed.mts'),
  () => ({
...
describe('handleThreatFeed', () => {
  ...
  it('fetches and outputs threat feed successfully', async () => {
    const { fetchThreatFeed } =
      await import('../../../../src/command/threat-feed/fetch-threat-feed.mts')
    const { outputThreatFeed } =
      await import('../../../../src/command/threat-feed/output-threat-feed.mts')
