 ...
 *
 * Related Files: - src/command/fix/coana-fix.mts - Main fix implementation -
 * src/command/fix/handle-fix.mts - Fix command handler.
 */

import { beforeEach, describe, expect, it, vi } from 'vitest'

import { coanaFix } from '../../../../src/command/fix/coana-fix.mts'

import type { FixConfig } from '../../../../src/command/fix/types.mts'
...
vi.mock(import('../../../../src/command/fix/branch-cleanup.mts'), () => ({
...
vi.mock(import('../../../../src/command/fix/ghsa-tracker.mts'), () => ({
  isGhsaFixed: vi.fn(() => false),
  markGhsaFixed: vi.fn(),
}))

vi.mock(import('../../../../src/command/fix/pr-lifecycle-logger.mts'), () => ({
  logPrEvent: vi.fn(),
}))
...
describe('socket fix --limit behavior verification', () => {
  ...
  describe('local mode (no PRs)', () => {
    it('should process only N GHSAs when --limit N is specified', async () => {
      ...
      // Verify spawnCoana was called once with only the first 3 GHSAs.
    ...
    it('should process no GHSAs when --limit 0 is specified', async () => {
      ...
      // spawnCoana should not be called at all with limit 0.
