...
import { cmdBundler } from '../../../../src/command/bundler/cmd-bundler.mts'
...
describe('bundler firewall integration', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.run.mockResolvedValue({ code: 0, signal: noChildSignal })
    mocks.which.mockResolvedValue('/example/bin/bundler')
  ...
  it('retains command metadata', () => {
    expect(cmdBundler).toMatchObject({
  ...
  it.each([
    ...
    await cmdBundler.run(args, import.meta, context)
    expect(mocks.run).toHaveBeenCalledWith(['bundler', ...args], {
      stdio: 'inherit',
    })
  })
  it('filters wrapper prefix flags and preserves child configuration', async () => {
    await cmdBundler.run(
      ['--config', '{}', '--no-banner', 'install', '--config', 'child.json'],
      import.meta,
      context,
    )
    expect(mocks.run).toHaveBeenCalledWith(
      ['bundler', 'install', '--config', 'child.json'],
      { stdio: 'inherit' },
    )
  })
  ...
  it.each(['SIGTERM', 'SIGINT'] as const)(
    ...
    async signal => {
      ...
      await cmdBundler.run([], import.meta, context)
  ...
  it('retains failure status when firewall setup rejects', async () => {
    ...
    await expect(
      cmdBundler.run([], import.meta, context),
