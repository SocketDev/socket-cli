 ...
 * organization license policy configuration. Validates organization slug
 * forwarding, output format selection, and error propagation through the
 * fetch/output pipeline.
 *
 * Test Coverage: - Successful license policy fetch and output - Fetch error
 * handling and propagation - Multiple output format support (json, text,
 * markdown) - Organization slug parameter passing.
 *
 * Testing Approach: Mocks fetchLicensePolicy and outputLicensePolicy modules to
 * test orchestration logic without actual API calls or terminal output. Uses
 * test helpers for CResult pattern validation.
 *
 * Related Files: - src/command/organization/handle-license-policy.mts -
 * Command handler - src/command/organization/fetch-license-policy.mts -
 * License policy fetcher - src/command/organization/output-license-policy.mts.
 *
 * - Output formatter.
 */
...
import { handleLicensePolicy } from '../../../../src/command/organization/handle-license-policy.mts'
import { outputLicensePolicy } from '../../../../src/command/organization/output-license-policy.mts'
import {
  createErrorResult,
  createSuccessResult,
...
vi.mock(
  import('../../../../src/command/organization/fetch-license-policy.mts'),
  () => ({
...
vi.mock(
  import('../../../../src/command/organization/output-license-policy.mts'),
  () => ({
