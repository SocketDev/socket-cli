...
describe('lockfile-readers', () => {
  ...
  describe('LOCKS', () => {
    it('maps bun lockfiles to bun', () => {
      expect(LOCKS.get('bun.lock')).toBe('bun')
      expect(LOCKS.get('bun.lockb')).toBe('bun')
    })

    it('maps npm lockfiles to npm', () => {
      expect(LOCKS.get('package-lock.json')).toBe('npm')
      expect(LOCKS.get('npm-shrinkwrap.json')).toBe('npm')
    })

    it('maps pnpm lockfile to pnpm', () => {
      expect(LOCKS.get('pnpm-lock.yaml')).toBe('pnpm')
    })

    it('maps yarn.lock to yarn/classic', () => {
      // Agent name uses '/' as the separator between flavor variants.
      expect(LOCKS.get('yarn.lock')).toBe('yarn/classic')
    })

    it('maps the hidden .package-lock.json to npm', () => {
      expect(LOCKS.get('node_modules/.package-lock.json')).toBe('npm')
    })
