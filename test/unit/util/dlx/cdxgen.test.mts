...
import { spawnCdxgen } from '../../../../src/util/dlx/spawn.mts'

// Mock spawnDlx function.
vi.mock(import('../../../../src/util/dlx/spawn.mts'), () => {
  const mockSpawnDlx = vi.fn()
  // Return the actual implementation for spawnCdxgen.
  return {
...
describe('spawnCdxgen', () => {
  ...
  it('passes options through to spawnDlx', async () => {
    ...
    await spawnCdxgen(['--output', 'sbom.json'], options)
  ...
  it('returns spawnDlx result', async () => {
    ...
    const result = await spawnCdxgen(['--type', 'npm'])
  ...
  it('handles recursive scanning arguments', async () => {
    ...
    await spawnCdxgen(['-r', '/path/to/scan'])
