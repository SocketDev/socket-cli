//
// Socket Brotli2C - JavaScript to C++ converter with Brotli compression.
// Based on Node.js js2c tool with Socket Security enhancements.
//
#include <algorithm>
#include <array>
#include <cassert>
...
std::vector<uint8_t> CompressWithBrotli(const std::vector<char>& code,
  ...
  if (!result) {
    fprintf(stderr, "Brotli compression failed for %s\n", var.c_str());
    return {};
  }
...
Fragment GetDefinitionImpl(const std::vector<char>& code,
  ...
  constexpr size_t unit = (is_two_byte ? 5 : 3) + 1;
...
Fragment GetDefinition(const std::string& var, const std::vector<char>& code) {
  ...
  std::vector<uint8_t> compressed = CompressWithBrotli(code, var);
