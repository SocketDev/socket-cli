...
| `git --no-verify` (skips the `.git-hooks/` chain), or `HUSKY=0 git …` (the heavier husky-compatible form — one-shot skip of EVERY hook type, including post-commit which has no `--no-verify` flag; humans may also `export HUSKY=0` for a session) | `Allow no-verify bypass` |
...
| `brew` invocation while Homebrew is below 6.0.0 or unhardened (`HOMEBREW_REQUIRE_TAP_TRUST` / `HOMEBREW_CASK_OPTS_REQUIRE_SHA` unset) — `brew upgrade` to clear the floor, run `setup-security-tools` to set the knobs | `Allow brew-supply-chain bypass` |
...
| `git push --force` / `-f` / `--force-with-lease` (any spelling, bare or `=<branch>:<expected-sha>`) / `--force-if-includes` — one phrase unlocks BOTH the bare form and the safer lease form; the block message teaches the fleet default `--force-with-lease=<branch>:$(git rev-parse origin/<branch>)`. Legacy aliases from before the split still work: `Allow force-with-lease bypass`, `Allow force-push-hard bypass`. | `Allow force-push bypass` |
| Lease-force push TO A PROTECTED BRANCH (the squash-repo reconcile): the branch-scoped combo satisfies BOTH `no-force-push-guard` and `push-protected-branch-guard` in one phrase, and only for pushes whose destination is exactly `<branch>` — it cannot leak to a different branch or a later unrelated push. | `Allow force-with-lease <branch> bypass` |
...
| READING the system clipboard from a script or hook via a read CLI (`pbpaste` / `wl-paste` / `xclip -o` / `xsel`) or an OSC-52 escape in source (`no-clipboard-access-guard`). Reading is an exfil surface; explicit writes (`pbcopy`) are allowed. Bypass only for an operator-driven read. | `Allow clipboard-access bypass` |
...
| Landing a CLAUDE.md edit that leaves the file over the 40 KB whole-file cap (`claude-md-size-guard`). One phrase authorizes one over-cap edit; prefer trimming detail into `docs/agents.md/fleet/<topic>.md` first. | `Allow claude-md-size bypass` |
| Ending a turn with a dirty PRIMARY checkout — uncommitted/untracked/staged-but-uncommitted (`dirty-worktree-stop-guard`). For the rare can't-commit-yet case; prefer committing, or stack WIP in a linked worktree and defer via `git commit --no-verify`. | `Allow dirty-worktree bypass` |
...
4. Cascade via `node scripts/sync-scaffolding.mts --all --fix` so every fleet repo picks up the change.
